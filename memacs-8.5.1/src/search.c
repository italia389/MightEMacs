...
// character.  The final pattern length is returned.  The basic rule for interpreting a pattern is as follows: "If a pattern
// ends with a colon followed by one or more valid option letters, and the colon is not the first character of the pattern, then
...
bool exactbmode(void) {
	...
	return (srch.m.flags & SOpt_Exact) || ((modetab[MdRec_Global].flags & MdExact) && !(srch.m.flags & SOpt_Ignore));
	}

// Return true if buffer Match record specifies Regexp search mode.
bool rebmode(void) {

	return (srch.m.flags & SOpt_Regexp) || ((modetab[MdRec_Global].flags & MdRegexp) && !(srch.m.flags & SOpt_Plain));
	}
...
// Make original search pattern from given Match record and store in dest.  Return dest.
...
// false) via newspat() or newrpat().  If script mode, *argpp contains the pattern; otherwise, get it interactively.  In the
// latter case, the pattern variable is not updated if the user enters an empty line.  If that happens and there is no old
// pattern, it is an error.  Return status.
int getpat(Datum **argpp,char *prmt,bool srchpat) {
	...
	if(opflags & OpScript) {
		...
		if(terminp(tpatp,prmt,srchpat ? mkpat(patbuf,&srch.m) : srch.m.rpat,srch.sdelim,0,srchpat ? CFNotNull1 : 0,0)
		 != Success)
			return rc.status;
		}

	// Check search string.
	if(srchpat && disnull(tpatp))
		return rcset(Failure,RCNoFormat,text80);	// Null search string not allowed.
				// "No pattern set"
	// New search pattern?
	if(srchpat) {
		ushort flags = 0;
...
int searchForw(Datum *rp,int n,Datum **argpp) {
	...
	if(n == INT_MIN)
		...
		return searchBack(rp,-n,argpp);		// Search backward.
	...
	if(getpat(argpp,text78,true) == Success)
			// "Search"
...
int huntForw(Datum *rp,int n,Datum **argpp) {
	...
	if(n == INT_MIN)
		...
		return huntBack(rp,-n,argpp);		// Search backward.
	...
	if(((rebmode() && (srch.m.flags & SRegical)) ? mcscan(n,Forward) : scan(n,Forward)) == NotFound)
		dsetbool(false,rp);
...
int searchBack(Datum *rp,int n,Datum **argpp) {
	...
	if(n == INT_MIN)
		...
		return searchForw(rp,-n,argpp);		// Search forward.

	// Ask the user for the text of a pattern.  If the response is Success, search for the pattern up to n times, as long as
	// the pattern is there to be found.
	if(getpat(argpp,text81,true) == Success)
			// "Reverse search"
...
int huntBack(Datum *rp,int n,Datum **argpp) {
	...
	if(n == INT_MIN)
		...
		return huntForw(rp,-n,argpp);		// Search forward.
	...
	// Perform appropriate search and return result.
	if(((rebmode() && (srch.m.flags & SRegical)) ? mcscan(n,Backward) : scan(n,Backward)) == NotFound)
		dsetbool(false,rp);
	else if(datcpy(rp,srch.m.matchp) != 0)
		(void) drcset();
	return rc.status;
...
Fail:
		...
		if(loopCount <= ProgressLoopCt) {
			if(loopCount++ == ProgressLoopCt)
				if(mlputs(MLWrap | MLForce,text379) != Success)
						// "Searching..."
					return rc.status;
			}
		}
		...
		if(mcp->mc_type & MCE_Closure) {
			...
			case MCE_EOSAlt:
				if(sdot.type == ScanDot_Buf) {
					...
					switch(mcp->mc_type) {
					case MCE_BOL:
						if(dotp->off != 0 && lgetc(dotp->lnp,dotp->off - 1) != '\n')
							return false;
						break;
					case MCE_EOL:
						if(dotp->off != lused(dotp->lnp) && lgetc(dotp->lnp,dotp->off) != '\n')
							return false;
						break;
					case MCE_BOS:
						if(flags & SOpt_Multi) {
...
int showRegexp(Datum *rp,int n,Datum **argpp) {
	...
	do {
		...
		if(mcobjp->pat == srch.m.bmcpat && dputs("\n\n",&rpt) != 0)
			return drcset();
		if(dputf(&rpt,"%s %s %s /",mcobjp->hdr,text309,text311) != 0 || dvizs(patbuf,0,VBaseDef,&rpt) != 0 ||
		 dputs("/\n",&rpt) != 0)
						// "SEARCH","PATTERN"
			return drcset();
...
Grp:
	...
	if(dputf(&rpt,"\n\n%s %s /",text310,text311) != 0 || dvizs(srch.m.rpat,0,VBaseDef,&rpt) != 0 ||
	 dputs("/\n",&rpt) != 0)
				// "SEARCH","PATTERN"
		return drcset();
