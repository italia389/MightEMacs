...
// All arrays are kept in a linked list of ArrayWrapper structures.  (See comments for adispose() function for usage details.)
...
typedef struct {
	...
	int p_termch;			// Statement termination character (TokC_Comment or TokC_ExprEnd).
...
// are used.
#define Term_NoKeyEcho	0x00000100	// Don't echo keystrokes.
#define Term_OneKey	0x00000200	// Get one key only.
#define Term_Eval	0x00000400	// Evaluate string read from terminal.
#define Term_C_NoAuto	0x00000800	// Don't auto-complete; wait for return key.
#define Term_C_CFAM	0x00001000	// Command, function, alias, or macro name.
#define Term_C_Buffer	0x00002000	// Buffer name.
#define Term_C_Filename	0x00004000	// Filename (via directory search).
#define Term_C_BMode	0x00008000	// Buffer mode name.
#define Term_C_GMode	0x00010000	// Global mode name.
#define Term_C_Var	0x00020000	// Variable name -- all.
#define Term_C_SVar	0x00040000	// Variable name -- excluding constants.
...
extern int atosfc(DStrFab *destp,Datum *srcp,char *dlm,uint flags);
...
extern int dobuf(Datum *rp,int n,struct Buffer *bufp,char *runpath,ushort flags);
extern int doestmt(Datum *rp,char *cl,int termch,char **clp);
extern int dofile(Datum *rp,char *fname,int n,ushort flags);
#if MMDebug & Debug_Array
extern int dtosf(DStrFab *destp,Datum *srcp,char *dlm,uint flags);
#endif
extern int dtosfc(DStrFab *destp,Datum *srcp,char *dlm,uint flags);
...
extern int parsebegin(Parse *newp,char *cl,int termch);
extern void parseend(Parse *oldp);
extern int parsetok(Datum *destp,char **srcp,int delim);
extern void ppfree(struct Buffer *bufp);
extern int quote(DStrFab *destp,char *src,bool full);
extern int rparsetok(Datum *destp,char **srcp,char *base,int delim);
...
extern int strfmt(Datum *rp,int n,Datum *formatp,Datum *arg1p);
