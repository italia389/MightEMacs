...
#define NKEYWORDS	(sizeof(kwtab) / sizeof(KeywordInfo)) - 1
...
// Return keyword name, given index.  Used in binary() calls.
static char *kwname(int i) {
...
// Set and return proper status from a failed Datum object function call.
int drcset(void) {

	return rcset((plexcep.flags & ExcepMem) ? Panic : Failure,0,"%s",plexcep.msg);
	}
...
int tostr(Datum *datp) {
	...
	if(datp->d_type == dat_int) {
		...
		if(datp->d_type == dat_nil) {
			...
			if(dopentrk(&sf) != 0)
				goto DFail;
			if(atosfc(&sf,datp,NULL,0) == Success) {
...
int parsetok(Datum *destp,char **srcp,int delim) {
	int delim1,delim2,c;
	char *src0,*src;

	// Check if source is null.
	src = (delim == -1) ? nonwhite(*srcp) : *srcp;
	if(*src == '\0')
		return NotFound;
...
int rparsetok(Datum *destp,char **srcp,char *base,int delim) {
	int delim1,delim2,c;
	char *src,*srcz;
...
int parsebegin(Parse *newp,char *cl,int termch) {
...
// character.  Set *srcp to terminator or trailing null and return symbol.
...
// last->p_termch is TokC_Comment, or the (TokC_ExprEnd) terminator otherwise, and return NotFound (bypassing rcset()).
// Notes:
...
int getsym(void) {
	...
	if((c = *src) != '\0' && c != last->p_termch) {
		switch(c) {
			case '"':
			case '\'':
				sym = getslit(&src,c);		// String literal.
...
Unk:
	...
	last->p_sym = sym;
	last->p_cl = (*src == last->p_termch && last->p_termch == TokC_Comment) ? strchr(src,'\0') : src;
#if MMDebug & Debug_Token
