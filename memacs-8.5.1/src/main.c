...
int abortOp(Datum *rp,int n,Datum **argpp) {
	...
	if(buildmsg(&datp,text189) == Success)
			// "Abort"
		(void) rcset(UserAbort,RCNoFormat,datp->d_str);
...
int notice(Datum *rp,int n,Datum **argpp) {
	...
	if(havesym(s_any,true)) {
		...
		if(buildmsg(&datp,NULL) == Success) {
			...
			(void) rcset(Success,n <= 0 && n != INT_MIN ? RCNoFormat | RCNoWrap | RCForce : RCNoFormat | RCForce,
			 datp->d_str);
			}
		}
...
Done:
	...
	if(gotoflag) {
		...
		if(searchflag)
			return rcset(FatalError,RCNoFormat,text101);
