...
# to 'C-c C-f'.
$MemacsMacroRE = '^[ \t]*macro +%NAME%\b:re'

##### Macros #####

# Wrap "if 0" and "endif" around a block of lines.
macro memacsWrapIf0(0) {desc: 'Wrap "if 0" and "endif" around a block of lines'}
	$0 => langWrapPrep false,'','0','endif'
endmacro

# Wrap "if 1" and "endif" around a block of lines.
macro memacsWrapIf1(0) {desc: 'Wrap "if 1" and "endif" around a block of lines'}
	$0 => langWrapPrep false,'','1','endif'
endmacro

# Wrap "if 0" and "else" around a block of lines, duplicate them, and add "endif".
macro memacsWrapIfElse(0) {desc: 'Wrap "if 0" and "else" around a block of lines, duplicate them, and add "endif"'}
	$0 => langWrapIfElse ''
endmacro

# Find first file that contains requested macro name and render it according to n argument if found.
macro memacsFindMacro(0) {desc: 'Find first file that contains requested macro name (with "selectBuf" options)'}
...
macro memacsGotoBlockEnd(0) {desc: "Go to block directive paired with one in current line and set marks '.','T','E','B'"}
	formerMsgState = -1 => alterGlobalMode 'msg'
	wordChars = $wordChars
