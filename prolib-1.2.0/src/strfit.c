...
char *strfit(char *dest,size_t maxlen,char *src,size_t srclen) {
	size_t slen,len,cutsize;
	char *str,*ellipsis;

	// Check for minimum shrinking parameters.
	slen = (srclen == 0) ? strlen(src) : srclen;
	if(maxlen < 5 || slen <= maxlen)

		// Dest or source too short, so just copy the maximum.
		(void) stplcpy(dest,src,(slen < maxlen ? slen : maxlen) + 1);
	else {
