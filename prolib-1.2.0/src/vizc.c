...
char *vizc(int c,uint flags) {
	uint base;
	static char lit[6];

	// Valid base?
	if((base = flags & VBaseMask) > VBaseMax) {
		(void) emsgf(-1,"vizc(): Invalid base (%u)",base);
		return NULL;
		}
...
int fvizc(int c,uint flags,FILE *fp) {
	char *str;
...
int fvizs(const void *str,size_t len,uint flags,FILE *fp) {
	char *str1 = (char *) str;
