...
// given ncurses window (unless TAScanOnly flag is set) and update *strp and TAAltUL flag in *flags if applicable; otherwise,
...
int termattr(char **strp,char *strz,ushort *flags,WINDOW *wp) {
	short c,fg,bg;
	attr_t attr;
	int attrLen = 1;
	char *str = *strp;
	...
	if((c = *str++) == AttrSpecBegin) {		// ~~ sequence?
		if(!(*flags & TAScanOnly)) {		// Yes, process it.
...
// string is displayed (zero if none).  Invalid sequences or any that would begin at or past "maxoff" are ignored.  If len < 0,
// string is assumed to be null terminated.
int attrCount(char *str,int len,int maxoff) {
	char *strz;
	int curoff;			// Running count of visible characters.
	ushort flags = TAScanOnly;

	strz = (len < 0) ? strchr(str,'\0') : str + len;
	curoff = len = 0;
	while(str < strz && curoff < maxoff) {
...
int wupd_reframe(EWindow *win) {
	Line *lnp;
	int i;
	int nlines;
	Line *dotlnp = win->w_face.wf_point.lnp;

	// Get current window size.
	nlines = win->w_nrows;

	// If not a forced reframe, check for a needed one.
	if(!(win->w_flags & WFReframe)) {

		// If the top line of the window is at EOB...
		if((lnp = win->w_face.wf_toplnp)->l_next == NULL && lnp->l_used == 0) {
			if(lnp == win->w_buf->b_lnp)
				return rc.status;	// Buffer is empty, no reframe needed.
			win->w_face.wf_toplnp = win->w_buf->b_lnp;
			goto Reframe;			// Buffer is not empty, reframe.
			}

		// Check if point is in the window.
		if(inwind(win,dotlnp))
...
Reframe:
	...
	// macro buffer), the line containing point is not in the window, or it's a force.  Set i to the new window line number
	// (0 .. nlines - 1) where the point line should "land".  If it's a force, use w_rfrow for i and just range-check it;
	// otherwise, calcluate a reasonable value (either vjump/100 from top, center of window, or vjump/100 from bottom)
	// depending on direction point has moved.
	i = win->w_rfrow;
	...
	if(!(win->w_flags & WFReframe)) {
		...
		forwlnp = backlnp = win->w_face.wf_toplnp;
		lnp1 = win->w_buf->b_lnp;
		i = 0;
		for(;;) {
			...
			if(backlnp == dotlnp) {
				...
				i = (i > 1) ? nlines / 2 : (vtc.vjump > 0) ? nlines * vtc.vjump / 100 :
				 (dotlnp->l_next == NULL) ? nlines / 2 : 0;
				break;
				}

			// Advance forward and back.
			if(forwlnp->l_next != NULL)
				forwlnp = forwlnp->l_next;
			else {
				if(backlnp == lnp1)
					break;
				goto Back;
...
Back:
	...
	// Forced reframe.  Make sure i is in range (within the window).
		...
		i = nlines / 2;			// Center point line in window.

	// Now set top line i lines above point line.
	wnewtop(win,dotlnp,-i);
...
static int vupd_wind(EWindow *win) {
	Line *lnp;		// Line to update.
	int row;		// Virtual screen line to update.
	int endrow;		// Last row in current window + 1.
	bool attrOn = (win->w_buf->b_flags & BFTermAttr) && (!modeset(MdIdxHScrl,win->w_buf) ||
	 win->w_face.wf_firstcol == 0);
	Line *curlnp = (win == si.curwin) ? win->w_face.wf_point.lnp : NULL;

	// Loop through the lines in the window, updating the corresponding rows in the virtual screen.
	lnp = win->w_face.wf_toplnp;
	endrow = (row = win->w_toprow) + win->w_nrows;
	do {
		// Update the virtual line.
		vt.taboff = modeset(MdIdxHScrl,win->w_buf) ? win->w_face.wf_firstcol :
		 lnp == curlnp ? si.curscr->s_firstcol : 0;
...
static int supd_dex(void) {
	...
	win = si.whead;
	do {
		attrOn = (win->w_buf->b_flags & BFTermAttr) && (!modeset(MdIdxHScrl,win->w_buf) ||
		 win->w_face.wf_firstcol == 0);
		lnp = win->w_face.wf_toplnp;
		endrow = (row = win->w_toprow) + win->w_nrows;
		do {
			...
			if((win != si.curwin || lnp != win->w_face.wf_point.lnp) && ((*vt.vlp & VFExt) ||
			 ((*vt.vlp & VFPoint) && attrOn))) {
				if(lnp != NULL) {
#if VTDebug && 0
...
static int vupd_dotline(EWindow *win) {
	Line *lnp;		// Line to update.
	...
	lnp = win->w_face.wf_toplnp;
	row = win->w_toprow;
	while(lnp != win->w_face.wf_point.lnp) {
		++row;
		lnp = lnp->l_next;
		}

	// and update the virtual line.
	vt.taboff = modeset(MdIdxHScrl,win->w_buf) ? win->w_face.wf_firstcol : (win == si.curwin) ? si.curscr->s_firstcol : 0;
	if(vtmove(row,-vt.taboff) == Success) {
		vtputln(lnp,win != si.curwin && (win->w_buf->b_flags & BFTermAttr) &&
		 (!modeset(MdIdxHScrl,win->w_buf) || win->w_face.wf_firstcol == 0));
		vteeol();
		vt.taboff = 0;
		}
	return rc.status;
...
// Redisplay the mode line for the window pointed to by win.  If popbuf is NULL, display a fully-formatted mode line containing
...
static int vupd_modeline(EWindow *win,Buffer *popbuf) {
	short c;
	int n;
	Buffer *buf;
	Datum *el;
	...
	BufMode *bmp = win->w_buf->b_modes;
	bool colorML = term.itemColor[ColorIdxML].colors[0] >= -1;
	...
	n = win->w_toprow + win->w_nrows;		// Row location.
	...
	if(colorML)			// Use color mode line if possible; otherwise, reverse video.
		...
		ttrev(false,true);
	if(win == si.curwin)				// Make the current window's mode line stand out.
		lchar = '=';
	...
	if(popbuf == NULL) {
		buf = win->w_buf;
		vtputc((buf->b_flags & BFNarrowed) ? SBNarrowed : lchar);	// < if narrowed.
		vtputc((buf->b_flags & BFChanged) ? SBChanged :			// * if changed.
		 (buf->b_flags & BFReadOnly) ? SBReadOnly : lchar);		// # if changed.
		vtputc(' ');
		n = 3;

		// Is window horizontally scrolled?
		if(win->w_face.wf_firstcol > 0) {
			sprintf(wkbuf,"[<%d] ",win->w_face.wf_firstcol);
			n += vtputs(wkbuf);
			}
		...
		if(win->w_next == NULL) {
			...
			if(kmacro.km_state == KMRecord) {
				if(term.itemColor[ColorIdxKMRI].colors[0] >= -1) {
			...
			if(scrcount() > 1) {
				sprintf(wkbuf,"S%hu ",si.curscr->s_num);
				n += vtputs(wkbuf);
				}
			}

		// Display the line and/or column point position if enabled.
		if(modeset(MdIdxLine,buf)) {
			sprintf(wkbuf,"L:%ld ",getlinenum(buf,win->w_face.wf_point.lnp));
			n += vtputs(wkbuf);
			}
		if(modeset(MdIdxCol,buf)) {
			sprintf(wkbuf,"C:%d ",getcol(&win->w_face.wf_point,false));
			n += vtputs(wkbuf);
			}
		...
		do {
			...
			if(win->w_next == NULL || mli != mltab) {
				c = mli->leadch;
				for(;;) {
					if(mli == mltab) {
						// Get next global mode.
						if((el = aeach(&ary)) == NULL)
							break;
						mspec = msptr(el);
					...
					if(!(mspec->ms_flags & MdHidden)) {
						n += vtputc(c);
						c = ' ';
						if(condensed >= 0)
							n += vtputs(mspec->ms_name);
						else {
							n += vtputc(mspec->ms_name[0]);
							if(mspec->ms_name[1] != '\0')
								n += vtputc(mspec->ms_name[1]);
							}
						}
					}
				if(c != mli->leadch) {
					n += vtputc(mli->trailch);
					n += vtputc(' ');
					}
				}
			} while((++mli)->leadch > 0);
		...
		vtputc(lchar);
		vtputc((win->w_flags & WFMode) ? 'M' : lchar);
		vtputc((win->w_flags & WFHard) ? 'H' : lchar);
		vtputc((win->w_flags & WFEdit) ? 'E' : lchar);
		vtputc((win->w_flags & WFMove) ? 'V' : lchar);
		vtputc((win->w_flags & WFReframe) ? 'R' : lchar);
		vtputc(lchar);
		n += 8;
#endif
		if(n > 3)
	...
	if(buf->b_fname != NULL) {
		...
		if(condensed < 0) {
			...
			n += vtputs(text34);
				// "File: "
		n += vtputs(strfit(wkbuf,term.t_ncol - n - 1,buf->b_fname,0)) + 1;
		vtputc(' ');
		}

	// If it's the current window, not a pop-up, "WkDir" mode, and room still available, display the working directory as
	// well.
	if(win == si.curwin && popbuf == NULL && (mi.cache[MdIdxWkDir]->ms_flags & MdEnabled) && (term.t_ncol - n) > 12) {
		char wkbuf[TT_MaxCols];

		n += vupd_tab(lchar);
		n += vtputs(text274);
			// "WD: "
		n += vtputs(strfit(wkbuf,term.t_ncol - n - 1,si.curscr->s_wkdir,0)) + 1;
		vtputc(' ');
		}

	// If bottom window, not a pop, and root still available, display program name and version.
	if(!condensed && popbuf == NULL && win->w_next == NULL) {
...
static int vupd_cursor(void) {
	...
	lnp = wfp->wf_toplnp;
	lastrow = si.curscr->s_cursrow;
	si.curscr->s_cursrow = si.curwin->w_toprow;
	...
	si.curscr->s_curscol = i = 0;
	while(i < wfp->wf_point.off)
		si.curscr->s_curscol = newcol(lnp->l_text[i++],si.curscr->s_curscol);
	...
	if(modeset(MdIdxHScrl,NULL))
		si.curscr->s_curscol -= *(firstcol = &wfp->wf_firstcol);
	else {
		if(si.curscr->s_cursrow == lastrow)
			si.curscr->s_curscol -= si.curscr->s_firstcol;
		else
			si.curscr->s_firstcol = 0;
		firstcol = &si.curscr->s_firstcol;
		}

	// Make sure it is not off the left edge of the screen or on a LineExt ($) character at the left edge.
	while(si.curscr->s_curscol < 0 || (si.curscr->s_curscol == 0 && si.curscr->s_firstcol > 0)) {
		if(*firstcol >= vtc.hjumpcols) {
			si.curscr->s_curscol += vtc.hjumpcols;
			*firstcol -= vtc.hjumpcols;
			}
		else {
			si.curscr->s_curscol += *firstcol;
			*firstcol = 0;
			}
		si.curwin->w_flags |= (WFHard | modeflag);
		}

	// Calculate window or line shift if needed.
	while(si.curscr->s_curscol >= term.t_ncol - 1) {
		si.curscr->s_curscol -= vtc.hjumpcols;
		*firstcol += vtc.hjumpcols;
		si.curwin->w_flags |= (WFHard | modeflag);
		}

	// Mark line as a "point" line and flag it if extended so that it can be re-rendered and/or de-extended by supd_dext()
	// in a future update() cycle.
	vlp = &vlflags[si.curscr->s_cursrow];
	if(*firstcol > 0 && !modeset(MdIdxHScrl,NULL))
		*vlp |= (VFPoint | VFExt);
	else
		*vlp = *vlp & ~VFExt | VFPoint;

	// Update the virtual screen if current line or left shift was changed, or terminal attributes are "live".
	if(si.curwin->w_flags & WFHard) {
#if VTDebug && 0
ushort u; mlprintf(MLHome | MLFlush,
 "Calling vupd_wind() from vupd_cursor(): wf_firstcol %d, curscr->s_firstcol %d, curscr->s_curscol %d...",
 wfp->wf_firstcol,si.curscr->s_firstcol,si.curscr->s_curscol); getkey(true,&u,false);
#endif
		if(vupd_wind(si.curwin) != Success)
			return rc.status;
		}
	else if((si.curwin->w_flags & WFEdit) && vupd_dotline(si.curwin) != Success)
		return rc.status;
	...
	if((si.curwin->w_flags & WFMode) || modeset(MdIdxCol,NULL) || (si.curwin->w_flags && modeset(MdIdxLine,NULL)))
		vupd_modeline(si.curwin,NULL);
	si.curwin->w_flags = 0;

	// If horizontal scrolling mode not enabled and line is shifted, put a '$' in column 0.
	if(!modeset(MdIdxHScrl,NULL) && si.curscr->s_firstcol > 0)
		(void) mvaddch(si.curscr->s_cursrow,0,LineExt);
	return rc.status;
	}
...
int update(int n) {
			...
			// Does current screen need to shrink vertically?
				...
				do {
					...
					if(win->w_toprow >= term.t_nrow - 2) {
						...
						// Update si.curwin and lastwin if needed.
						if(win == si.curwin)
							if(wswitch(si.whead,0) <= FatalError)	// Defer any non-fatal error.
								return rc.status;
						if(lastwin != NULL)
							lastwin->w_next = NULL;
						...
						if(win->w_toprow + win->w_nrows - 1 >= term.t_nrow - 2) {
							win->w_nrows = term.t_nrow - win->w_toprow - 2;
							win->w_flags |= (WFHard | WFMode);
							}
						nrow += win->w_nrows + 1;
						}
				...
				si.curscr->s_nrow = nrow;
				}
			} while(si.curscr->s_nrow != term.t_nrow);
	...
	if((si.opflags & OpScrRedraw) && (n == INT_MIN || n > 0)) {
	...
	win = si.whead;
	do {
		if(win->w_flags) {					// WFMove is set, at minimum.

			// The window has changed in some way: service it.
			if(wupd_reframe(win) != Success)		// Check the framing.
				return rc.status;
			if((win->w_flags & (WFEdit | WFMove)) ==	// Promote update if WFEdit and WFMove both set or
			 (WFEdit | WFMove) || ((win->w_flags & WFMove)	// WFMove set and terminal attributes enabled in buffer.
			 && (win->w_buf->b_flags & BFTermAttr)))
				win->w_flags |= WFHard;

			// Skip remaining tasks for current window.  These will be done by vupd_cursor().
			if(win != si.curwin) {
				if((win->w_flags & ~WFMode) == WFEdit) {
					if(vupd_dotline(win) != Success)	// Update current (edited) line only.
						return rc.status;
					}
				else if(win->w_flags & WFHard) {
					if(vupd_wind(win) != Success)	// Update all lines.
						return rc.status;
					}
				if(win->w_flags & WFMode)
					vupd_modeline(win,NULL);	// Update mode line.
				win->w_flags = win->w_rfrow = 0;
				}
			}
	...
	if(n == INT_MIN || n > 0) {
		...
		if(ttmove(si.curscr->s_cursrow,si.curscr->s_curscol) == Success)
			(void) ttflush();
		}
...
// Do "updateScreen" command.  Return status.
int updscr(Datum *rval,int n,Datum **argv) {
	...
	if(n != INT_MIN) {
		...
		if(options[0].cflags & OptSelected) {
			if(options[1].cflags & OptSelected)
				n = -1;
			else
				n = 1;
			}
...
static bool iscmd(uint ek,int (*cfunc)(Datum *rval,int n,Datum **argv)) {
	KeyBind *kbind;

	return (kbind = getbind(ek)) != NULL && kbind->k_targ.p_type == PtrCmd && kbind->k_targ.u.p_cfp->cf_func == cfunc;
	}

// Find key(s) bound to given command, convert to string form (if not prefixed), and write to "prmt" underscored.  Return -1
// if error.  Used by bpop() for help prompt.
static int hkey(int (*cfunc)(Datum *rval,int n,Datum **argv),DStrFab *prmt) {
	...
	kbind = nextbind(&kw);
	do {
		if(kbind->k_targ.p_type == PtrCmd && kbind->k_targ.u.p_cfp->cf_func == cfunc) {

			// Found one.  Skip if prefixed or function key.
			if(!(kbind->k_code & (Prefix | FKey))) {
				if(dputf(prmt,"%c%c%c%s%c%c",c,AttrSpecBegin,AttrULOn,ektos(kbind->k_code,wkbuf,false),
	...
	return dputf(prmt," %c%c%c%s%c%c,",AttrSpecBegin,AttrBoldOn,cmd0,cmd,AttrSpecBegin,AttrBoldOff);
	}
...
int bpop(Buffer *buf,ushort flags) {
	Line *lnp1,*lnp,*lpmax;
	...
	vupd_modeline(wnextis(NULL),flags & RendAltML ? buf : NULL);
	...
	if(flags & RendShift) {
		...
		wkpoint.lnp = buf->b_lnp;
		do {
			wkpoint.off = wkpoint.lnp->l_used;
			if((col = getcol(&wkpoint,false)) > maxcol)
				maxcol = col;
			} while((wkpoint.lnp = wkpoint.lnp->l_next) != NULL);
	...
	for(;;) {
		...
		// Moving forward?
			...
			if(lpmax != NULL)
				do {
					...
					if(lnp1->l_next == NULL || lnp1 == lpmax)
						break;
		...
		if(n != 0 && lnp1 == lnp)
			...
			do {
				...
				if(vtmove(row,-vt.taboff) != Success)
					return rc.status;
				vtputln(lnp,firstcol == 0 && (buf->b_flags & BFTermAttr));
				vteeol();
				vt.taboff = 0;
		...
		for(;;) {
			...
			// SPC f C-v +pg, b C-z -pg, d +half, u -half, C-n +ln, C-p -ln, g top, G bot, ESC q quit, ? help:
				...
				dputf(&prompt," %c%cd%c%c %c%c+half%c%c, %c%cu%c%c %c%c-half%c%c,",AttrSpecBegin,AttrULOn,
				 AttrSpecBegin,AttrULOff,AttrSpecBegin,AttrBoldOn,AttrSpecBegin,AttrBoldOff,
				 AttrSpecBegin,AttrULOn,AttrSpecBegin,AttrULOff,AttrSpecBegin,AttrBoldOn,
		 ...
		 " %c%cg%c%c %c%ctop%c%c, %c%cG%c%c %c%cbot%c%c, %c%cESC%c%c|%c%cq%c%c %c%cquit%c%c, %c%c?%c%c %c%chelp%c%c: ",
		 		 AttrSpecBegin,AttrULOn,AttrSpecBegin,AttrULOff,AttrSpecBegin,AttrBoldOn,
		 		 AttrSpecBegin,AttrBoldOff,AttrSpecBegin,AttrULOn,AttrSpecBegin,AttrULOff,
		 		 AttrSpecBegin,AttrBoldOn,AttrSpecBegin,AttrBoldOff,AttrSpecBegin,AttrULOn,
		 		 AttrSpecBegin,AttrULOff,AttrSpecBegin,AttrULOn,AttrSpecBegin,AttrULOff,
		 		 AttrSpecBegin,AttrBoldOn,AttrSpecBegin,AttrBoldOff,AttrSpecBegin,AttrULOn,
				...
				if(mlputs(MLHome | MLTermAttr | MLFlush,prompt.sf_datum->d_str) != Success)
					return rc.status;
				}
			else {
