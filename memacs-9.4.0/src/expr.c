...
//	agarbfree() function.
//   4. The job of agarbfree() is to scan the aryGarb list, freeing arrays which are not associated with a global variable.
...
//	include nested arrays.  "aryGarb" is set to NULL at this point so that the list will not be scanned every time the
//	user presses a key.
...
typedef struct {
	...
	enum e_sym *sym;		// Valid operator token(s).
...
static int ausage(bool first) {
	...
	if(aryGarb != NULL) {
		...
		Datum **elp,**elpz;
		ArraySize i;
		ArrayWrapper *aw = aryGarb;

		fprintf(logfile,"Array List: *%s*\n\n",first ? "BEFORE" : "AFTER");
		do {
			ary = aw->aw_ary;
			fprintf(logfile,"\tFound wrapper %.8X -> array %.8X (keep %d):\n",(uint) aw,(uint) ary,aw->aw_mark);
			elpz = (elp = ary->a_elp) + ary->a_used;
			i = 0;
			while(elp < elpz) {
				fprintf(logfile,"\t\t%2d: ",i++);
				if((*elp)->d_type == dat_blobRef) {
					ary = awptr(*elp)->aw_ary;
					fprintf(logfile,"ARRAY [%.8X], size %d\n",(uint) ary,ary->a_used);
					}
				else
					fprintf(logfile,((*elp)->d_type & DStrMask) ? "\"%s\"\n" : "%s\n",dtos(*elp,true));
				++elp;
				}
			} while((aw = aw->aw_next) != NULL);
		if(first)
			fputc('\n',logfile);
		}
	return rc.status;
...
static int ausage1(ArrayWrapper *aw,DStrFab *strloc) {
	Datum **elp = aw->aw_ary->a_elp;
	Datum **elpz = elp + aw->aw_ary->a_used;
	bool first = true;
	...
	while(elp < elpz) {
		if(first)
			first = false;
		else if(dputc(',',strloc) != 0)
			goto LibFail;
		if((*elp)->d_type == dat_blobRef) {
			Array *ary = awptr(*elp)->aw_ary;
			if(dputf(strloc,"[%.8X:%.8X:%d]",(uint) awptr(*elp),(uint) ary,ary->a_used) != 0)
				goto LibFail;
			}
		else if(dtosf(strloc,*elp,NULL,CvtExpr) != Success)
			return rc.status;
		++elp;
		}
	if(dputc(']',strloc) != 0)
...
static int ausage(DStrFab *strloc,int stage) {
	...
	if(stage <= 0) {
		...
		// Dump array info.
		if((stage < 0 && dopentrk(strloc) != 0) ||
		 dputf(strloc,"Array List: *%s*\n\n",stage < 0 ? "BEFORE" : "AFTER") != 0)
			goto LibFail;
		do {
			if(dputf(strloc,"%.8X   %.8X   %c",(uint) aw,(uint) aw->aw_ary,aw->aw_mark ? '*' : ' ') != 0)
				goto LibFail;
#if 0
			dsetblobref((void *) aw,sizeof(ArrayWrapper),&d);
			if(atosfclr(strloc,&d,NULL,CvtExpr | CvtForceArray) != Success)
				return rc.status;
#else
			if(ausage1(aw,strloc) != Success)
				return rc.status;
#endif
			if(dputc('\n',strloc) != 0)
				goto LibFail;
		...
		for(uvar = gvarshead; uvar != NULL; uvar = uvar->uv_next) {
			if(uvar->uv_datum->d_type == dat_blobRef) {
				aw = awptr(uvar->uv_datum);
				if(dputf(strloc,"%-18s %.8X   %.8X   %c",uvar->uv_name,(uint) aw,(uint) aw->aw_ary,
				 aw->aw_mark ? '*' : ' ') != 0)
					goto LibFail;
				if(ausage1(awptr(uvar->uv_datum),strloc) != Success)
					return rc.status;
				if(dputc('\n',strloc) != 0)
					goto LibFail;
				}
		...
		if(bfind("Debug",BS_Create,0,&buf,&created) != Success || (!created && bclear(buf,BC_IgnChgd) != Success))
			return rc.status;
		if(bappend(buf,strloc->sf_datum->d_str) == Success) {
			Datum d;

			dinit(&d);
			(void) render(&d,2,buf,RendReset);
			dclear(&d);
			}
		}
	return rc.status;
...
static void agarbpushall(Datum *datum) {
	ArrayWrapper *agp = aryGarb;
	ArrayWrapper *aw = awptr(datum);
...
int agarbfree(void) {
	...
	if(aryGarb != NULL) {
		...
		for(uvar = gvarshead; uvar != NULL; uvar = uvar->uv_next)
			if(uvar->uv_datum->d_type == dat_blobRef)
				agarbpush(uvar->uv_datum);

		// Clear all "marked" flags in wrapper list.
		aclrmark();

#if MMDebug & Debug_Array
		if(ausage(&sfab,-1) != Success)
			return rc.status;
#endif
...
#if MMDebug & Debug_Array
				if(dputf(&sfab,"Freeing array %.8X...\n",(uint) aw->aw_ary) != 0)
					return librcset(Failure);
#endif
				free((void *) aclear(aw->aw_ary));
				free((void *) aw);
				}
			} while(aryGarb != NULL);
#if MMDebug & Debug_Array
...
// status.
int awrap(Datum *rval,Array *ary) {
	ArrayWrapper *aw;

	if((aw = (ArrayWrapper *) malloc(sizeof(ArrayWrapper))) == NULL)	// Create wrapper object...
		return rcset(Panic,0,text94,"awrap");
			// "%s(): Out of memory!"
	aw->aw_ary = ary;							// initialize it...
	aw->aw_next = aryGarb;						// add it to garbage list...
	aryGarb = aw;
	dsetblobref((void *) aw,sizeof(ArrayWrapper),rval);			// and set in Datum object.
	return rc.status;
	}

// Pop datGarb to given pointer, releasing heap space for Datum objects and arrays, if applicable.
void garbpop(Datum *datum) {
	Datum *datum1;
...
void nodeinit(ENode *np,Datum *rval,bool toplev) {
...
bool charval(Datum *datum) {

	return isval(datum->d_type == dat_int && datum->u.d_int >= 0 && datum->u.d_int <= 0xFF,text288);
			// "Character expected"
	}

// Return true if a datum object is an integer; otherwise, set an error and return false.
bool intval(Datum *datum) {

	return isval(datum->d_type == dat_int,text166);
			// "Integer expected"
	}

// Return true if a datum object is a string; otherwise, set an error and return false.
bool strval(Datum *datum) {

	return isval(datum->d_type & DStrMask,text171);
			// "String expected"
	}

// Return true if a datum object is an array; otherwise, set an error and return false.
bool aryval(Datum *datum) {

	return isval(datum->d_type == dat_blobRef,text371);
			// "Array expected"
	}
...
void dumpval(Datum *datum) {
	char *str;

	fprintf(logfile,"(%.8X %hu) ",(uint) datum,datum->d_type);
	switch(datum->d_type) {
		case dat_nil:
			str = viz_nil;
			break;
		...
		case dat_true:
			...
			break;
		case dat_int:
			fprintf(logfile,"%ld",datum->u.d_int);
			return;
		case dat_miniStr:
		case dat_soloStr:
			fprintf(logfile,"\"%s\"",datum->d_str);
			return;
		default:
			str = "[array]";
		}
...
static void ge_dump(char *fname,ENode *np1,ENode *np2,bool begin) {
	...
	fprintf(logfile,", EvalMode %x",si.opflags & OpEval);
	if(begin)
		fputs("...\n",logfile);
	else
		fprintf(logfile," [%d]\n",rc.status);
...
static int ge_primary(ENode *np) {
	...
	switch(last->p_sym) {
		...
		case s_clit:
			...
			if(si.opflags & OpEval) {
				short c;
				char *str = last->p_tok.d_str + 1;	// Past '?'.
				if((c = *str) == '\\' && evalclit(&str,&c,true) != Success)
					return rc.status;
				dsetint(c,np->en_rp);
				}		
			goto NextSym;
		case s_slit:
			// String literal.
			if((si.opflags & OpEval) && evalslit(np->en_rp,last->p_tok.d_str) != Success)
				return rc.status;
			goto NextSym;
		...
		case s_lbrkt:
			...
			for(;;) {
				...
				if(ge_andor(np) != Success)
					return rc.status;
				if((si.opflags & OpEval) && apush(ary,np->en_rp,true) != 0)
					goto LibFail;
...
static int fcall(ENode *np,bool needrparen,bool *found) {
	UnivPtr univ;
...
#endif
	if(execfind(np->en_rp->d_str,OpQuery,PtrCFAM,&univ)) {
#if MMDebug & (Debug_Datum | Debug_Expr)
		fputs("found '",logfile);
		switch(univ.p_type) {
			case PtrAlias_C:
			case PtrAlias_F:
			case PtrAlias_M:
				fprintf(logfile,"%s' ALIAS",univ.u.p_alias->a_name);
				break;
			case PtrMacro_C:
			case PtrMacro_O:
				fprintf(logfile,"%s' MACRO",univ.u.p_buf->b_bname);
				break;
			case PtrCmd:
			case PtrFunc:
				fprintf(logfile,"%s' %s",univ.p_type == PtrCmd ? "COMMAND" : "FUNCTION",univ.u.p_cfp->cf_name);
			}
		fprintf(logfile," (type %hu) for token '%s'\n",univ.p_type,np->en_rp->d_str);
#endif
		...
		if(univ.p_type == PtrCmd) {
			if(univ.u.p_cfp->cf_aflags & CFTerm)
				return rcset(Failure,RCTermAttr,text282,univ.u.p_cfp->cf_name);
					// "'~b%s~B' command not allowed in a script (use '~brun~B')"

			// If "alias" command (which uses "alias xxx = yyy" syntax), parentheses not allowed.
			if(needrparen && univ.u.p_cfp->cf_func == aliasCFM)
		...
		// Set to -1 if unknown.
		short minArgs = (univ.p_type & PtrMacro) ? univ.u.p_buf->b_mip->mi_minArgs :
		 !(univ.u.p_cfp->cf_aflags & (CFAddlArg | CFNoArgs)) ? univ.u.p_cfp->cf_minArgs : !(si.opflags & OpEval) ? -1 :
		 np->en_narg == INT_MIN ? univ.u.p_cfp->cf_minArgs : (univ.u.p_cfp->cf_aflags & CFNoArgs) ? 0 :
		 univ.u.p_cfp->cf_minArgs + 1;
		...
		if(needrparen && havesym(s_rparen,false)) {
			...
			if(!(univ.p_type & PtrMacro)) {
				if((univ.u.p_cfp->cf_aflags & CFNoArgs) && !(np->en_flags & EN_HaveNArg))
					goto Wrong;
				if(!(si.opflags & OpEval) && !(univ.u.p_cfp->cf_aflags & CFSpecArgs))
					goto Retn;
				}
			}
		...
		if(univ.p_type & PtrMacro)
			maxArgs = (univ.u.p_buf->b_mip->mi_maxArgs < 0) ? SHRT_MAX : univ.u.p_buf->b_mip->mi_maxArgs;
		else if((maxArgs = univ.u.p_cfp->cf_maxArgs) < 0)
			maxArgs = SHRT_MAX;
		else if((si.opflags & OpEval) && (univ.u.p_cfp->cf_aflags & (CFAddlArg | CFNoArgs))) {
			if((univ.u.p_cfp->cf_aflags & CFNoArgs) && np->en_narg != INT_MIN)
				maxArgs = 0;
			else if((univ.u.p_cfp->cf_aflags & CFAddlArg) && np->en_narg == INT_MIN)
				--maxArgs;
			}
		si.opflags = (si.opflags & ~OpParens) | (needrparen ? OpParens : 0);
		...
		// just a repeat count.
		if(((univ.p_type & (PtrCmd | PtrFunc)) && (univ.u.p_cfp->cf_aflags & CFSpecArgs)) || ((si.opflags & OpEval) &&
		 ((univ.p_type & PtrMacro) || np->en_narg != 0 || !(univ.u.p_cfp->cf_aflags & CFNCount)))) {
			...
			if(univ.p_type & PtrMacro) {
				...
				(void) debug_execbuf(np->en_rp,(int) np->en_narg,univ.u.p_buf,NULL,
				 needrparen ? ArgFirst | SRun_Parens : ArgFirst,"fcall");
#else
				(void) execbuf(np->en_rp,(int) np->en_narg,univ.u.p_buf,NULL,
				 needrparen ? ArgFirst | SRun_Parens : ArgFirst);
#endif
				}
			else {
				CmdFunc *cfp = univ.u.p_cfp;
				if(!(si.opflags & OpEval) || allowedit(cfp->cf_aflags & CFEdit) == Success) {
#if MMDebug & Debug_Expr
			...
			if(rc.status != Success)
				return rc.status;
			if((si.opflags & OpEval) && !fevalcall)
				(void) rcsave();
			}
		else {
...
Wrong:
		...
		if(found == NULL)
			return rcset(Failure,0,text244,np->en_rp->d_str);
					// "No such command, alias, or macro '~b%s~B'"
		*found = false;
#if MMDebug & (Debug_Datum | Debug_Expr)
		fputs("not a FAB.\n",logfile);
#endif
...
static int ge_postfix(ENode *np) {
	bool needrparen;
	uint oldparens = si.opflags & OpParens;
		...
		switch(last->p_sym) {
			...
			case s_lbrkt:
				...
				if(!(np->en_flags & EN_HaveWhite)) {
					...
					else if((si.opflags & OpEval) && !aryval(np->en_rp))
						goto Retn;
					...
					if(needsym(s_comma,false)) {
						...
						if(ge_andor(&node2) != Success)
							return rc.status;
						if((si.opflags & OpEval) && !intval(node2.en_rp))
							return rc.status;
						}
					...
					if(si.opflags & OpEval) {
						...
						if(haveTwo) {
							...
							if((ary = aslice(awptr(np->en_rp)->aw_ary,i1,node2.en_rp->u.d_int,
							 true,false)) == NULL)
								goto LibFail;
							if(awrap(np->en_rp,ary) != Success)
								return rc.status;
							}
						else {
							np->en_index = i1;
...
Retn:
	si.opflags = (si.opflags & ~OpParens) | oldparens;
	return rc.status;
LibFail:
	return librcset(Failure);
	}
	...
	switch(sym = last->p_sym) {
		...
		case s_bnot:
			...
			if(sym == s_incr || sym == s_decr) {
				...
				// Perform ++ or -- operation if evaluating.
				if((si.opflags & OpEval) && lvalue(np,true))
...
static int concat(ENode *np1,ENode *np2,bool clone) {
	...
	if(np1->en_rp->d_type == dat_blobRef) {
		...
		if(clone && aryclone(np1->en_rp,np1->en_rp,0) != Success)
			return rc.status;
		ary1 = awptr(np1->en_rp)->aw_ary;

		// If node2 is an array, expand it.
		if(np2->en_rp->d_type == dat_blobRef) {
			if(agraph(ary1,awptr(np2->en_rp)->aw_ary) == NULL)
				goto LibFail;
			}
		else {
			if(apush(ary1,np2->en_rp,false) != 0)
				goto LibFail;
			duntrk(np2->en_rp);
			}
		}
		...
		if(dopenwith(&sfab,np1->en_rp,SFAppend) != 0 || dputd(np2->en_rp,&sfab) != 0 || dclose(&sfab,sf_string) != 0)
			goto LibFail;
		}
	return rc.status;
LibFail:
...
static int aequal(Datum *datum1,Datum *datum2,bool ignore,bool *result) {
	ArrayWrapper *awp1 = awptr(datum1);
	ArrayWrapper *awp2 = awptr(datum2);
	if(awp1->aw_mark || awp2->aw_mark)
		return rcset(Failure,RCNoFormat,text195);
			// "Endless recursion detected (array contains itself)"
	Array *ary1 = awp1->aw_ary;
	Array *ary2 = awp2->aw_ary;
	...
	Datum **elp1 = ary1->a_elp;
	Datum **elp2 = ary2->a_elp;
	Datum *el1,*el2;
	do {
		el1 = *elp1++;
		el2 = *elp2++;
		if(el1->d_type == dat_blobRef) {
			if(el2->d_type != dat_blobRef)
				goto FalseRetn;
			if(aequal(el1,el2,ignore,&rval) != Success)
				return rc.status;
			if(!rval)
				goto Retn;
			}
		else if(!dateq(el1,el2,ignore))
			goto FalseRetn;
		} while(--size > 0);
TrueRetn:
	rval = true;
...
int aryeq(Datum *datum1,Datum *datum2,bool ignore,bool *result) {

	agarbpush(datum1);
	agarbpush(datum2);
	aclrmark();
	return aequal(datum1,datum2,ignore,result);
	}
...
static int ge_binop(ENode *np,OpInfo *oip) {
	...
	for(;;) {
		...
		// ignore it as well.
		if(si.opflags & OpEval) {
			if(*sym == s_band && (((oip->flags & FF_Concat) && np->en_rp->d_type == dat_int) ||
			 ((oip->flags & FF_BitOp) && ((np->en_flags & EN_Concat) || np->en_rp->d_type != dat_int))))
				goto Retn;
			if(*sym == s_mod && (((oip->flags & FF_Format) && np->en_rp->d_type == dat_int) ||
		...
		if(si.opflags & OpEval) {
			if(forcefit(np,&node2,*sym == s_req || *sym == s_rne ? FF_REQNE : oip->flags,op->d_str) != Success)
				goto Retn;
			switch(*sym) {
...
DivZero:
				...
				case s_ne:
					...
					if(np->en_rp->d_type != node2.en_rp->d_type)
						...
						if(np->en_rp->d_type == dat_blobRef) {
							if(aryeq(np->en_rp,node2.en_rp,false,&b) != Success)	// Cmp arrays.
								return rc.status;
							}
						else
				...
				case s_lt:
					...
					dsetbool(*sym == s_lt ? lval1 < lval2 : *sym == s_le ? lval1 <= lval2 :
					 *sym == s_gt ? lval1 > lval2 : lval1 >= lval2,np->en_rp);
					}
					break;
					...
					if(newspat(node2.en_rp->d_str,&rematch,NULL) == Success) {
						...
						if(mccompile(&rematch) == Success) {
							...
							if(recmp(np->en_rp,0,&rematch,&offset) == Success)
								dsetbool((offset >= 0) == (*sym == s_req),np->en_rp);
							}
						}
				}
			np->en_flags &= ~EN_LValue;
...
// Do logical and/or.
#if MMDebug & Debug_Expr
static int ge_landor(ENode *np,char *fname,int (*fnc)(ENode *np)) {
#else
static int ge_landor(ENode *np,int (*fnc)(ENode *np)) {
#endif
	bool b;
	Datum *rp2;
	ENode node2;
	...
	// Call function at next higher level.
#if MMDebug & Debug_Expr
	if(ge_debug1(fname,np,NULL,fnc) != Success)
#else
	if(fnc(np) != Success)
#endif
		return rc.status;
...
#endif
					goto Retn;
				if(!(si.opflags & OpEval)) {			// Eating arguments?
#if MMDebug & Debug_Expr
					if(ge_debug1(fname,&node2,NULL,fnc) != Success)
#else
					if(fnc(&node2) != Success)		// Yes, bon appetit.
#endif
						goto Retn;
					}
				else if(tobool(np->en_rp) == b) {		// No, does first argument determine outcome?
					dsetbool(b,np->en_rp);			// Yes, convert to logical...
					si.opflags &= ~OpEval;			// and eat second argument.
#if MMDebug & Debug_Expr
					(void) ge_debug1(fname,np,&node2,fnc);
#else
					(void) fnc(&node2);
#endif
					si.opflags |= OpEval;
...
#if MMDebug & Debug_Expr
					if(ge_debug1(fname,np,&node2,fnc) != Success ||
					 ge_debug1("ge_deref",&node2,NULL,ge_deref) != Success)
#else
					if(fnc(&node2) != Success ||		// No, evaluate second argument...
...
int ge_assign(ENode *np) {
	...
	if(si.opflags & OpEval) {
		...
		if(np->en_flags & EN_HaveIdent) {
			...
			// Error if name matches an existing command, pseudo-command, function, alias, or macro.
			if(execfind(np->en_rp->d_str,OpQuery,PtrAny,NULL))
BadLValue:
	...
	switch(sym = last->p_sym) {
		...
		case s_asrsh:
			...
			break;
		case s_asband:
			if((si.opflags & OpEval) && !intvar(&vd)) {
				f = FF_Concat;
				break;
				}
			// else fall through.
...
PAssign:
				...
				{Array *ary = awptr(node2.en_rp)->aw_ary;
				if(np->en_narg >= ary->a_used) {
					Datum d;
					dinit(&d);
					(void) putvar(&d,&vd);
					}
				else
					(void) putvar(ary->a_elp[np->en_narg],&vd);
				}
				datxfer(np->en_rp,node2.en_rp);
				goto Retn;
			case s_asadd:
...
AndOr:
				...
				if(!(si.opflags & OpEval)) {			// Eating arguments?
					if(eval && curTruth != priorTruth) {	// Yes, stop the gluttony?
						si.opflags |= OpEval;		// Yes, enough already.
						goto Eval;
						}
#if MMDebug & Debug_Expr
					...
					si.opflags &= ~OpEval;			// and eat next argument.
#if MMDebug & Debug_Expr
...
char *dtype(Datum *datum,bool terse) {
	char *str;
...
int valarg(Datum *datum,uint aflags) {

	if((aflags & (ArgInt1 | ArgMay)) == ArgInt1)				// 'int' flag set without "may be"?
		(void) intval(datum);						// Yes, must be integer.
	else if((aflags & (ArgArray1 | ArgNIS1 | ArgMay)) == ArgArray1)		// No, 'array' set without other types?
		(void) aryval(datum);						// Yes, must be array.
	else if(!(aflags & (ArgNil1 | ArgBool1 | ArgInt1 | ArgArray1 | ArgNIS1))) {	// No, non-string flags not set?
		if(strval(datum) && (aflags & ArgNotNull1) && disnull(datum))	// Yes, must be string.  Check if null string
			goto NotNull;						// is an error.
		}
	else if((!(aflags & ArgArray1) && datum->d_type == dat_blobRef) ||	// No required types.  Check if found type
	 (!(aflags & ArgBool1) && (datum->d_type & DBoolMask)) ||		// is allowed.
	 (!(aflags & ArgNIS1) && (
	 (!(aflags & ArgNil1) && datum->d_type == dat_nil) ||
	 (!(aflags & ArgInt1) && datum->d_type == dat_int))))
		return rcset(Failure,0,text329,dtype(datum,false));
				// "Unexpected %s argument"
	else if((datum->d_type & DStrMask) && (aflags & ArgNotNull1) &&		// Null string error?
	 disnull(datum))
NotNull:
...
int funcarg(Datum *rval,uint aflags) {
	...
	if((si.opflags & OpEval) && valarg(rval,aflags) == Success && (aflags & ArgPath) && (rval->d_type & DStrMask))
		expath(rval,NULL);
	return rc.status;
	}
...
// *input is assumed to point to either a string or array Datum object, and is set to NULL after the first call to this routine.
// Routine is called repeatedly to obtain next argument until NotFound is returned or a non-Success status.  *rvalp is set to
// either "work" or the next array element for each extracted argument; however, if argument does not conform to validation
// flags, an error is returned.  All arguments are assumed to be optional, including the first.
int nextarg(Datum **rvalp,Datum **input,Datum *work,char **kwlist,Datum ***elpp,ArraySize *elct) {

	for(;;) {
		// First call?
		if(*input != NULL) {
			if((*input)->d_type == dat_blobRef) {

				// Set up for array processing.
				Array *ary = awptr(*input)->aw_ary;
				*elpp = ary->a_elp;
				*elct = ary->a_used;
				*kwlist = NULL;
				}
		...
		if(*elpp != NULL) {
			...
			*rvalp = *(*elpp)++;
			...
			char *str = strparse(kwlist,',');
			if(str == NULL)
				return NotFound;
			if(*str == '\0')
				continue;
			if(dsetstr(str,work) != 0)
				return librcset(Failure);
			*rvalp = work;
			break;
			}
	...
	return valarg(*rvalp,ArgNotNull1);
	}
