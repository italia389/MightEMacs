...
typedef char EBitMap[256 >> 3];

// String "point" for RE scanning.
typedef struct {
...
typedef struct {
	ushort type;
	union {
		Point bpoint;		// Buffer point.
		StrPoint strpoint;	// String point.
		} u;
	} ScanPoint;
...
// Structure for saving search results for group matches.  "elen" and "ml" are used only during scan; "match" is the scan
...
typedef struct {
	...
	Datum *match;			// Pointer to heap copy of matched string.
	} GrpInfo;
...
typedef struct {
	...
	Datum *match;			// Pointer to heap copy of matched string (taken from groups[0].match).
...
typedef struct {
	...
	int fdelta1[256];		// Forward non-RE delta1 table.
	int bdelta1[256];		// Backward non-RE delta1 table.
...
typedef struct {
	int sc_n;			// Original n argument.
	Buffer *sc_list;		// Buffer to put list into.
	DStrFab sc_rpt;
	void *sc_item;			// Pointer to current item in list.
	Datum sc_name;			// Name of CFAM or variable plus arguments.
	Datum sc_value;			// Item value (or nil).
	char *sc_desc;			// Item description (or NULL);
	Datum sc_mstr;			// Match string for apropos.
	Match sc_match;			// Match record for apropos.
	} ShowCtrl;

// Request types and processing flags used by showbuild().
#define SHReqNext	1		// Find next item and return its name.
#define SHReqUsage	2		// Save usage and description in control record.
#define SHReqValue	3		// Write item value to string-fab object.
...
extern int boundary(Point *point,int dir);
extern int chkopts(char *pat,ushort *flags);
extern bool exactbmode(void);
extern int findkeys(ShowCtrl *scp,uint ktype,void *tp);
extern void freerpat(Match *match);
extern void freespat(Match *match);
extern int getpat(Datum **argv,char *prmt,bool srchpat);
extern void grpclear(Match *match);
extern int huntBack(Datum *rval,int n,Datum **argv);
extern int huntForw(Datum *rval,int n,Datum **argv);
extern void initInfoColors(void);
extern void mcclear(Match *match);
extern int mccompile(Match *match);
extern int mcscan(int n,int direct);
extern void minit(Match *match);
extern void mkdeltas(void);
extern char *mkpat(char *dest,Match *match);
extern int newrpat(char *pat,Match *match);
extern int newspat(char *pat,Match *match,ushort *flags);
extern int nextCommand(ShowCtrl *scp,ushort req,char **name);
extern int nextGlobalVar(ShowCtrl *scp,ushort req,char **name);
extern int nextLocalVar(ShowCtrl *scp,ushort req,char **name);
extern int nextMacro(ShowCtrl *scp,ushort req,char **name);
extern int nextSysVar(ShowCtrl *scp,ushort req,char **name);
extern bool plainsearch(void);
extern bool rebmode(void);
extern int recmp(Datum *sp,int scanoff,Match *match,int *result);
extern int replstr(Datum *rval,int n,bool qrepl,Datum **argv);
extern int rmccompile(Match *match);
extern int savematch(Match *match);
extern int scan(int n,int direct);
extern int searchBack(Datum *rval,int n,Datum **argv);
extern int searchForw(Datum *rval,int n,Datum **argv);
extern int showbuild(ShowCtrl *scp,ushort flags,char *title,int (*fp)(ShowCtrl *scp,ushort req,char **name));
extern int showclose(Datum *rval,int n,ShowCtrl *scp);
extern int showopen(ShowCtrl *scp,int n,char *plabel,Datum **argv);
#if MMDebug & Debug_ShowRE
extern int showRegexp(Datum *rval,int n,Datum **argv);
#endif
extern int showReplaceRing(Datum *rval,int n,Datum **argv);
extern int showSearchRing(Datum *rval,int n,Datum **argv);
extern int sindex(Datum *rval,ushort flags,Datum *src,Datum *patp,Match *match);
