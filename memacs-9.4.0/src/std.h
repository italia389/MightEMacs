...
#define Debug_Wrap	0x00040000	// Write line-wrapping info to log file.
#define Debug_NCurses	0x00080000	// Debug ncurses routines.
...
#define NKillRing	40		// Default number of entries in kill ring.
#define NDelRing	30		// Default number of entries in delete ring.
#define NPatRing	20		// Default number of entries in search and replacement rings.
...
#define SBBackground	'.'		// Buffer in background (not being displayed).
...
#define SBReadOnly	'#'		// BFReadOnly flag indicator (read-only buffer).
...
typedef struct {
	...
	ushort *km_slot;		// Pointer to next slot in buffer.
	ushort *km_end;		// Pointer to end of the last macro recorded.
...
typedef struct {
	char *keywd;			// Keyword.
	char *abbr;			// Abbreviation or NULL, to use keywd.
...
#define OptFalse	0x0002		// Set option's pointer value to false if selected.
#define OptSelected	0x0004		// Option was selected.

// Option header object.
typedef struct {
	uint aflags;			// Argument flags for funcarg() call.
	char *otyp;			// Type of option, for error reporting.
	bool single;			// Single option only (items in list are mutually exclusive).
	Option *options;		// Option list.
	} OptHdr;
...
typedef struct RingEntry {
	struct RingEntry *re_prev;	// Pointer to previous item in ring.
	struct RingEntry *re_next;	// Pointer to next item in ring.
	Datum re_data;			// Data item (text chunk).
	} RingEntry;

typedef struct {
	RingEntry *r_entry;		// Current ring item.
	ushort r_size;			// Number of items in ring.
	ushort r_maxsize;		// Maximum size of ring.
	char *r_rname;			// Name of ring.
	char *r_ename;			// Name of ring entry/object (for notifications).
	} Ring;
...
typedef struct ModeGrp {
	struct ModeGrp *mg_next;	// Pointer to next record in linked list, or NULL if none.
...
#define msptr(datum)	((ModeSpec *) (datum)->u.d_blob.b_mem)
...
typedef struct {
	...
	ModeGrp *ghead;			// Mode group table: linked list of ModeGrp records.
					// Built-in mode names.
	char *MdLitASave,*MdLitATerm,*MdLitBak,*MdLitClob,*MdLitCol,*MdLitExact,*MdLitFence1,*MdLitFence2,*MdLitHScrl,
	 *MdLitLine,*MdLitOver,*MdLitRdOnly,*MdLitRegexp,*MdLitRepl,*MdLitRtnMsg,*MdLitSafe,*MdLitWkDir,*MdLitWrap;
	} ModeInfo;

// Position of point in a buffer.
typedef struct {
	struct Line *lnp;		// Pointer to Line structure.
	int off;			// Offset of point in line.
	} Point;
...
// screen order.  The list is associated with one screen and the list head is stored in "whead" whenever the screen is current.
// Each window contains its own values of point and mark.  The flag field contains some bits that are set by commands to guide
...
typedef struct EWindow {
	struct EWindow *w_next;		// Next window.
	struct Buffer *w_buf;		// Buffer displayed in window.
	WindFace w_face;		// Point position, etc.
	...
	short w_rfrow;			// Target (reframe) row in window for line containing point.
...
typedef struct EScreen {
	struct EScreen *s_next;	// Pointer to next screen in list.
	EWindow *s_whead;		// Head of linked list of windows.
	EWindow *s_curwin;		// Current window in this screen.
	struct Buffer *s_lastbuf;	// Last buffer exited from.
...
typedef struct Mark {
	struct Mark *mk_next;		// Next mark.
	ushort mk_id;			// Mark identifier.
	short mk_rfrow;			// Target (reframe) row in window for point.
	Point mk_point;			// Point position.
	} Mark;

#define MkOpt_AutoR	0x0001		// Use mark RegMark if default n; otherwise, prompt with no default (setMark).
#define MkOpt_AutoW	0x0002		// Use mark WrkMark if default n; otherwise, prompt with no default (markBuf).
#define MkOpt_Hard	0x0004		// Always prompt, no default (delMark, gotoMark, swapMark).
#define MkOpt_Viz	0x0008		// Mark must exist and be visible (gotoMark, swapMark).
#define MkOpt_Exist	0x0010		// Mark must exit (delMark).
...
typedef struct {
	struct LoopBlock *mi_exec;	// Pointer to compiled macro loop blocks.
...
typedef struct BufMode {
	struct BufMode *bm_next;	// Next mode in linked list (or NULL if none).
	ModeSpec *bm_mode;		// Pointer to (enabled) mode in modetab.
	} BufMode;
...
typedef struct Buffer {
	WindFace b_face;		// Point position, etc. from last detached window.
	...
	Mark b_mroot;			// Point mark RegMark and list root.
	MacInfo *b_mip;			// Pointer to macro parameters, if applicable.
	EScreen *b_lastscr;		// Last screen buffer was displayed on.
...
#define bufptr(datum)	((Buffer *) (datum)->u.d_blob.b_mem)
...
#define BS_QExit	0x0004		// Called from quickExit command.
...
typedef struct {
	Point r_point;			// Origin point location.
	long r_size;			// Length in characters.
	int r_linect;			// Line count.
	} Region;

// Operation flags for "get region" functions.
#define RegForceBegin	0x0001		// Force point in Region object to beginning of region; otherwise, leave point at
					// original starting point and negate r_size if region extends backward.
#define RegInclDelim	0x0002		// Include line delimiter (if any) on last line selected when n > 0.
#define RegEmptyOK	0x0004		// Allow empty region when getting line block.
#define RegLineSelect	0x0008		// Include line at bottom of region in line count if possible.
...
typedef struct Line {
	...
	struct Line *l_prev;		// Pointer to the previous line.
...
typedef struct {
	...
	int (*cf_func)(Datum *rval,int n,Datum **argv);// C function that command or system function is bound to.
...
typedef struct {
	...
	union {
		...
		struct Alias *p_alias;	// Alias pointer.
		struct Buffer *p_buf;	// Buffer pointer.
		void *p_void;		// For generic pointer comparisons.
		} u;
	} UnivPtr;

// Macro for fetching UnivPtr pointer from Datum object in hash record, given HashRec *.
#define univptr(hrec)	((UnivPtr *) (hrec)->value->u.d_blob.b_mem)
...
#define PtrPseudo	0x0002		// Command-function pointer -- pseudo command (CFHidden flag set).
...
typedef struct {
	...
	struct Buffer *h_buf;		// Macro to execute.
	} HookRec;
...
enum cfid {
	cf_abort,cf_about,cf_abs,cf_alias,cf_appendFile,cf_apropos,cf_array,cf_backChar,cf_backLine,cf_backPage,cf_backPageNext,
	cf_backPagePrev,cf_backTab,cf_backWord,cf_backspace,cf_basename,cf_beep,cf_beginBuf,cf_beginKeyMacro,cf_beginLine,
	cf_beginText,cf_beginWhite,cf_bemptyQ,cf_bgets,cf_bindKey,cf_binding,cf_bprint,cf_bprintf,cf_bufAttrQ,cf_bufBoundQ,
	cf_bufInfo,cf_bufWind,cf_chgBufAttr,cf_chgDir,cf_chgMode,cf_chr,cf_clearBuf,cf_clearMsgLine,cf_clone,
	cf_copyFencedRegion,cf_copyLine,cf_copyRegion,cf_copyToBreak,cf_copyWord,
#if WordCount
	cf_countWords,
#endif
	cf_cycleDelRing,cf_cycleKillRing,cf_cycleReplaceRing,cf_cycleSearchRing,cf_definedQ,cf_delAlias,cf_delBackChar,
	cf_delBackTab,cf_delBlankLines,cf_delBuf,cf_delDelEntry,cf_delFencedRegion,cf_delForwChar,cf_delForwTab,cf_delKillEntry,
	cf_delLine,cf_delMacro,cf_delMark,cf_delRegion,cf_delReplacePat,cf_delScreen,cf_delSearchPat,cf_delToBreak,cf_delWhite,
	cf_delWind,cf_delWord,cf_detabLine,cf_dirname,cf_dupLine,cf_editMode,cf_editModeGroup,cf_emptyQ,cf_endBuf,
	cf_endKeyMacro,cf_endLine,cf_endWhite,cf_endWord,cf_entabLine,cf_env,cf_eval,cf_exit,cf_expandPath,cf_findFile,
	cf_forwChar,cf_forwLine,cf_forwPage,cf_forwPageNext,cf_forwPagePrev,cf_forwTab,cf_forwWord,cf_getInfo,cf_getKey,
	cf_getWord,cf_glob,cf_gotoFence,cf_gotoLine,cf_gotoMark,cf_groupModeQ,cf_growWind,cf_help,cf_huntBack,cf_huntForw,
	cf_includeQ,cf_indentRegion,cf_index,cf_insert,cf_insertBuf,cf_insertFile,cf_insertPipe,cf_insertSpace,cf_inserti,
	cf_interactiveQ,cf_join,cf_joinLines,cf_joinWind,cf_keyPendingQ,cf_kill,cf_killFencedRegion,cf_killLine,cf_killRegion,
	cf_killToBreak,cf_killWord,cf_lastBuf,cf_length,cf_let,cf_lowerCaseLine,cf_lowerCaseRegion,cf_lowerCaseStr,
	cf_lowerCaseWord,cf_markBuf,cf_match,cf_message,cf_metaPrefix,cf_modeQ,cf_moveWindDown,cf_moveWindUp,cf_narrowBuf,
	cf_negativeArg,cf_newline,cf_newlineI,cf_nextBuf,cf_nextScreen,cf_nextWind,cf_nilQ,cf_nullQ,cf_numericQ,cf_onlyWind,
	cf_openLine,cf_openLineI,cf_ord,cf_outdentRegion,cf_overwrite,cf_pathname,cf_pause,cf_pipeBuf,cf_pop,cf_popBuf,
	...
	cf_replace,cf_replaceText,cf_resetTerm,cf_resizeWind,cf_restoreBuf,cf_restoreScreen,cf_restoreWind,cf_revertYank,cf_run,
	cf_saveBuf,cf_saveFile,cf_saveScreen,cf_saveWind,cf_scratchBuf,cf_searchBack,cf_searchForw,cf_selectBuf,cf_selectLine,
	cf_selectScreen,cf_selectWind,cf_setBufFile,cf_setColorPair,cf_setDispColor,cf_setHook,cf_setMark,cf_setWrapCol,cf_seti,
	cf_shQuote,cf_shell,cf_shellCmd,cf_shift,cf_showAliases,cf_showBuffers,cf_showColors,cf_showCommands,cf_showDelRing,
	cf_showDir,cf_showFence,cf_showFunctions,cf_showHooks,cf_showKey,cf_showKillRing,cf_showMacros,cf_showMarks,
	cf_showModes,cf_showPoint,
	...
	cf_tr,cf_traverseLine,cf_trimLine,cf_truncBuf,cf_typeQ,cf_unbindKey,cf_undelete,cf_undeleteCycle,cf_universalArg,
	cf_unshift,cf_updateScreen,cf_upperCaseLine,cf_upperCaseRegion,cf_upperCaseStr,cf_upperCaseWord,cf_viewFile,cf_widenBuf,
...
typedef struct {
	...
	Ring *ring;			// Data ring to use during terminal input (or NULL for none).
	} TermInp;
...
#define ArgPath		0x02000000	// Argument is a file pathname to be expanded if string type.  (Applies to first
					// argument only if set in cftab, per code in execCF() function.)
...
typedef struct {
	...
	EScreen *curscr;			// Current screen.
	EWindow *curwin;			// Current window.
	...
	Buffer *savbuf;		// Saved buffer pointer.
	EScreen *savscr;		// Saved screen pointer.
	EWindow *savwin;		// Saved window pointer.
	EScreen *shead;		// Head of screen list.
	int stabsize;			// Current soft tab size (0: use hard tabs).
	int tjump;			// Line-traversal jump size.
	EWindow *whead;		// Head of window list.
...
extern int abortOp(Datum *rval,int n,Datum **argv);
extern int abortinp(void);
extern int aboutMM(Datum *rval,int n,Datum **argv);
extern int aliasCFM(Datum *rval,int n,Datum **argv);
extern int allowedit(bool edit);
extern int apropos(Datum *rval,int n,Datum **argv);
extern int attrCount(char *str,int len,int maxoff);
extern int aupdate(char *aname,ushort op,UnivPtr *univp);
extern int backChar(Datum *rval,int n,Datum **argv);
extern int backLine(Datum *rval,int n,Datum **argv);
extern int backPage(Datum *rval,int n,Datum **argv);
extern int backWord(Datum *rval,int n,Datum **argv);
extern int bactivate(Buffer *buf);
extern int bappend(Buffer *buf,char *text);
extern void bchange(Buffer *buf,ushort flags);
extern int bclear(Buffer *buf,ushort flags);
extern int bcomplete(Datum *rval,char *prmt,char *defname,uint flags,Buffer **bufp,bool *created);
extern int bconfirm(Buffer *buf,ushort flags);
extern Buffer *bdefault(void);
extern int bdelete(Buffer *buf,ushort flags);
extern int beginKeyMacro(Datum *rval,int n,Datum **argv);
extern int beginText(Datum *rval,int n,Datum **argv);
extern int begintxt(void);
extern int beline(Datum *rval,int n,bool end);
extern bool bempty(Buffer *buf);
extern int bextend(Buffer *buf);
extern int bfind(char *name,ushort cflags,ushort bflags,Buffer **bufp,bool *created);
extern int bftab(int n);
extern void bftowf(Buffer *buf,EWindow *win);
extern int bgets(Datum *rval,int n,Datum **argv);
extern int bhook(Datum **rpp,ushort flags);
extern int binary(char *key,char *(*tval)(int i),int tsize,int *index);
extern bool bmclear(Buffer *buf);
extern void bmclearall(ModeSpec *mspec);
extern int bmset(Buffer *buf,ModeSpec *mspec,bool clear,bool *wasSetp);
extern bool bmsrch(Buffer *buf,bool clear,int modect,...);
extern bool bmsrch1(Buffer *buf,ModeSpec *mspec);
extern int bpop(Buffer *buf,ushort flags);
extern int bprint(Datum *rval,int n,Datum **argv);
extern int brename(Datum *rval,ushort flags,Buffer *targbuf);
extern int bscratch(Buffer **bufp);
extern Buffer *bsrch(char *bname,ssize_t *index);
extern int bswitch(Buffer *buf,ushort flags);
extern int bufAttrQ(Datum *rval,int n,Datum **argv);
extern bool bufbegin(Point *point);
extern bool bufend(Point *point);
extern int bufInfo(Datum *rval,int n,Datum **argv);
extern long buflength(Buffer *buf,long *lp);
extern int bufop(Datum *rval,int n,char *prmt,uint op,int flag);
extern void calcHJC(void);
extern int chcase(short ch);
extern int chgBufAttr(Datum *rval,int n,Datum **argv);
extern int chgdir(char *dir);
extern int chgMode(Datum *rval,int n,Datum **argv);
extern int chgmode1(char *name,int action,Buffer *buf,long *formerStatep,ModeSpec **result);
extern int chgtext(Datum *rval,int n,ushort style,Buffer *buf);
extern int chwkdir(Datum *rval,int n,Datum **argv);
extern int clearBuf(Datum *rval,int n,Datum **argv);
extern void clearKeyMacro(bool stop);
extern void clfname(Buffer *buf);
extern int copyreg(Region *reg);
#if WordCount
extern int countWords(Datum *rval,int n,Datum **argv);
#endif
extern void cpause(int n);
extern int cvtcase(int n,ushort flags);
extern int delAlias(Datum *rval,int n,Datum **argv);
extern int delBlankLines(Datum *rval,int n,Datum **argv);
extern int delBuf(Datum *rval,int n,Datum **argv);
extern int delMacro(Datum *rval,int n,Datum **argv);
extern int delMark(Datum *rval,int n,Datum **argv);
extern int delMode(Datum *rval,int n,Datum **argv);
extern int delModeGroup(Datum *rval,int n,Datum **argv);
extern int delScreen(Datum *rval,int n,Datum **argv);
extern int delWind(Datum *rval,int n,Datum **argv);
extern int deltab(int n,bool force);
extern int delwhite(int n,bool prior);
extern int detabLine(Datum *rval,int n,Datum **argv);
extern int dkregion(int n,bool kill);
extern int doincl(Datum *rval,int n,Datum **argv);
extern int dopop(Datum *rval,int n,bool popBuf);
#if MMDebug & (Debug_ScrDump | Debug_Narrow | Debug_Temp)
extern void dumpbuffer(char *label,Buffer *buf,bool withData);
#endif
...
extern int dupLine(Datum *rval,int n,Datum **argv);
extern int editMode(Datum *rval,int n,Datum **argv);
extern int editModeGroup(Datum *rval,int n,Datum **argv);
extern short ektoc(ushort ek,bool extend);
extern int endKeyMacro(Datum *rval,int n,Datum **argv);
extern int endWord(Datum *rval,int n,Datum **argv);
extern int entabLine(Datum *rval,int n,Datum **argv);
extern int eopendir(char *fspec,char **fp);
extern int ereaddir(void);
extern int escattr(DStrFab *strloc);
extern int escattrtosf(DStrFab *strloc,char *str);
extern int execfind(char *name,ushort op,uint selector,UnivPtr *univp);
extern int execnew(char *name,UnivPtr *univp);
extern int expath(Datum *dest,Datum *src);
extern void faceinit(WindFace *wfp,struct Line *lnp,Buffer *buf);
extern int fenceMatch(short fence,bool dobeep);
extern EWindow *findwind(Buffer *buf);
extern void fixbface(Buffer *buf,int offset,int n,Line *lnp1,Line *lnp2);
extern void fixwface(int offset,int n,Line *lnp1,Line *lnp2);
extern int fnhook(Buffer *buf);
extern int forwChar(Datum *rval,int n,Datum **argv);
extern int forwLine(Datum *rval,int n,Datum **argv);
extern int forwPage(Datum *rval,int n,Datum **argv);
extern int forwWord(Datum *rval,int n,Datum **argv);
extern void garbpop(Datum *datum);
extern Buffer *getbufarg(Datum *bname);
extern int getcol(Point *point,bool termattr);
...
extern uint getFlagOpts(Option *opt);
extern int getInfo(Datum *rval,int n,Datum **argv);
extern long getlinenum(Buffer *buf,Line *targlnp);
extern int getlregion(int n,Region *reg,ushort flags);
extern int getmodes(Datum *rval,Buffer *buf);
extern int getnarg(Datum *rval,char *prmt);
extern int getregion(Region *reg,ushort flags);
extern int gettermsize(ushort *col,ushort *row);
extern void gettregion(Point *point,int n,Region *reg,ushort flags);
extern int getwid(ushort *wid);
extern int getwnum(EWindow *win);
extern int getWord(Datum *rval,int n,Datum **argv);
extern int getwpos(EWindow *win);
extern void gmclear(ModeSpec *mspec);
extern void gmset(ModeSpec *mspec);
extern int goline(Datum *datum,int n,int line);
extern int gotoFence(Datum *rval,int n,Datum **argv);
extern int gotoLine(Datum *rval,int n,Datum **argv);
extern int gotoMark(Datum *rval,int n,Datum **argv);
extern int gotoScreen(int n,ushort flags);
extern int gotoWind(Datum *rval,int n,ushort flags);
extern int groupMode(Datum *rval,int n,Datum **argv);
extern int growKeyMacro(void);
extern int gswind(Datum *rval,int n,int how);
extern bool haveColor(void);
extern int help(Datum *rval,int n,Datum **argv);
extern int indentRegion(Datum *rval,int n,Datum **argv);
extern int indexF(Datum *rval,int n,Datum **argv);
extern char *inflate(char *dest,char *src,bool plural);
extern void initBoolOpts(Option *opt);
extern void initchars(void);
extern void initInfoColors(void);
extern int insertBuf(Datum *rval,int n,Datum **argv);
extern int inserti(Datum *rval,int n,Datum **argv);
extern int insnlspace(int n,ushort flags);
extern int instab(int n);
extern bool inwind(EWindow *win,Line *lnp);
extern bool inword(void);
extern int iorstr(char *src,int n,ushort style,Ring *ring);
extern int iortext(char *src,int n,ushort style,Buffer *buf);
extern bool is_lower(short ch);
extern bool is_upper(short ch);
extern bool is_white(Line *lnp,int length);
extern int isdef(Datum *rval,int n,Datum **argv);
extern bool isletter(short ch);
extern int joinLines(Datum *rval,int n,Datum **argv);
extern int joinWind(Datum *rval,int n,Datum **argv);
...
extern int kdctext(int n,int kdc,Region *reg);
...
extern void llink(Line *lnp1,Buffer *buf,Line *lnp2);
extern int lnewline(void);
extern void lreplace1(Line *lnp1,Buffer *buf,Line *lnp2);
extern void lunlink(Line *lnp,Buffer *buf);
extern int markBuf(Datum *rval,int n,Datum **argv);
extern bool markval(Datum *datum);
extern int mdcreate(char *name,ssize_t index,char *desc,ushort flags,ModeSpec **mspecp);
extern void mdelete(Buffer *buf,ushort flags);
extern ModeSpec *mdsrch(char *name,ssize_t *index);
extern int message(Datum *rval,int n,Datum **argv);
extern int mfind(ushort id,Mark **markp,ushort flags);
extern int mgcreate(char *name,ModeGrp *prior,char *desc,ModeGrp **result);
extern bool mgsrch(char *name,ModeGrp **slot,ModeGrp **grp);
extern int mkarray(Datum *rval,Array **aryp);
...
extern int modeQ(Datum *rval,int n,Datum **argv);
extern bool modeset(ushort id,Buffer *buf);
extern int movech(int n);
extern int moveln(int n);
extern void movept(Point *point);
extern int movewd(int n);
extern int moveWindUp(Datum *rval,int n,Datum **argv);
extern void mset(Mark *mark,EWindow *win);
extern int narrowBuf(Datum *rval,int n,Datum **argv);
extern int newcol(short c,int col);
extern int newcolTA(Point *point,int col,int *attrLen);
extern int newlineI(Datum *rval,int n,Datum **argv);
extern int nextWind(Datum *rval,int n,Datum **argv);
extern void nukebufsp(EScreen *scr);
extern int onlyWind(Datum *rval,int n,Datum **argv);
extern int openLine(Datum *rval,int n,Datum **argv);
extern int openLineI(Datum *rval,int n,Datum **argv);
extern int otherfence(short fence,Region *reg);
extern int outdentRegion(Datum *rval,int n,Datum **argv);
extern int overprep(int n);
extern char *pad(char *s,uint len);
extern int parseopts(OptHdr *ohdr,char *prmt,Datum *opts,int *count);
extern int pathsearch(char *name,ushort flags,void *result,char *funcname);
extern int pipecmd(Datum *rval,int n,char *prmt,ushort flags);
extern int pnbuffer(Datum *rval,int n,bool backward);
extern int prevWind(Datum *rval,int n,Datum **argv);
extern bool ptinwind(void);
extern int quit(Datum *rval,int n,Datum **argv);
extern int quoteChar(Datum *rval,int n,Datum **argv);
...
extern int rcycle(Ring *ring,int n,bool msg);
extern int rdelete(Ring *ring,int n);
extern char *regcpy(char *buf,Region *reg);
extern int reglines(int *np);
extern int renameBuf(Datum *rval,int n,Datum **argv);
extern int render(Datum *rval,int n,Buffer *buf,ushort flags);
extern int resetTermCmd(Datum *rval,int n,Datum **argv);
extern int resizeWind(Datum *rval,int n,Datum **argv);
extern RingEntry *rget(Ring *ring,int n);
extern int rpthdr(DStrFab *rpt,char *title,bool plural,char *colhead,ColHdrWidth *wp,int *widthp);
extern int rsetpat(char *str,Ring *ring);
extern int runcmd(Datum *dsink,DStrFab *cmd,char *cname,char *arg,bool fullQuote);
extern int scallerr(char *caller,char *call,bool addTERM);
extern int scratchBuf(Datum *rval,int n,Datum **argv);
extern int scrcount(void);
extern int selectBuf(Datum *rval,int n,Datum **argv);
extern int selectLine(Datum *rval,int n,Datum **argv);
extern int selectScreen(Datum *rval,int n,Datum **argv);
extern int selectWind(Datum *rval,int n,Datum **argv);
extern int sepline(int len,DStrFab *strloc);
extern void setBoolOpts(Option *opt);
extern int setColorPair(Datum *rval,int n,Datum **argv);
extern int setDispColor(Datum *rval,int n,Datum **argv);
extern int setMark(Datum *rval,int n,Datum **argv);
extern int setccol(int pos);
extern int setfname(Buffer *buf,char *fname);
extern int seti(Datum *rval,int n,Datum **argv);
extern int setpath(char *path,bool prepend);
extern void settermsize(ushort ncol,ushort nrow);
extern int setwkdir(EScreen *scr);
extern int sfind(ushort scr_num,Buffer *buf,EScreen **spp);
extern int shellCLI(Datum *rval,int n,Datum **argv);
extern int showBuffers(Datum *rval,int n,Datum **argv);
extern int showColors(Datum *rval,int n,Datum **argv);
extern int showFence(Datum *rval,int n,Datum **argv);
extern int showMarks(Datum *rval,int n,Datum **argv);
extern int showModes(Datum *rval,int n,Datum **argv);
extern int showPoint(Datum *rval,int n,Datum **argv);
extern int showRing(Datum *rval,int n,Ring *ring);
extern int showScreens(Datum *rval,int n,Datum **argv);
extern int spanwhite(bool end);
extern int sswitch(EScreen *scr,ushort flags);
extern char *strrev(char *s);
extern char *strsamp(char *src,size_t srclen,size_t maxlen);
extern void supd_wflags(Buffer *buf,ushort flags);
extern int suspendMM(Datum *rval,int n,Datum **argv);
extern int swapMark(Datum *rval,int n,Datum **argv);
extern int swapmid(ushort id);
extern int sysbuf(char *root,Buffer **bufp,ushort flags);
extern int tabstop(int n);
extern int termattr(char **strp,char *strz,ushort *flags,WINDOW *wp);
extern int terminp(Datum *rval,char *prmt,uint aflags,uint cflags,TermInp *tip);
extern int terminpYN(char *prmt,bool *result);
extern int termResize(Datum *rval,int n,Datum **argv);
extern char *timeset(void);
extern int toString(Datum *rval,int n,Datum **argv);
extern int traverseLine(Datum *rval,int n,Datum **argv);
extern int trimLine(Datum *rval,int n,Datum **argv);
...
extern int typahead(int *count);
extern int undeleteCycle(Datum *rval,int n,Datum **argv);
extern int update(int n);
extern int updscr(Datum *rval,int n,Datum **argv);
extern int uprompt(Datum *rval,int n,Datum **argv);
extern int vtinit(void);
extern void wftobf(EWindow *win,Buffer *buf);
extern EWindow *whasbuf(Buffer *buf,bool skipCur);
extern int widenBuf(Datum *rval,int n,Datum **argv);
extern int wincount(EScreen *scr,int *wnum);
extern bool wnewtop(EWindow *win,Line *lnp,int n);
extern EWindow *wnextis(EWindow *win);
extern int wrapLine(Datum *rval,int n,Datum **argv);
extern int wrapWord(Datum *rval,int n,Datum **argv);
extern int writeBuf(Datum *rval,int n,Datum **argv);
extern int wscroll(Datum *rval,int n,int (*winfunc)(Datum *rval,int n,Datum **argv),
 int (*pagefunc)(Datum *rval,int n,Datum **argv));
extern int wsplit(int n,EWindow **winp);
extern int wswitch(EWindow *win,ushort flags);
extern int wupd_reframe(EWindow *win);
extern int xeqKeyMacro(Datum *rval,int n,Datum **argv);
extern long xorshift64star(long max);
extern int ycycle(Datum *rval,int n,Ring *ring,ushort flag,ushort cmd);
...
// Global variables.
Option battrinfo[] = {			// Buffer attribute table.
	{"Active",NULL,OptIgnore,BFActive},
	{"^Changed","^Chg",0,BFChanged},
	{"Constrained",NULL,OptIgnore,BFConstrain},
	{"^Hidden","^Hid",0,BFHidden},
	{"Macro",NULL,OptIgnore,BFMacro},
...
Ring dring = {				// Delete ring (initially empty).
	NULL,0,NDelRing,NULL,""};
...
ModeInfo mi = {				// Mode information record.
	.ghead = NULL,"ASave","ATerm","Bak","Clob","Col","Exact","Fence1","Fence2","HScrl","Line","Over","RdOnly","Regexp",
	 "Repl","RtnMsg","Safe","WkDir","Wrap"
	};
