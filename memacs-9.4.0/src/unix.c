...
int ttgetc(bool ml,ushort *cp) {
	...
	for(;;) {
		if((kcode = wgetch(wp)) != ERR) {
			...
			fprintf(logfile,"ttgetc(): got kcode %.4ho\n",kcode);
			if(!((c = kcode) & ~0xFF))
				break;
			c = keytable[kcode - KEY_MIN];
			fprintf(logfile,"  -> FKey x%.4hX\n",c);
			if(c != (FKey | '?'))
				break;
#else
			if(!((c = kcode) & ~0xFF) || (c = keytable[kcode - KEY_MIN]) != (FKey | '?'))
				break;
#endif
			}
		else if(errno != 0)
...
int gettermsize(ushort *col,ushort *row) {
	...
	if((*col = w.ws_col) > term.t_mcol)
		*col = term.t_mcol;
	if((*row = w.ws_row) > term.t_mrow)
		*row = term.t_mrow;
	return rc.status;
	}
...
int resetTermCmd(Datum *rval,int n,Datum **argv) {
	static char myname[] = "resetTermc";
	EScreen *scr;		// Current screen being examined.
	...
	if(gettermsize(&ncol,&nrow) != Success)
		return rc.status;
#if MMDebug & Debug_NCurses
	fprintf(logfile,"resetTermCmd(): gettermsize() returned %hu x %hu\n",ncol,nrow);
#endif
	...
	do {
		...
		if(scr->s_nrow != nrow || scr->s_ncol != ncol) {
			scr->s_flags |= EScrResize;
			n = 1;
			}
...
Err:
	...
	else if(kentry.lastkseq == TermResizeKey)
DimSet:
...
int ttopen(void) {
	...
	if(has_colors()) {
		...
		if(term.maxColor >= 7 && term.maxPair >= 63 && (freePairs = term.maxPair - ReservedPairs) >= (8 * 4)) {
			...
			static struct colorinfo ci[] = {
				{DefColorText,ColorText,&term.colorText},
				{DefColorKMRI,ColorKMRI,&term.colorKMRI},
			...
			cip = ci;
			do {
				*cip->termColor = (cip->desiredColor <= term.maxColor) ? cip->desiredColor : cip->defaultColor;
				} while((++cip)->termColor != NULL);

			// Set default colors for informational displays.
			term.itemColor[ColorIdxInfo].colors[0] = term.colorText;
			term.itemColor[ColorIdxInfo].colors[1] = term.colorInfo;
			...
			if(term.maxColor > 7 && ColorML <= term.maxColor) {
				term.itemColor[ColorIdxML].colors[0] = term.colorText;
				term.itemColor[ColorIdxML].colors[1] = ColorML;
			...
			term.itemColor[ColorIdxKMRI].colors[0] = term.colorText;
			term.itemColor[ColorIdxKMRI].colors[1] = term.colorKMRI;
			(void) init_pair(term.maxPair - ColorPairKMRI,term.colorText,term.colorKMRI);
	...
	KeyTab *ktp = keylist;
	do {
		keytable[ktp->kcode - KEY_MIN] = ktp->ek;
		} while((++ktp)->kcode != -1);
	char *fn;
	nrow = KEY_F0 - KEY_MIN;
	ncol = 0;
...
static void vattr(bool ml,ushort opflag,int aflag,bool attrOn) {
	...
	if(si.opflags & opflag) {
		WINDOW *win = ml ? term.mlwin : stdscr;
		(void) (attrOn ? wattron(win,aflag) : wattroff(win,aflag));
		}
	}
...
int chwkdir(Datum *rval,int n,Datum **argv) {
	HookRec *hrec = hooktab + HkChDir;
	TermInp ti = {NULL,RtnKey,MaxPathname,NULL};

	// Get directory name.
	if(si.opflags & OpScript)
		expath(rval,argv[0]);
	else if(terminp(rval,text277,ArgNil1,Term_C_Filename,&ti) != Success || rval->d_type == dat_nil)
			// "Change directory"
		return rc.status;

	// Make the change.
	if(chgdir(rval->d_str) != Success)
		return rc.status;

	// Get new path (absolute pathname), save it in current screen, and return it to caller.
	if(setwkdir(si.curscr) != Success)
		return rc.status;
	...
	if(hrec->h_buf != NULL && hrec->h_buf->b_mip->mi_nexec == 0 && exechook(NULL,n,hrec,0) != Success)
		return rc.status;
	...
	return (!(si.opflags & OpScript) && !(mi.cache[MdIdxWkDir]->ms_flags & MdEnabled)) ?
	 rcset(Success,RCNoFormat | RCNoWrap,si.curscr->s_wkdir) : rc.status;
	}

// Suspend MightEMacs.
int suspendMM(Datum *rval,int n,Datum **argv) {
...
int rename(const char *file1,const char *file2) {
	...
	if(stat(file2,&buf2) == 0) {
		...
		if(buf.st_dev == buf2.st_dev && buf.st_ino == buf2.st_ino)
...
int shellCLI(Datum *rval,int n,Datum **argv) {
	int rcode;
	char *sh = shpath();
	...
	dsetbool(rcode == 0,rval);			// set return value to false if error occurred; otherwise, true...
	if(rcode != 0)
...
static int getcmd(Datum **cmdlinep,char *prmt) {

	if(dnewtrk(cmdlinep) != 0)
		return librcset(Failure);
	if(si.opflags & OpScript) {

		// Concatenate all arguments into *cmdlinep.
		if(catargs(*cmdlinep,1,NULL,0) == Success && disnull(*cmdlinep))
			(void) rcset(Failure,0,text187,text53);
				// "%s cannot be null","Value"
		}
	else
		(void) terminp(*cmdlinep,prmt,ArgNotNull1 | ArgNil1,0,NULL);

	return rc.status;
	}

// Read data from open file descriptor, insert into given buffer, and close file.  Return status.
static int ipipe(int fd,int n,Buffer *buf,DataInsert *dip,ushort flags) {

	if(rinit(fd) == Success) {
		dip->targbuf = buf;
		dip->targpoint = (buf == si.curbuf) ? NULL : &buf->b_face.wf_point;
		dip->msg = (flags & PipeInsert) ? text153 : text139;
					// "Inserting data...","Reading data..."

		// Enable non-blocking reads and timeouts.
		if(fcntl(fd,F_SETFL,O_NONBLOCK) == -1)
			return scallerr("ipipe","fcntl",false);
		fi.flags |= FIUseTimeout;
		fi.fname = FNpipefile;
...
// Save results of command execution in a temporary buffer.  If cmdln is NULL, skip header line.  Return status.
static int resultBuf(Buffer **bufp,Datum *cmdln,int *pipe3,int *pipe2) {
	ushort mflag;
	DataInsert di;
	...
	if(sysbuf(text424,bufp,0) != Success)
			// "ShellCmd"
		return rc.status;

	// Write command.
	if(cmdln != NULL) {
		char wkbuf[strlen(cmdln->d_str) + 4];

		sprintf(wkbuf,"%c %s",getuid() == 0 ? '#' : '$',cmdln->d_str);
		if(insline(wkbuf,strlen(wkbuf),true,*bufp,&(*bufp)->b_face.wf_point) != Success)
			goto ERtn;
		}

	// Read pipe3 (standard error) into buffer.
	if(pipe3 != NULL && ipipe(pipe3[0],1,*bufp,&di,PipeInsert) != Success)
		goto ERtn;

	// Read pipe2 (standard output) into buffer.
	if(pipe2 != NULL && ipipe(pipe2[0],1,*bufp,&di,PipeInsert) != Success)
		goto ERtn;
...
int pipecmd(Datum *rval,int n,char *prmt,ushort flags) {
	char *fname;
	Buffer *rbuf;
	Datum *cmdln;
	...
	static Option sctab[] = {
		...
		{NULL,NULL,0,0}};
	static OptHdr ohdr = {
		ArgFirst | ArgNil1,text410,false,sctab};
				// "command option"
	...
	if(flags & PipePopOnly) {
		if(parseopts(&ohdr,text448,NULL,NULL) != Success ||
				// "Options"
		 ((si.opflags & OpScript) && !needsym(s_comma,true)))
						// "Options"
			return rc.status;
		}

	// Get shell command.  Cancel operation if nothing entered.
	if(getcmd(&cmdln,prmt) != Success || cmdln->d_type == dat_nil)
		return rc.status;
	...
	if(!(flags & (PipePopOnly | PipeInsert)) && (n == INT_MIN || n == 1) && bconfirm(si.curbuf,0) != Success)
		return rc.status;
...
DupErr:
	...
	if(flags & PipeWrite) {
		switch(fork()) {
			case 0:
				...
				(void) close(pipe2[0]);
				(void) close(pipe3[0]);
				winit(si.curbuf,pipe1[1]);
				fi.fname = FNpipefile;
				exit(writefd(si.curbuf,NULL) == Success ? 0 : -1);
			case -1:
				fname = FNfork;
				goto OSErr;
			default:
	...
	for(;;) {
		...
		cpause(PipePause);		// Pause and try again.
		action = (action + 1) % 3;
		}
	...
	switch(action) {
		...
		case 2:
			...
			if(flags & PipePopOnly) {
				...
				if(sctab[1].cflags & OptSelected) {
					n = true;
					goto SetResult;
					}
				if(resultBuf(&rbuf,sctab[2].cflags & OptSelected ? NULL : cmdln,pipe3,pipe2) != Success)
					goto CloseWait;
				if(sctab[0].cflags & OptSelected)
					rflags |= RendShift;
				n = true;
				goto PopBuf;
				}
			...
			if((flags & PipeInsert) || (!(flags & PipeWrite) && (n == INT_MIN || n == 1))) {
				buf = si.curbuf;
				if(!(flags & PipeInsert) && readprep(buf,BC_IgnChgd) != Success)
					goto CloseWait;
				}
			else if(bscratch(&buf) != Success)
				goto CloseWait;
			if(ipipe(pipe2[0],n,buf,&di,flags) == Success) {
				if(dopentrk(&sfab) != 0)
					goto LibFail;
				if(iostat(&sfab,di.finalDelim ? 0 : IOS_NoDelim,NULL,Success,NULL,flags & PipeInsert ?
				 text154 : text140,di.lineCt) == Success) {
					...
					if(!(flags & PipeInsert)) {
						...
						if((flags & PipeWrite) && (n == INT_MIN || n == 1)) {
							Line *lnp = buf->b_lnp;
							buf->b_lnp = si.curbuf->b_lnp;
							si.curbuf->b_lnp = lnp;
							(void) bdelete(buf,BC_IgnChgd);	// Can't fail.
							buf = si.curbuf;
							buf->b_flags &= ~BFChanged;
							supd_wflags(buf,WFHard | WFMode);
							}
						faceinit(buf == si.curbuf ? &si.curwin->w_face : &buf->b_face,buf->b_lnp,buf);
						(void) render(rval,n == INT_MIN ? 1 : n,buf,n == INT_MIN ? 0 :
		...
		default:
			...
			if(resultBuf(&rbuf,cmdln,pipe3,pipe2) != Success)
				goto CloseWait;
			if(!(flags & PipePopOnly)) {
				DStrFab alert;

				if(dopenwith(&alert,rval,SFClear) != 0 || dputf(&alert,text194,cmdln->d_str) != 0 ||
										// "Shell command \"%s\" failed"
				 dclose(&alert,sf_string) != 0)
					goto LibFail;
				if(mlputs(MLHome | MLFlush,rval->d_str) != Success)
					goto CloseWait;
				cpause(160);
				}
			n = false;
PopBuf:
			if(render(rval,-1,rbuf,rflags) == Success)
SetResult:
				dsetbool(n,rval);
		}
CloseWait:
...
// Copy pathname in src to dest, expanding any "~/" or "~user/" at beginning to user's HOME directory and any embedded
// environmental variables in form "$var" or "${var}".  If src is NULL, string in dest is expanded in place.  Return status.
int expath(Datum *dest,Datum *src) {
	char *str1,*str,*name;
	DStrFab sfab;
	bool braceForm;
	short c1,c2;
		...
		switch(c1) {
			case '~':
				...
				if(str != str1)
					break;
				if(*str == '/') {
					if((name = getenv("HOME")) != NULL && dputs(name,&sfab) != 0)
						goto LibFail;
					continue;
					}
				// Fall through.
			case '$':
				...
				*str = '\0';
				...
				// Have "$var" or "${var}".
				if((name = getenv(name)) != NULL && dputs(name,&sfab) != 0)
					goto LibFail;
				*str = c2;
				if(braceForm)
					++str;
...
// Save pathname on heap (in path) and set *dest to it.  Return status.
static int savepath(char **dest,char *name) {
...
int pathsearch(char *name,ushort flags,void *result,char *funcname) {
	char *str1,*str2,**np;
	Datum *ename;				// Name with extension, if applicable.
	glob_t *gp;
	char **pathp;
	...
	if(!(flags & XPGlobPat) && ((str1 = strchr(fbasename(name,true),'.')) == NULL || strcmp(str1,scriptExt) != 0)) {
		maxlen += scriptExtLen;
		if(dsalloc(ename,maxlen + 1) != 0)
			goto LibFail;
		sprintf(ename->d_str,"%s%s",name,scriptExt);
		}
	else
		dsetnull(ename);
	...
	if(*name == '/') {
		if(!(flags & XPGlobPat))
			return fexist(name) == 0 ? savepath(pathp,name) :
			 (!disnull(ename) && fexist(ename->d_str) == 0) ? savepath(pathp,ename->d_str) : rc.status;
...
int getpath(char *fname,bool resolve,Datum *pathp) {
	...
	if(!resolve) {
		...
		strcpy(bn,str);
		if(dopenwith(&sfab,pathp,SFClear) != 0)
			goto LibFail;
		if(realpath(fname = fdirname(fname,1),buf) == NULL)
			goto ErrRetn;
		if(dputs(buf,&sfab) != 0 || (strcmp(buf,"/") != 0 && dputc('/',&sfab) != 0) ||
		 dputs(bn,&sfab) != 0 || dclose(&sfab,sf_string) != 0)
			goto LibFail;
		}
	else {
RegFile:
...
int eopendir(char *fspec,char **fp) {
	...
	if((dir = opendir(*rdbuf == '\0' ? "." : rdbuf)) == NULL)
