...
#define Literal34	"N[,bufname]"
#define Literal35	"m"
#define Literal36	"file,type"
#define Literal37	"{op | name}[,bufname,...]"
#define Literal38	"dir"
#define Literal39	"dlm,str[,limit]"
#define Literal40	"var,dlm"
#define Literal41	"var,dlm,val"
#define Literal42	"expr[,opts]"
#define Literal43	"bufname[,opts]"
#define Literal44	"type"
#define Literal45	"[opts]"
#define Literal46	"bufname,attrs"
#define Literal47	"opts,array,expr,..."
#define Literal48	"bufname,modes"
#define Literal49	"[mode,...]"
#define Literal50	"old,new"
#define Literal51	"bufname,file"
#define Literal52	"[m]"
#define Literal53	"bufname,fmt,..."
#define Literal54	"mode[,attr,...]"
#define Literal55	"group[,attr,...]"
#define Literal56	"bufname,group[,opt]"
#define Literal57	"op,{key-lit | name}"
#define Literal58	"[c]"
#define Literal59	"{file | array}"
#define Literal60	"{file | glob}[,opts]"
#define Literal61	"c"
#define Literal62	"opts,arg,..."
#define Literal63	"tab-size"
#define Literal64	"bufname,attr"
#define Literal65	"pair-no,fg-color,bg-color"
#define Literal66	"type,colors"
#define Literal67	"type,name"
#define Literal68	"bufname"
#define Literal69	"file"
#define Literal70	"file[,arg,...]"
#define Literal71	"file[,opts]"
...
Command switches and arguments:\n\
  ...
  -r		    Read-only: open all files with \"ReadOnly\" buffer attribute ON\n\
		    by default; otherwise, OFF.\n\
...
Argument switches:\n\
  ...
  -r		    Read-only: open data file with \"ReadOnly\" buffer attribute ON.\n\
  -rw		    Read-write: open data file with \"ReadOnly\" buffer attribute OFF\n\
...
char
 ...
 text29[] = "Rename",
 text30[] = "AHMCPNTBRC   Size      Buffer              File",
 text31[] = "Active",
 ...
 text41[] = "Type ~#u%s~U for help, ~#u%s~U to quit",
 ...
 text58[] = "Buffer",
 text59[] = "Wrap column",
 text60[] = "%c%cLine:%c%c %lu of %lu, %c%cCol:%c%c %d of %d, %c%cByte:%c%c %lu of %lu (%s%%); %c%cchar%c%c = %s",
 ...
 text254[] = "key literal",
 text255[] = "repetition operand in RE pattern",
 ...
 text339[] = "to",
 text340[] = "%c%cCol:%c%c %d of %d; %c%cchar%c%c = %s",
 ...
 text382[] = "\" ~bwith~B \"",
 text383[] = "(self insert)",
 text384[] = "%c%ci:%c%c %d, %c%cformat:%c%c \"%s\", %c%cinc:%c%c %d",
 ...
 text464[] = "beginning",
 text465[] = "end",
 text466[] = "Case of text",
 text467[] = "~uESC~U|~uq~U ~bStop here~B, ~u.~U ~bStop and go back~B, ~uu~U ~bUndo last~B, ~ur~U ~bRestart~B, ~u?~U ~bHelp~B",
 text468[] = "Scan completed.  Press ~uq~U to quit or ~u.~U to go back.",
 text469[] = "Write to",
 text470[] = "written",
 text471[] = "Cannot write to current buffer";
...
#define CFLit_apropos		"Generate list of commands, macros, functions, aliases, and variables whose names match given\
 pattern in a new buffer and render it per ~bselectBuf~B options (in a pop-up window if default n).  If pattern is plain text,\
 match is successful if name contains pat.  If pattern is nil or a null string, all names are listed.  Returns: ~bselectBuf~B\
 values."
...
#define CFLit_binding		"Return key binding information for given op and value.  Operation keywords are: \"Name\" ->\
 name of command or macro bound to given key-lit binding, or nil if none; \"Validate\" -> key-lit in standardized form, or nil\
 if it is invalid; or \"KeyList\" -> array of key binding(s) bound to given command or macro.  Case of keywords is ignored."
...
#define CFLit_bufAttrQ		"Check if given attribute is set in named buffer and return Boolean result.  Attributes are:\
 \"Active\" (file was read), \"Changed\", \"Constrained\", \"Hidden\", \"Macro\", \"Narrowed\", \"ReadOnly\", and \"TermAttr\"\
...
#define CFLit_bufInfo		"Return information for named buffer or multiple buffers, with comma-separated option(s)\
 specified in string argument opts if n argument.  If opts not specified, an array of form [bufname,filename,bytes,lines] is\
 returned for named buffer, where the second element is the filename associated with the buffer (or nil if none), and the third\
 and fourth elements are the total number of bytes and lines in the buffer; otherwise, if opts contains keyword \"Verbose\", an\
 array of form [bufname,filename,bytes,lines,[buf-attr,...],[buf-mode,...]] is returned instead, where [buf-attr,...] and [buf-\
mode,...] are lists of enabled attributes (as described for the ~bbufAttr?~B function) and modes for the specified buffer.  If\
 bufname is nil, multiple arrays are returned (or buffer names only, if \"Brief\" is specified), nested within a single array. \
 One array or buffer name is returned for each visible buffer by default.  Additional buffers may be selected by specifying\
 zero or more of the following attribute keywords in opts: \"Hidden\" and \"Macro\".  Case of option keywords is ignored."
...
#define CFLit_chgBufAttr	"Clear (n < 0), toggle (n == 0, default), set (n == 1), or clear all and set (n > 1) all of the\
 ...
 \"ReadOnly\", and \"TermAttr\" (terminal attributes enabled).  Case of attribute keywords is ignored.  If n > 1, opts may be\
...
#endif
#define CFLit_cycleDelRing	"Cycle delete ring [-]n times (default 1)."
...
#define CFLit_definedQ		"Check if an object of given type is defined and return result.  Type keywords are: \"Name\"\
 ...
 is ignored.  If \"Name\" is specified, a keyword result is returned if object is found; otherwise, nil.  If one of the other\
 types is specified, a Boolean result is returned.  If \"Mark\" is specified, name argument is assumed to be an ASCII (int)\
 character; otherwise, a string.  Returned keywords are: \"alias\", \"buffer\", \"command\", \"function\", \"macro\", \"pseudo-\
command\" (prefix key), or \"variable\"."
...
#define CFLit_delBuf		"Delete named buffer(s) (and ignore changes if n < 0).  If n >= 0, delete multiple buffers\
 as selected by one or more of the following comma-separated option(s) specified in string argument op: \"AllVisible\" ->\
 select all visible buffers; \"Inactive\" -> selected inactive, visible buffers only; \"Unchanged\" -> selected unchanged,\
 visible buffers only; \"Hidden\" -> include hidden buffers in selection; \"Confirm\" -> ask for confirmation on every buffer;\
 and \"Force\" -> delete all selected buffers, ignoring changes, with initial confirmation.  Case of option keywords is\
 ignored.  Either \"AllVisible\", \"Inactive\", or \"Unchanged\" must be specified, and \"Confirm\" and \"Force\" may not both\
 be specified.  If neither of the latter two options is specified, buffers are deleted with confirmation on changed ones only. \
 Buffers being displayed and macro buffers are skipped unconditionally.  Hidden buffers are also skipped unless \"Hidden\" is\
 specified.  Returns: zero if failure; otherwise, number of buffers deleted."
#define CFLit_delDelEntry	"Delete the most recent n entries (default 1) from delete ring (or entry number n if n < 0, all\
 entries if n == 0)."
...
#define CFLit_delKillEntry	"Delete the most recent n entries (default 1) from kill ring (or entry number n if n < 0, all\
 entries if n == 0)."
#define CFLit_delLine		"Delete [-]n lines (default 1, region lines if n == 0)."
#define CFLit_delMacro		"Delete named macro(s).  Returns: zero if failure; otherwise, number of macros deleted."
...
#define CFLit_delSearchPat	"Delete the most recent n entries (default 1) from search ring (or entry number n if n < 0, all\
 entries if n == 0).  The most recent remaining entry is subsequently set as the current search pattern."
#define CFLit_delReplacePat	"Delete the most recent n entries (default 1) from replace ring (or entry number n if n < 0,\
...
#define CFLit_getInfo		"Return informational item for given type keyword: \"Colors\" -> list of display item colors\
 in form [[item-kw,[fg-color,bg-color]],...] where color pair array is nil if color not set for item (or number of colors and\
 color pairs available in form [colors,pairs] if n argument); \"Editor\" -> editor name; \"Hooks\" -> hook list in form\
 [[hook-name,macro-name],...] where macro-name is nil if hook not set; \"Language\" -> language of text messages; \"Modes\" ->\
 mode list in form [[mode-name,group-name,user?,global?,hidden?,scope-locked?,active?],...] where group-name is nil if mode is\
 not in a group, and last five elements are Boolean values; \"OS\" -> operating system name; \"Screens\" -> screen list in form\
 [[screen-num,wind-count,work-dir],...]; \"Version\" -> editor version; or \"Windows\" -> window list in form [[wind-num,\
bufname],...] (or form [[screen-num,wind-num,bufname],...] if n argument).  Case of keywords is ignored."
...
#define CFLit_gotoMark		"Move point to mark m and restore its window framing.  If n < 0, also delete mark after point\
 is moved."
...
#define CFLit_includeQ		"Return true if any expression value(s) are in array; otherwise, false.  opts must be either a\
 string containing one or more comma-separated option(s), or nil if none.  Options are: \"All\" -> check if all expressions are\
 in array instead of any; and \"Ignore\" -> ignore case in string comparisons.  Case of option keywords is ignored."
...
#define CFLit_markBuf	"Save point position in mark " WrkMarkStr " (or mark m if n argument) and mark whole buffer as region."
...
#define CFLit_message		"Set a return message, with one or more comma-separated option(s) specified in string argument\
 opts (or nil if none), and return true (or false if n < 0).  Argument(s) following opts are converted to string and\
 concatenated to form the message.  By default, a message is set only if one has not already been set, and is wrapped in\
 brackets (thus informational) if default n or n >= 0.  Options are: \"Force\" -> any existing message is replaced\
 unconditionally (forced); \"High\" -> message is considered high priority and will replace any existing message that is not\
 high priority and was not forced; \"NoWrap\" -> do not wrap message in brackets; and \"TermAttr\" -> enable terminal\
 attributes in message.  Case of option keywords is ignored.  If \"Force\" is specified and message is nil or a null string,\
 any existing message is cleared."
...
#define CFLit_prompt		"Get terminal input and return string, ASCII (int) character, or nil result, given prompt\
 ...
 arguments that follow (if any) are strings in form \"option\" or \"keyword: value\".  Options of the first form are:\
 ...
 sequence; \"KillRing\" -> kill ring access; \"MutableVar\" -> mutable variable name completion; \"ReplaceRing\" -> replace\
 ring access; \"SearchRing\" -> search ring access; \"String\" -> string (default); or \"Var\" -> variable name completion\
 ...
 the string to be used as is) and any trailing white space is moved to the very end of the final prompt string that is built. \
 If a non-empty default value is specified, it is displayed after the prompt string between square brackets ~#u[ ]~U if the\
 prompt type is \"Char\", and must be an ASCII character in that case; otherwise, it must be a string or int and is stored in\
 the input buffer in string form as the initial value.  A key literal is returned for types \"Key\" and \"KeySeq\"; otherwise,\
 if the user presses just the delimiter key, the default value (if specified) is returned as an ASCII character if prompt type\
 is \"Char\"; otherwise, nil is returned.  If the user presses ~uC-SPC~U at any time, a null string is returned instead (or a\
 null character if the prompt type is \"Char\"); otherwise, the string in the input buffer is returned when the delimiter key\
 is pressed."
...
#define CFLit_setDispColor	"Set color for a display item, given item type and either a two-element array of foreground\
 ...
 \"ModeLine\" for mode lines, and \"Record\" for keyboard macro recording indicator."
...
#define CFLit_setMark		"Save point position and window framing in mark " RegMarkStr " (or mark m if n argument)."
...
#define CFLit_shellCmd		"Execute a shell command and display result in a pop-up window.  opts must be either a string\
 containing one or more comma-separated option(s), or nil if none.  Argument(s) following opts are converted to string and\
 concatenated to form the command.  Options are: \"NoHdr\" -> do not write command at top of result buffer; \"NoPop\" -> skip\
 pop-up; and \"Shift\" -> shift long lines left in pop-up window.  Case of option keywords is ignored.  Returns: false if\
 command fails; otherwise, true."
...
#define CFLit_showBuffers	"Generate list of visible buffers (or all buffers if n argument, excluding macros if n == -1)\
 ...
 buffer attribute and state information: ~u~bA~Zctive, ~u~bH~Zidden, ~u~bM~Zacro, ~u~bC~Zonstrained, ~u~bP~Zreprocessed,\
 ~u~bN~Zarrowed, ~u~bT~ZermAttr, ~u~bB~Zackground, ~u~bR~ZeadOnly, and ~u~bC~Zhanged.  Returns: ~bselectBuf~B values."
...
#define CFLit_showCommands	"Generate list of commands and their bindings matching given pattern in a new buffer and render\
 it per ~bselectBuf~B options (in a pop-up window if default n).  If pattern is plain text, match is successful if name\
 contains pat.  If pattern is nil or a null string, all commands are listed.  Returns: ~bselectBuf~B values."
#define CFLit_showDelRing	"Generate list of delete ring entries in a new buffer and render it per ~bselectBuf~B options\
...
#define CFLit_showFunctions	"Generate list of system functions matching given pattern in a new buffer and render it per\
 ~bselectBuf~B options (in a pop-up window if default n).  If pattern is plain text, match is successful if name contains pat. \
 If pattern is nil or a null string, all functions are listed.  Returns: ~bselectBuf~B values."
...
#define CFLit_showMacros	"Generate list of non-constrained macros (or all macros if n argument) and their bindings\
 matching given pattern in a new buffer and render it per ~bselectBuf~B options (in a pop-up window if default n).  If pattern\
 is plain text, match is successful if name contains pat.  If pattern is nil or a null string, all macros of type selected by n\
 argument are listed.  Returns: ~bselectBuf~B values."
...
#define CFLit_swapMark		"Swap point position with mark m and restore its window framing."
...
#define CFLit_toStr		"Return expression value converted to string form.  If n argument, one or more of the following\
 comma-separated option(s) may be specified in string argument opts to control the conversion process: \"Delimiters\" ->\
 ...
 ignored if expression is an array and \"Delimiters\" is not also specified."
...
#define CFLit_writeFile		"Write current buffer to named file.  If default n or n > 0, also set buffer's filename to file\
 path and derive new buffer name from it."
...
#define CFLit_xPathname		"Search for given file or directory in $execPath directories and return\
 absolute pathname if found; otherwise, nil.  (Pathname is searched for verbatim first, then with \"" Script_Ext "\" appended\
...
#define VLit_delRingSize	"Maximum size of delete ring (or unlimited if zero)."
 ...
 *help0,*help1,*help2[],*usage[],
 ...
 text435[],text436[],text437[],text438[],text439[],text440[],text441[],text442[],text443[],text444[],text445[],text446[],
 text447[],text448[],text449[],text450[],text451[],text452[],text453[],text454[],text455[],text456[],text457[],text458[],
 text459[],text460[],text461[],text462[],text463[],text464[],text465[],text466[],text467[],text468[],text469[],text470[],
 text471[];
#if MMDebug & Debug_ShowRE
extern char
