...
int vputc(int c,StrList *slp) {
	...
	if(c == '\0') {
		*slp->sl_strp = '\0';
		return vmsgf(-1,"vputc(): Cannot store a null byte! (in buf '%.16s ...')",slp->sl_buf);
		}
#endif
...
int vunputc(StrList *slp) {
	...
	return vmsg(-1,"vunputc(): No bytes left to \"unput\"");
	}
...
solo:
			...
			break;
		default:	// VALSLIST
			return vmsg(-1,"vputv(): Cannot put a string list!");
#endif
		}
...
#if VTest
			if(len == 0)
				return vmsg(-1,"vmksolo(): Zero length string list!");
#endif
			// Get heap space ...
...
#if VTest
		if(vp->v_type != VALSLIST)
			return vmsgf(-1,"vopen(): \"Get\" value object (type %hu, '%.16s ...') not a list!",vp->v_type,
			 vp->v_strp);
#endif
		slp->sl_vp = vp;
