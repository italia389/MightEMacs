...
bebuf:
		...
		case cf_includeQ:
			...
			if(!vvoid(argp[0]) && !visnull(argp[2])) {
				...
				ushort dlm = nildlm ? -1 : *argp[1]->v_strp;
				strp = argp[0]->v_strp;
				for(;;) {
				...
				strp = argp[0]->v_strp;
				do {
					if(strncmp(strp,argp[2]->v_strp,len) == 0) {

						// Match found.  Check following byte.
						strp += len;
						if((i = (*strp == '\0' || *strp == dlm ||
						 (dlm < 0 && (*strp == ' ' || *strp == '\t')))))
							break;
						++strp;
						}

					// Match not found or not at delimiter boundary.  Find next delimiter and continue.
					for(;;) {
						if(*strp == '\0')
							goto rtn;
						if(*strp == dlm)
