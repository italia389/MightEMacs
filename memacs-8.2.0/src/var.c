	...
	if(svp->sv_vp != NULL) {
		...
		switch(vid = svp->sv_id) {
			...
			case sv_lineChar:
				...
				vsetchr(lused(dotp->lnp) == dotp->off ? '\r' : lgetc(dotp->lnp,dotp->off),rp);
				}
				break;
			case sv_lineCol:
...
int setwlist(char *wclistp) {
	...
	// expand the new value (and close the string list)...
...
int setvar(Value *rp,int n) {
	...
	if(vnew(&vp,false) != 0)
		return vrcset();
	if(terminp(vp,text51,NULL,CTRL | 'M',0,TERM_C_SVAR) != SUCCESS || vistfn(vp,VNIL))
			// "Assign variable"
		return rc.status;
...
int showVariables(Value *rp,int n) {
	...
	do {
		...
		if(doapropos && strcasestr(svp->sv_name,mstrp->v_strp) == NULL)
			continue;
	...
	do {
		if((uvp = **uvppp) != NULL) {
			...
			do {
				...
				// Add in the value.
				if((strp = vizstr(uvp->uv_vp,&wasStr)) == NULL)
					return rc.status;
				if(wasStr) {
					if(vstrlit(&rpt,strp,0) != 0)
						return vrcset();
