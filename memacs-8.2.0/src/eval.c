...
// Examine a value object and return its visible (string) form, or NULL if error.  If object was originally a string (and not
// nil or Boolean), set *wasStr to true; otherwise, false.
...
bebuf:
		...
		case cf_env:
			...
			break;
		case cf_fileExistsQ:
			if(vsetstr((i = fexist(argp[0]->v_strp)) < 0 ? val_nil : i > 0 ? text335 : text99,rp) != 0)
										// "directory","file"
				(void) vrcset();
			break;
		case cf_findFile:
