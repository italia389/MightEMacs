...
static int getkb(char *prmtp,int n,uint *result) {
...
int aliasCFM(Value *rp,int n) {
	...
	else if(terminp(vnamep,text215,NULL,CTRL | 'M',0,0) != SUCCESS || vistfn(vnamep,VNIL))
			// "Create alias "
		return rc.status;
...
int apropos(Value *mstrp,char *prmtp) {
	...
	sprintf(wkbuf,"%s %s",text20,prmtp);
			// "Apropos"
	if(getarg(mstrp,wkbuf,"",CTRL | 'M',0,ARG_FIRST | ARG_STR) == SUCCESS && vistfn(mstrp,VNIL))
		vnull(mstrp);
...
int showBindings(Value *rp,int n) {
	...
	do {
		...
		if((cfp->cf_flags & CFFUNC) || (doapropos && strcasestr(cfp->cf_name,mstrp->v_strp) == NULL))
			continue;
	...
	for(ap = aheadp; ap != NULL; ap = ap->a_nextp) {
		...
		if(doapropos && strcasestr(ap->a_name,mstrp->v_strp) == NULL && strcasestr(ap->a_type == PTRALIAS_M ?
...
static bool stoek1(char **klpp,int *cp,bool *firstp) {
	int c;
	int ek = *cp;
	struct clit *ltabp;
	char *klp = *klpp;
...
int stoek(char *keylitp,int *resultp) {
	int ek = 0;		// Extended key to return.
