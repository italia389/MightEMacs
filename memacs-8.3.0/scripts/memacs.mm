...
# Rename file associated with current buffer.
macro renameFile,1
	...
	fileExists?($bufFile) and shellCmd 'mv ',shQuote($bufFile),' ',shQuote(newFile)
...
# Fence word(s); that is, put quotes or fence characters around one or more words at point.
macro fenceWord,1
	fencePairs = '\'\':"":():[]:{}:<>'
	formerMsgState = -1 => alterGlobalMode 'msg'
	loop
...
macro grepFiles,0
	...
	deleteBuf listBuf
	formerMsgState => alterGlobalMode 'msg'
	notice count,' files found: ',2 => sub(fileList,TabDelim,', ')
endmacro
