...
$RubyClassModuleRE = '^[ \t]*[cm][lo][ad][su][sl]e? +%NAME%\b:re'

##### Macros #####

# Wrap "=begin" and "=end" around a block of lines.
macro rubyWrapBeginEnd(0) {desc: 'Wrap "=begin" and "=end" around a block of lines'}
...
macro rubyFindMethod(0) {desc: 'Find first file that contains requested method name (with "selectBuf" options)'}
	$0 => langFindRoutine '$RubyMethodRE','$rubyFindDir','*.rb','Method'
endmacro

# Find first file that contains requested class or module name and render it according to n argument if found.
macro rubyFindClassModule(0) {desc: 'Find first file that contains requested class or module name (with "selectBuf" options)'}
	$0 => langFindRoutine '$RubyClassModuleRE','$rubyFindDir','*.rb','Class or module'
endmacro
...
macro rubyGotoBlockEnd(0) {desc: "Go to block keyword paired with one in current line and set marks '.','T','E','B'"}
	formerMsgState = -1 => alterGlobalMode 'msg'
	wordChars = $wordChars
	$wordChars = 'A-Za-z'
	1 => setMark '?'
