...
static int mlrquery(void) {
	...
	strfit(buf,(size_t) term.t_ncol / 2 - 9,mtp->matchp->d_str,mtp->groups[0].ml.reg.r_size);
	...
	if(rebmode() && (mtp->flags & RRegical)) {
		...
		if(dopentrk(&msg) != 0)
			return drcset();
		while(rmcp->mc_type != MCE_Nil) {
			if(dputs(rmcp->mc_type == MCE_LitString ? rmcp->u.rstr : rmcp->mc_type == MCE_Match ?
			 mtp->matchp->d_str : mtp->groups[rmcp->u.grpnum].matchp->d_str,&msg) != 0)
				return drcset();
			++rmcp;
			}
		...
		strfit(buf,(size_t) term.t_ncol - ml.ttcol - 2,msg.sf_datp->d_str,0);
		}
	else
...
// otherwise, get them interactively.  If rp is not NULL, do query replace and set rp to false if search ends prematurely;
// otherwise, true.  If dot has moved after search ends, set mark '.' to original position and notify user of such.  Return
// status.
int replstr(Datum *rp,int n,Datum **argpp) {
	...
	if(!qrepl)
		...
		if(dnewtrk(&last.matchstrp) != 0)
			return drcset();

		// Initialize the screen if it's garbage.
		if((opflags & OpScrRedraw) && update(true) != Success)
			return rc.status;
		}
...
Retn:
	...
	if(dotp->lnp != origdot.mk_dot.lnp || dotp->off != origdot.mk_dot.off) {
		...
		if(dopenwith(&msg,&rc.msg,true) != 0 || dputs(", ",&msg) != 0 ||
