...
static int supd_redraw(void) {
	...
	for(i = 0; i < (int) term.t_nrow - 1; ++i) {		// Exclude message line.
		...
		pscreen[i]->v_right = 0;
		strz = (str = pscreen[i]->v_text) + term.t_ncol;
		do {
			*str++ = ' ';
			} while(str < strz);
...
void wupd_modeline(EWindow *winp,Buffer *popbuf) {
	...
	if(popbuf == NULL) {
		...
		md[0].flags = modetab[MdRec_Global].flags & modetab[MdRec_Show].flags;
	...
	// well.
	if(winp == curwp && popbuf == NULL && (modetab[MdRec_Global].flags & MdWkDir) && ((int) term.t_ncol - n) > 12) {
		char *wd;
		char wkbuf[TT_MaxCols];
		...
		n += vtputs(text274);
			// "WD: "
		(void) getwkdir(&wd,false);
		n += vtputs(strfit(wkbuf,term.t_ncol - n - 1,wd,0)) + 1;
		vtputc(' ');
		}
...
static void wupd_cursor(void) {
	...
	if(modetab[MdRec_Global].flags & MdHScrl) {
...
int update(bool force) {
	...
	if(opflags & OpScrRedraw) {
		if(modetab[MdRec_Global].flags & MdNoUpd)
			opflags &= ~OpScrRedraw;
...
static int hkey(DStrFab *destp,int (*cfunc)(Datum *rp,int n,Datum **argpp),int c) {
		...
		if(kdp->k_cfab.p_type == PtrCmd && kdp->k_cfab.u.p_cfp->cf_func == cfunc) {
			...
			if((kdp->k_code & KeySeq) == 0) {
				...
				if(dputs(str,destp) != 0)
					return drcset();
				str = ",";
				if(dputs(ektos(kdp->k_code,wkbuf),destp) != 0)
					return drcset();
				found = true;
				}
			}
	...
	return dputf(destp,") %c%s",c,text205) == 0 ? rc.status : drcset();
				// "line"
	}
...
int bpop(Buffer *bufp,bool altmodeline,bool endprompt) {
	...
	char *str,*strz;	// Line text pointers.
	...
	for(;;) {
		...
		if(n != 0 && lnp1 == lnp)
			...
			do {
				...
				vtmove(crow,0);
				strz = (str = ltext(lnp)) + (lused(lnp) <= (int) term.t_ncol ? lused(lnp) : term.t_ncol);
				while(str < strz)
					vtputc(*str++);
				vteeol();
#if Color
		...
		for(;;) {
			...
			if(ek == '?') {
				...
				if(dclose(&msg,sf_string) != 0)
					return drcset();
				mlputs(MLHome | MLForce,msg.sf_datp->d_str);
				}
			else {
