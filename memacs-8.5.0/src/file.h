...
extern int fileout(Datum *rp,int n,char *prmt,int mode);
extern int getfile(Datum *rp,int n,bool view);
extern int getpath(Datum *pathp,int n,char *fname);
extern int getwkdir(char **strp,bool force);
extern int gtfilename(Datum *rp,char *prmt,uint flags);
...
extern int rdfile(Datum *rp,int n,char *fname,bool scratch);
extern int readin(struct Buffer *bufp,char *fname,bool keep);
extern int readPipe(Datum *rp,int n,Datum **argpp);
#if USG || HPUX8
extern int rename(const char *file1,const char *file2);
#endif
extern int savebufs(int n,bool edexit);
extern int setBufFile(Datum *rp,int n,Datum **argpp);
extern int writeout(struct Buffer *bufp,char *fn,int mode);
