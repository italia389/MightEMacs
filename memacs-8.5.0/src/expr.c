...
//	To accomplish this, it (a), scans the garbage list and sets aw_mark to false in each array; (b), scans the global
//	variable list and sets aw_mark to true in any arrays and nested arrays found via the akeep() function, which also
//	uses aw_mark during recursion to prevent an endless loop if an array contains itself; and (c), scans the garbage list
//	again and frees any arrays which have aw_mark set to false; otherwise, clears the flag.
...
static int ausage(DStrFab *sfp,int stage) {
	...
	if(stage <= 0) {
		...
		do {
			...
			if(atosfc(sfp,&d,NULL,CvtExpr | CvtForceArray) != Success)
				return rc.status;
#else
...
static void akeep(Datum *datp) {
	...
	if(!awptr(datp)->aw_mark) {
		awptr(datp)->aw_mark = true;
		aryp = awptr(datp)->aw_aryp;
		elppz = (elpp = aryp->a_elpp) + aryp->a_used;
		while(elpp < elppz) {
			if((*elpp)->d_type == dat_blobRef)
				akeep(*elpp);
...
int agarbfree(void) {
	...
	if(aryGarbp != NULL) {
		...
		if(ausage(&sf,-1) != Success)
			return rc.status;
#endif
		// Scan global variable table and mark any arrays found as "keepers".
		for(uvp = gvarsheadp; uvp != NULL; uvp = uvp->uv_nextp)
			if(uvp->uv_datp->d_type == dat_blobRef)
				akeep(uvp->uv_datp);
#if MMDebug & Debug_Array
...
static int fcall(ENode *np,bool needrparen,bool *foundp) {
	...
	if(cfabsearch(np->en_rp->d_str,&cfab,PtrCFAM) == 0) {
		...
		if(cfab.p_type == PtrCmd) {
			if(cfab.u.p_cfp->cf_aflags & CFTerm)
				return rcset(Failure,0,text282,cfab.u.p_cfp->cf_name);
					// "'%s' command not allowed in a script (use \"run\")"
		...
		// Set to -1 if unknown.
		short minArgs = (cfab.p_type == PtrMacro) ? (cfab.u.p_bufp->b_mip->mi_nargs < 0 ? 0 :
		 cfab.u.p_bufp->b_mip->mi_nargs) : !(cfab.u.p_cfp->cf_aflags & (CFAddlArg | CFNoArgs)) ?
		 cfab.u.p_cfp->cf_minArgs : !(opflags & OpEval) ? -1 : np->en_narg == INT_MIN ? cfab.u.p_cfp->cf_minArgs :
		 (cfab.u.p_cfp->cf_aflags & CFNoArgs) ? 0 : cfab.u.p_cfp->cf_minArgs + 1;

		// Check if "xxx()" call.
		if(needrparen && havesym(s_rparen,false)) {

			// Error if argument(s) required (whether or not evaluating).
			if(minArgs > 0 || ((cfab.u.p_cfp->cf_aflags & CFNoArgs) && !(np->en_flags & EN_HaveNArg)))
				goto Wrong;
		...
		short maxArgs = (cfab.p_type == PtrMacro) ? (cfab.u.p_bufp->b_mip->mi_nargs < 0 ? SHRT_MAX :
		 cfab.u.p_bufp->b_mip->mi_nargs) : ((opflags & OpEval) && np->en_narg != INT_MIN &&
		 (cfab.u.p_cfp->cf_aflags & CFNoArgs)) ? 0 : cfab.u.p_cfp->cf_maxArgs < 0 ? SHRT_MAX : cfab.u.p_cfp->cf_maxArgs;
...
static int concat(ENode *np1,ENode *np2,bool clone) {
	...
	if(np1->en_rp->d_type == dat_blobRef) {
		...
		if(dopenwith(&sf,np1->en_rp,true) != 0 || dputd(np2->en_rp,&sf) != 0 || dclose(&sf,sf_string) != 0)
			(void) drcset();
		}
	return rc.status;
	}
...
static int aequal(Datum *datp1,Datum *datp2,bool *resultp) {
	...
	if(awp1->aw_mark || awp2->aw_mark)
		return rcset(Failure,0,text374);
...
int ge_assign(ENode *np) {
	...
	// Have valid operator.  Check if node is an lvalue.
	if(!(np->en_flags & (EN_HaveIdent | EN_ArrayRef)))
		goto BadLValue;

	// Build a VDesc object from node if evaluating.
	if(opflags & OpEval) {
		if(np->en_flags & EN_HaveIdent) {
...
int funcarg(Datum *rp,uint aflags) {
	...
	if(opflags & OpEval) {
		if((aflags & (CFInt1 | CFMay)) == CFInt1)			// 'int' flag set without "may be"?
			(void) intval(rp);					// Yes, must be integer.
		else if((aflags & (CFArray1 | CFNIS1 | CFMay)) == CFArray1)	// No, 'array' set without other types?
			(void) aryval(rp);					// Yes, must be array.
		else if(!(aflags & (CFNil1 | CFBool1 | CFInt1 | CFArray1 | CFNIS1))) {	// No, non-string flags not set?
			if(!strval(rp))						// Yes, must be string.
				return rc.status;
			if((aflags & CFNotNull1) && disnull(rp))		// Check if null string is an error.
				goto NotNull;
			}
		else if((!(aflags & CFArray1) && rp->d_type == dat_blobRef) ||	// No required types.  Check if found type
		 (!(aflags & CFBool1) && (rp->d_type & DBoolMask)) ||		// is allowed.
		 (!(aflags & CFNIS1) && (
		 (!(aflags & CFNil1) && rp->d_type == dat_nil) ||
		 (!(aflags & CFInt1) && rp->d_type == dat_int))))
			return rcset(Failure,0,text329,dtype(rp,false));
					// "Unexpected %s argument"
		else if((rp->d_type & DStrMask) && (aflags & CFNotNull1) && disnull(rp))	// Null string error?
NotNull:
