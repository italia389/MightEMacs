...
//
// Programming Notes:
// 1. In general, keys are read in editloop(), commands or macros are called with a numeric argument, and a status code is
// returned.  If no numeric argument is specified by the user, INT_MIN is used as the default (and many commands will have a
// default behavior in that case).  The called routines typically call (many) other routines which return a status code as well.
// The return code is usually SUCCESS or FAILURE; however, it can be more severe (see edef.h for list), such as USERABORT or
...
// function in the call chain will be ultimately be returned to editloop(), where it will be dealt with; that is, a message will
// be displayed if applicable before beginning the next loop.  This means that a more severe return code and message will always
...
int chkcpy(char **destp,char *srcp,size_t destlen,char *emsgp) {
	...
	if((srclen = strlen(srcp)) >= destlen)
		return rcset(FAILURE,0,text255,emsgp,strsamp(srcp,srclen,0));
			// "%s string '%s' too long"
