...
bool getnum(char *prmtp,int *np) {
	...
	if(opflags & OPSCRIPT)
		...
		if(vnew(&vp,false) != 0)
			(void) vrcset();
		else if(terminp(vp,prmtp,NULL,CTRL | 'M',0,ARG_NOTNULL) == SUCCESS && !vistfn(vp,VNIL) &&
		 toint(vp) == SUCCESS) {
			*np = vp->u.v_int;
			return true;
			}
...
int gotoLine(Value *rp,int n) {
	...
	sprintf(wkbuf,"%s %s",text7,text205);
			// "Go to","line"
	if(getarg(vp,wkbuf,NULL,CTRL | 'M',0,ARG_FIRST | ARG_NOTNULL) != SUCCESS ||
	 (!(opflags & OPSCRIPT) && vistfn(vp,VNIL)) || toint(vp) != SUCCESS)
		return rc.status;
	if((line = vp->u.v_int) < 0)
...
static int getmark(char *prmptp,int n,uint flags,Mark **mkpp) {
	...
	if(opflags & OPSCRIPT) {
		...
		if(terminp(vp,pbuf,NULL,CTRL | 'M',0,ARG_NOTNULL | ARG_PRINT | TERM_ONEKEY) != SUCCESS)
			return rc.status;
		if(vistfn(vp,VNIL)) {
			*mkpp = NULL;
			return rc.status;
...
int showMarks(Value *rp,int n) {
	...
	if(vputs(text354,&rpt) != 0 || vputc('\r',&rpt) != 0 || vputs("----  ------  ",&rpt) != 0)
	...
	for(;;) {
		...
		do {
			if(mkp->mk_id <= '~' && mkp->mk_dot.lnp == lnp) {
				mkp->mk_id == ' ' ? (void) strcpy(wkbuf,"\r' '") : (void) sprintf(wkbuf,"\r %c ",mkp->mk_id);
				sprintf(strchr(wkbuf,'\0')," %8d",mkp->mk_dot.off);
				if(vputs(wkbuf,&rpt) != 0)
					return vrcset();
				if(n1 && (vputs("  ",&rpt) != 0 || vstrlit(&rpt,lnp == curbp->b_hdrlnp ? "(EOB)" : ltext(lnp),
				 lused(lnp) > max ? max : lused(lnp)) != 0))
					return vrcset();
				n1 = 0;
				}
			} while((mkp = mkp->mk_nextp) != NULL);
