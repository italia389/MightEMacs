...
// Get one keystroke from the terminal driver, resolve any keyboard macro action, and return it in *keyp as an extended key.
// The legal prefixes here are the FKey and Ctrl.  If savekey is true, store extended key in kentry.lastkseq.  Return status.
int getkey(bool ml,ushort *keyp,bool savekey) {
	ushort ek;
...
int getkseq(bool ml,ushort *keyp,KeyBind **kbpp,bool savekey) {
...
// Insert a character into the message line input buffer.  If flush is true, call mlflush() if echoing.
...
fprintf(logfile,"mli_putc('%s',...)...\n",vizc(c,VBaseDef));
		...
		if((isp->flags & Term_NoKeyEcho))
			...
			if(((term.mlcol + len < term.t_ncol - 1) ?
			 mli_write(isp,term.mlcol,1,false,NULL,term.mlcol + len) : mli_update(isp,false)) != Success)
				return rc.status;
			goto Flush;
			}
		}
...
static int cMatchBuf(InpState *isp) {
	...
	while(isp->ibufe <= isp->ibufz) {
		...
		comflag = true;
		isp->ibufc = isp->ibufe;
		if(mli_putc(c,isp,false) != Success || mlflush() != Success)
			return rc.status;
		++n;
		}
Fail:
