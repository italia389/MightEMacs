...
ClearSet:
	...
	if(!(bufp->b_flags & BFMacro) && fnChange)
		(void) exechook(NULL,INT_MIN,hooktab + HkFilename,2,bufp->b_bname,bufp->b_fname);
...
int bconfirm(Buffer *bufp,ushort flags) {
	bool yep;
	bool eraseML = false;
	bool eraseOK = false;
...
int bclear(Buffer *bufp,ushort flags) {
	...
	if(bconfirm(bufp,flags) != Success)
		return rc.status;
	...
	if(!(si.opflags & OpScript)) {
		...
		do {
			if(bfsp->abbr != NULL) {
				...
				if(dputf(&prompt,"%s%c%c%c%c%c%c%c%s",lead,AttrSpecBegin,AttrULOn,AttrSpecBegin,AttrBoldOn,
				 (int) attrname[0],AttrSpecBegin,AttrAllOff,attrname + 1) != 0)
					goto LibFail;
				lead = ", ";
				}
...
int insertBuf(Datum *rp,int n,Datum **argpp) {
	...
	return idata(n,bufp,&di) != Success ? rc.status :
...
int dopop(Datum *rp,int n,bool popbuf) {
	...
	if(popbuf) {
		...
		bool created;
		ushort cflags = n <= 0 && n != INT_MIN ? BS_Create | BS_Derive : BS_Create | BS_Derive | BS_Force;
	...
	if(n != INT_MIN) {
		...
		if(si.opflags & OpScript) {
			if(funcarg(rp,ArgNotNull1) != Success)
				...
				// "Options (~u~ba~Zlt mode line, ~u~bd~Zelete buffer, ~u~bs~Zhift long lines, ~u~bt~Zermattr)"
				// "Opts (~u~ba~Zltml, ~u~bd~Zel, ~u~bs~Zhift, ~u~bt~Zattr)"
			return rc.status;
		str = rp->d_str;
		do {
			switch(*str) {
...
// Create a scratch buffer; that is, one with a unique name and no filename associated with it.  Set *bufpp to buffer pointer,
// and return status.
int bscratch(Buffer **bufpp) {
	uint maxtries = 100;
	Buffer *bufp;
	bool created;
	...
	do {
		...
		if(bfind(bname,BS_Create | BS_Hook,0,&bufp,&created) != Success)
			return rc.status;
	...
	(void) bfind(bname,BS_Create | BS_Hook | BS_Force,0,&bufp,NULL);
Retn:
	*bufpp = bufp;
	return rc.status;
	}
...
int bhook(Datum **rpp,ushort flags) {

	if(!(si.curbp->b_flags & BFMacro)) {
		if(*rpp == NULL && dnewtrk(rpp) != 0)
			return librcset(Failure);
		if(flags & SWB_ExitHook) {

			// Run exit-buffer user hook on current (old) buffer.
			dsetnil(*rpp);
			(void) exechook(*rpp,INT_MIN,hooktab + HkExitBuf,0);
			}
		else
			// Run enter-buffer user hook on current (new) buffer.
			(void) exechook(*rpp,INT_MIN,hooktab + HkEnterBuf,0x21,*rpp);
		}
	return rc.status;
	}
...
int bswitch(Buffer *bufp,ushort flags) {
	Datum *rp = NULL;
	...
	if(!(flags & SWB_NoHooks) && bhook(&rp,SWB_ExitHook) != Success)
		return rc.status;
...
int pnbuffer(Datum *rp,int n,bool backward) {
	Buffer *bufp0,*bufp1;
	ushort bflags;
	Datum **blp0,**blp,**blpz;
	ssize_t index;
...
int renameBuf(Datum *rp,int n,Datum **argpp) {
	...
	if(n == INT_MIN)
		bufp = si.curbp;
	else if(bcomplete(rp,text29,n > 0 ? si.curbp->b_bname : (bufp = bdefault()) != NULL ? bufp->b_bname : NULL,
...
int showBuffers(Datum *rp,int n,Datum **argpp) {
	...
	static ColHdrWidth hcols[] = {
		{8,8},{9,9},{MaxBufName - 4,MaxBufName - 4},{31,8},{0,0}};
	...
	if(rpthdr(&rpt,text159,false,text30,hcols,&pagewidth) != Success)
			// "Buffers","ACHMCPNT   Size      Buffer              File"
		return rc.status;
	...
	while((datp = aeach(&aryp)) != NULL) {
		...
		sbp = sb;
		do {
			if(!(bufp->b_flags & sbp->bflag))
				c = ' ';
			else if((c = sbp->sbchar) == AttrSpecBegin && dputc(AttrSpecBegin,&rpt) != 0)
				goto LibFail;
			if(dputc(c,&rpt) != 0)
				goto LibFail;
			} while((++sbp)->bflag != 0);
		...
		dputf(&rpt,"%3hu",bufp->b_nwind);
#endif
		// Output the buffer size, buffer name, and filename.
		if(dputf(&rpt,"%s%*lu%s%-*s",space,hcols[1].minwidth,buflength(bufp,NULL),space,hcols[2].minwidth,
		 bufp->b_bname) != 0 || (bufp->b_fname != NULL && (dputs(space,&rpt) != 0 || dputs(bufp->b_fname,&rpt) != 0)))
			goto LibFail;
		}
