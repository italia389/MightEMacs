...
static int putctext(char *iline) {
	...
	if(si.curwp->w_face.wf_dot.lnp->l_used > 0) {
...
static int getsvar(Datum *rp,SVar *svp) {
	...
	switch(vid = svp->sv_id) {
		...
		case sv_TermSize:
			...
			if((aryp = anew(2,NULL)) == NULL)
				goto LibFail;
			if(awrap(rp,aryp) == Success) {
				dsetint((long) term.t_ncol,aryp->a_elpp[0]);
				dsetint((long) term.t_nrow,aryp->a_elpp[1]);
				}
			}
			break;
		case sv_WindCount:
	...
	if(vd.vd_type == VTyp_SVar) {
		if(vd.p.vd_svp->sv_flags & (V_GetKey | V_GetKeySeq)) {
			if(getsvar(datp,vd.p.vd_svp) != Success)
				goto Retn;
			if(dputf(&sf,"%c%c%c%s%c%c",AttrSpecBegin,AttrAlt,AttrULOn,datp->d_str,AttrSpecBegin,AttrULOff) != 0)
				goto LibFail;
			cflags |= Term_Attr;
			}
