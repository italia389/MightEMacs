	...
	switch(itp->id) {
		...
		case cf_showColors:;		// [[item-kw,[fg-color,bg-color]],...] or [colors,pairs]
			if(n == INT_MIN) {
				...
				// Cycle through color items and make list.  For each item, create array in aryp1 and push onto
				// aryp0.  Also create color-pair array in aryp and store in aryp1 if applicable.
				ctabpz = (ctabp = term.itemColor) + sizeof(term.itemColor) / sizeof(ItemColor);
				do {
					if((aryp1 = anew(2,NULL)) == NULL)
						goto LibFail;
					if(ctabp->colors[0] < -1)
						dsetnil(aryp1->a_elpp[1]);
					else {
						if((aryp = anew(2,NULL)) == NULL)
							goto LibFail;
						dsetint(ctabp->colors[0],aryp->a_elpp[0]);
						dsetint(ctabp->colors[1],aryp->a_elpp[1]);
						if(awrap(aryp1->a_elpp[1],aryp) != Success)
							goto Retn;
						}
					if(dsetstr(ctabp->name,aryp1->a_elpp[0]) != 0 ||
