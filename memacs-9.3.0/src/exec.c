...
int showHooks(Datum *rp,int n,Datum **argpp) {
	...
	// For all hooks...
	do {
		if(hrp != hooktab && sepline(pagewidth,&rpt) != 0)
			goto LibFail;
		if(dputf(&rpt,"\n%-*s%*s%-*s",(int) hcols[0].minwidth,hrp->h_name,spacing,"",(int) hcols[1].minwidth,
		 hrp->h_bufp == NULL ? "" : hrp->h_bufp->b_bname + 1) != 0)
			goto LibFail;
		indent = spacing;
		strn = hrp->h_narg;
		strm = hrp->h_margs;
	...
	if((univp->p_type & (PtrAlias_F | PtrMacro_C)) || (univp->p_type == PtrAlias_M &&
	 (univp->u.p_aliasp->a_targ.u.p_bufp->b_flags & BFConstrain))) {
		(void) rcset(Failure,RCTermAttr,"%s%s '%c%c%s%c%c'%s",text314,univp->p_type == PtrMacro_C ? text414 :
		 ...
		 text127,AttrSpecBegin,AttrBoldOn,univp->p_type == PtrMacro_C ? univp->u.p_bufp->b_bname + 1 :
			// "alias"
		  univp->u.p_aliasp->a_name,AttrSpecBegin,AttrBoldOff,text415);
							// " interactively"
		return false;
	 	}
	return true;
...
int aliasCFM(Datum *rp,int n,Datum **argpp) {
	...
	if(si.opflags & OpScript) {
		...
		// Get the command, function, or macro name.
		sprintf(wkbuf,"%s'%c%c%s%c%c' %s %s",text215,AttrSpecBegin,AttrBoldOn,dnamep->d_str,AttrSpecBegin,AttrBoldOff,
						// "Create alias "
		 text325,text313);
