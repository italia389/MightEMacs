...
static int ausage(DStrFab *sfp,int stage) {
	...
	if(stage <= 0) {
		...
		if(bfind("Debug",BS_Create,0,&bufp,&created) != Success || (!created && bclear(bufp,BC_IgnChgd) != Success))
			return rc.status;
...
bool charval(Datum *datp) {

	return isval(datp->d_type == dat_int && datp->u.d_int >= 0 && datp->u.d_int <= 0xFF,text288);
			// "Character expected"
	}
		...
		case s_lbrkt:
			...
			for(;;) {
				...
				if(ge_andor(np) != Success)
					return rc.status;
				if((si.opflags & OpEval) && apush(aryp,np->en_rp,true) != 0)
					goto LibFail;
...
static int fcall(ENode *np,bool needrparen,bool *foundp) {
	...
	if(execfind(np->en_rp->d_str,OpQuery,PtrCFAM,&univ)) {
		...
		switch(univ.p_type) {
			...
			case PtrAlias_F:
			case PtrAlias_M:
				fprintf(logfile,"%s' ALIAS",univ.u.p_aliasp->a_name);
				break;
			case PtrMacro_C:
			case PtrMacro_O:
				fprintf(logfile,"%s' MACRO",univ.u.p_bufp->b_bname);
				break;
			case PtrCmd:
			case PtrFunc:
		...
		// Set to -1 if unknown.
		short minArgs = (univ.p_type & PtrMacro) ? univ.u.p_bufp->b_mip->mi_minArgs :
		...
		if(univ.p_type & PtrMacro)
			maxArgs = (univ.u.p_bufp->b_mip->mi_maxArgs < 0) ? SHRT_MAX : univ.u.p_bufp->b_mip->mi_maxArgs;
		...
		if(((univ.p_type & (PtrCmd | PtrFunc)) && (univ.u.p_cfp->cf_aflags & CFSpecArgs)) || ((si.opflags & OpEval) &&
		 ((univ.p_type & PtrMacro) || np->en_narg != 0 || !(univ.u.p_cfp->cf_aflags & CFNCount)))) {
			...
			if(univ.p_type & PtrMacro) {
				...
				(void) debug_execbuf(np->en_rp,(int) np->en_narg,univ.u.p_bufp,NULL,
				 needrparen ? ArgFirst | SRun_Parens : ArgFirst,"fcall");
#else
				(void) execbuf(np->en_rp,(int) np->en_narg,univ.u.p_bufp,NULL,
...
static int concat(ENode *np1,ENode *np2,bool clone) {
	...
	if(np1->en_rp->d_type == dat_blobRef) {
		...
		if(clone && aryclone(np1->en_rp,np1->en_rp,0) != Success)
			return rc.status;
		aryp1 = awptr(np1->en_rp)->aw_aryp;

		// If node2 is an array, expand it.
		if(np2->en_rp->d_type == dat_blobRef) {
			if(agraph(aryp1,awptr(np2->en_rp)->aw_aryp) == NULL)
				goto LibFail;
			}
		else {
			if(apush(aryp1,np2->en_rp,false) != 0)
				goto LibFail;
			duntrk(np2->en_rp);
			}
		}
...
static int aequal(Datum *datp1,Datum *datp2,bool *resultp) {
	...
	Array *aryp1 = awp1->aw_aryp;
	Array *aryp2 = awp2->aw_aryp;
...
static int valarg(Datum *datp,uint aflags) {
	...
	if((aflags & (ArgInt1 | ArgMay)) == ArgInt1)				// 'int' flag set without "may be"?
		...
		if(strval(datp) && (aflags & ArgNotNull1) && disnull(datp))	// Yes, must be string.  Check if null string
			goto NotNull;						// is an error.
		}
	else if((!(aflags & ArgArray1) && datp->d_type == dat_blobRef) ||	// No required types.  Check if found type
	 (!(aflags & ArgBool1) && (datp->d_type & DBoolMask)) ||			// is allowed.
	 (!(aflags & ArgNIS1) && (
	 (!(aflags & ArgNil1) && datp->d_type == dat_nil) ||
	 (!(aflags & ArgInt1) && datp->d_type == dat_int))))
		return rcset(Failure,0,text329,dtype(datp,false));
				// "Unexpected %s argument"
	else if((datp->d_type & DStrMask) && (aflags & ArgNotNull1) &&		// Null string error?
	 disnull(datp))
NotNull:
...
int funcarg(Datum *rp,uint aflags) {
	...
	return (si.opflags & OpEval) ? valarg(rp,aflags) : rc.status;
	}
...
// or NotFound if no arguments left.  If ArgReq not set, it is assumed the next argument is optional.
		...
		if(*elppp != NULL) {
			...
			*elppp = NULL;
			...
			if(*aflagsp & ArgFirst) {
				...
				else if(!havesym(s_any,false))
					return NotFound;
				}
			else if(!havesym(s_comma,false))
				return NotFound;

			// Argument required or optional and comma found... get next argument.
