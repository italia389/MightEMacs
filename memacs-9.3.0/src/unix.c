...
// 0x08.  Hence, we can set the keytab table accordingly and get consistent extended key values for those two keys.
	...
	for(;;) {
		if((kcode = wgetch(wp)) != ERR) {
			...
			if(c != (FKey | '?'))
				break;
#else
			if(!((c = kcode) & ~0xFF) || (c = keytab[kcode - KEY_MIN]) != (FKey | '?'))
				break;
#endif
			}
		else if(errno != 0)
...
int ttopen(void) {
	...
	if(has_colors()) {
		...
		if(term.maxColor >= 7 && term.maxPair >= 63 && (freePairs = term.maxPair - ReservedPairs) >= (8 * 4)) {
			...
			cip = ci;
			do {
				*cip->termcp = (cip->desiredColor <= term.maxColor) ? cip->desiredColor : cip->defaultColor;
				} while((++cip)->termcp != NULL);
...
// key_dc		kdch1		kD	delete-character key
	...
	if(kbs != NULL && strcmp(kbs,"\x7F") == 0)
		keytab[KEY_BACKSPACE - KEY_MIN] = Ctrl | '?';
#endif
	// Finish up.
