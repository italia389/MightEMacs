...
int rcycle(Ring *ringp,int n,bool msg) {
	...
	if(ringp->r_size == 0)
		(void) rcset(Failure,0,text202,ringp->r_rname);
				// "%s ring is empty"
		...
		if(msg)
			(void) rcset(Success,0,text42,ringp->r_rname);
				// "%s ring cycled"
		}
	return rc.status;
	}
...
int rdelete(Ring *ringp,int n) {
	...
	if(ringp->r_size == 0)
		return rcset(Failure,0,text202,ringp->r_rname);
				// "%s ring is empty"
	if(n == INT_MIN)
		n = 1;
	else if(n == 0)
	...
	if((n = rtrim(ringp,n)) == sz0)
		(void) rcset(Success,0,text228,ringp->r_rname);
				// "%s ring cleared"
	else
NukeMsg:
		(void) rcset(Success,0,text357,ringp->r_rname,ringp->r_ename,n > 1 ? "s" : "");
				// "%s%s%s deleted"
...
// Insert, overwrite, or replace a string from a string buffer (srcp != NULL) or the kill or undelete buffer (srcp == NULL) at
...
int iorstr(char *srcp,int n,ushort style,bool kill) {
	long size;
	int reps;
	...
	if(srcp == NULL) {
		...
		if(!kill) {
			...
			return rcset(Failure,0,text202,kring.r_rname);
					// "%s ring is empty"
		else if(n >= 0)
			rep = kring.r_entryp;
		else {
	...
	while(reps-- > 0) {
		size = 0;
		str = (srcp == NULL) ? rep->re_data.d_str : srcp;
		do {
...
int showRing(Datum *rp,int n,Ring *ringp) {
	...
	sprintf(wkbuf,text305,ringp->r_rname);
			// "%sRing"
	...
	// Write headers.
	sprintf(wkbuf,"%s %s",ringp->r_rname,text305 + 2);
					// "%sRing"
