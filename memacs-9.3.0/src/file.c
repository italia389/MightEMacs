...
// Initialize FileInfo object for input.
int rinit(int fd) {
	fi.fd = fd;
...
static int ffwopen(Buffer *bufp,char *fn,short mode) {
	int fd;
...
int insline(char *src,int len,bool hasDelim,Buffer *bufp,Dot *dotp) {
	Line *lnp;
	int size;
...
// Read data from a buffer or file and insert at point in target buffer.  DataInsert object contains additional input and output
// parameters.  If srcbufp is not NULL, lines are "read" from that buffer; otherwise, they are read from current (opened) input
// file, status parameter is set to result, and finalDelim parameter is set to true if last line read had a delimiter;
// otherwise, false.  If targdotp parameter is not NULL, target buffer is not marked as changed after lines are inserted; if
// targdotp parameter is NULL, dot from current window is used and (1), inserted lines are marked as a region; (2), target
// buffer is marked as changed if any lines were inserted; and (3), point is moved to beginning of region if n is zero.  In all
// cases, "n" parameter is updated to number of lines inserted.  Return status.
int idata(int n,Buffer *srcbufp,DataInsert *dip) {
	Dot *dotp,dot0;
	Line *dotlnp0;
	int wpos;
	int len;
	...
	if(srcbufp != NULL) {
		...
		do {
			...
			if(insline(buflnp->l_text,len = buflnp->l_used,hasDelim,dip->targbufp,dotp) != Success)
				return rc.status;
			++dip->lineCt;
		...
		while((dip->status = ffgetline(&len,&hasDelim)) == Success) {		// and read the file.
			if(insline(fi.lbuf,len,hasDelim,dip->targbufp,dotp) != Success)
				return rc.status;
			++dip->lineCt;
			}
	...
	if(!hasDelim) {
		if(dip->targbufp->b_nwind > 0)
			fixwface(dot0.off,len,dotlnp0,dotp->lnp);
		fixbface(dip->targbufp,dot0.off,len,dotlnp0,dotp->lnp);
		}

	// All lines inserted.  Set mark RegMark to first inserted line and restore original buffer framing, if applicable.
	if(dip->targdotp == NULL) {
		dip->targbufp->b_mroot.mk_dot.lnp = (dot0.lnp == NULL) ? dip->targbufp->b_lnp : dot0.lnp->l_nextp;
		dip->targbufp->b_mroot.mk_dot.off = dot0.off;
		dip->targbufp->b_mroot.mk_rfrow = wpos;
	...
	if(dip->lineCt > 0) {
		bchange(dip->targbufp,WFHard);
		if(dip->targbufp == si.curbp && n != 0) {
			si.curwp->w_rfrow = 0;
...
int iostat(DStrFab *sfp,ushort flags,Datum *bknamep,int status,char *fname,char *lcmsg,uint lineCt) {
	char *str;
...
int readFP(Datum *rp,int n,char *fname,ushort flags) {
	...
	if(n == INT_MIN)
		...
		if(flags & RWScratch) {						// if scratch buffer requested...
			if(bscratch(&bufp) != Success)				// get one...
				return rc.status;
			}							// otherwise, create a buffer.
		else if(bfind(fname,BS_Create | BS_Hook | BS_Derive | BS_Force,0,&bufp,NULL) != Success)
			return rc.status;
		}
	if(!(flags & RWScratch))
		flags |= RWKeep;
...
int insertFile(Datum *rp,int n,Datum **argpp) {
	...
	if(ffropen(rp->d_str,true) == Success) {
		DStrFab sf;
		DataInsert di = {
			si.curbp,	// Target buffer.
			NULL,		// Target line.
		...
		if(idata(n,NULL,&di) == Success) {
			if(dopentrk(&sf) != 0)
				return librcset(Failure);
			(void) iostat(&sf,di.finalDelim ? 0 : IOS_NoDelim,NULL,di.status,rp->d_str,text154,di.lineCt);
												// "Inserted"
			}
		}
	dclear(rp);
...
int fvfile(Datum *rp,int n,bool view) {
	...
	if(bfind(rp->d_str,BS_Create | BS_Hook | BS_Derive | BS_Force,0,&bufp,&created) != Success)
		return rc.status;
...
int readprep(Buffer *bufp,ushort flags) {

	if(bclear(bufp,flags) == Success) {
...
int readin(Buffer *bufp,char *fname,ushort flags) {
	...
	DataInsert di = {
		bufp,			// Target buffer.
		&bufp->b_face.wf_dot,	// Target line.
		text139			// "Reading data..."
		};

	if(readprep(bufp,0) != Success)
	...
	if(flags & RWStats) {
		...
		if(dopentrk(&sf) != 0)
			return librcset(Failure);
		if(iostat(&sf,di.finalDelim ? 0 : IOS_NoDelim,NULL,di.status,fname,text140,di.lineCt) != Success)
										// "Read"
			return rc.status;
		}
Retn:
	...
	if(!(flags & RWKeep))
		clfname(bufp);
	else if(di.status >= Success && !(flags & RWNoHooks) && !(bufp->b_flags & BFMacro))
...
int awfile(Datum *rp,int n,char *prmt,short mode) {
	...
	if(mode == 'w' && setfname(si.curbp,rp->d_str) > FatalError)
		if(!(si.curbp->b_flags & BFMacro) && (n == INT_MIN || n > 0))
			(void) brename(NULL,1,si.curbp);
	dclear(rp);
...
int writefd(Buffer *bufp,uint *lcp) {
	Line *lnp,*lnpz;
	uint lineCt;
...
int writeout(Buffer *bufp,char *fn,short mode,ushort flags) {
	...
	if(status != Success || (status = writefd(bufp,&lineCt)) != Success) {
		if(sflag)
...
// Set the filename associated with a buffer and call the filename hook if appropriate.  If default n, use current buffer for
// target buffer and first argument for filename; otherwise, use first argument for target buffer and second argument for
// filename.  If buffer name is nil, use current buffer.  If the new filename is nil or null, set the filename to null;
// otherwise, if n > 0 or is the default, change the buffer name to match the filename.  rp is set to a two-element array
// containing new buffer name and new filename.  Return status.
int setBufFile(Datum *rp,int n,Datum **argpp) {
	...
	else if(bcomplete(rp,text151,(bufp = bdefault()) != NULL ? bufp->b_bname : NULL,OpDelete,&bufp,NULL) != Success
	 || bufp == NULL)
			// "Set filename in"
		return rc.status;
	...
	if(bufp->b_fname != NULL && !(bufp->b_flags & BFMacro) && (n == INT_MIN || n > 0))
		if(brename(NULL,1,bufp) != Success)
			return rc.status;
