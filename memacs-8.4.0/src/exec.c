...
int setHook(Value *rp,int n) {
	...
	if(vnew(&vp,false) != 0)
		return vrcset();
	if(getarg(vp,text318,NULL,RTNKEY,0,ARG_FIRST | ARG_NOTNULL | ARG_STR) != SUCCESS ||
...
int showHooks(Value *rp,int n) {
	...
	// For all hooks...
	do {
		if(vputf(&rpt,"\n%-13s%s",hrp->h_name,hrp->h_cfab.p_type == PTRNUL ? "" : hrp->h_cfab.p_type == PTRMACRO ?
...
int eval(Value *rp,int n) {
	...
	else if(terminp(vp,": ",NULL,RTNKEY,0,n == INT_MIN ? ARG_NOTNULL : ARG_NOTNULL | TERM_EVAL) != SUCCESS ||
	 vistfn(vp,VNIL))
		return rc.status;
...
static enum e_sym dfind(char **clpp,int len) {
	...
	if(is_lower(*clp)) {
		...
		stplcpy(wkbuf,clp,(uint) len < (sizeof(wkbuf) - 1) ? (uint) len + 1 : sizeof(wkbuf));
...
static int execbuf(Value *rp,Buffer *bufp,uint flags) {
	...
	while(lnp != hlp) {
		...
		len = lused(lnp);
#if MMDEBUG & (DEBUG_SCRIPT | DEBUG_PPBUF)
		fprintf(logfile,"%.8X [level %u.%u] >> %.*s\n",(uint) lnp,myinstance,elevp->level,len,eline); //fflush(logfile);
#endif
...
doloop:
					...
					if(elevp->live) {
						...
						// continue.
						if(dirnum == kw_loop || tobool(vp) == go) {
							if(nextlevel(&elevp) != SUCCESS)
								goto rcexit;
							elevp->live = true;
...
int dofile(Value *rp,char *fname,int n,uint flags) {
	...
	uint mflag = modetab[MDR_GLOBAL].flags & MDMSG;			// save current message flag...
	modetab[MDR_GLOBAL].flags &= ~MDMSG;				// clear it...
	(void) readin(bufp,fname,true);					// read in the file...
	modetab[MDR_GLOBAL].flags |= mflag;				// restore message flag...
	if(rc.status == SUCCESS)
	...
	if(rc.status == SUCCESS && bufp->b_nwind == 0)
		(void) bdelete(bufp,CLBIGNCHGD,NULL);
	return rc.status;
	}
...
int xeqFile(Value *rp,int n) {
	...
	// Get the filename.
	if(getarg(rp,text129,NULL,RTNKEY,MaxPathname,ARG_FIRST | ARG_STR | ARG_NOTNULL | TERM_C_FNAME) != SUCCESS ||
