...
// (SUCCESS) if unique name matched; otherwise, return NOTFOUND (bypassing rcset()).
static int comp_mode(char *namep,uint *cposp,uint selector) {
	...
	for(;;) {
		...
		do {
			// Is this a match?
			if(*cposp > 0 && strncasecmp(namep,msp->mlname,*cposp) != 0)
				goto nextentry;

			// A match.  If this is the first match, simply record it.
			if(ematchp == NULL) {
				ematchp = msp;
				namep[*cposp] = msp->mlname[*cposp];	// Save next char.
				}
			else if(namep[*cposp] != msp->mlname[*cposp])
...
// status.
static int clist_mode(char *namep,uint cpos,Buffer **bufpp,uint selector) {
	...
	do {
		// Add name to the buffer if it's a match...
		if(cpos == 0 || strncasecmp(namep,msp->mlname,cpos) == 0)
...
static int terminp1(Value *rp,char *defvalp,uint terminator,uint terminator2,uint flags) {
	ushort ek;
...
// "return"; otherwise, if terminator is "return" or "newline", it's partner is also accepted as a terminator; otherwise,
// "return" is converted to "newline".  If maxlen is zero, it is set to the maximum.
int terminp(Value *rp,char *prmtp,char *defvalp,uint terminator,uint maxlen,uint flags) {
	...
	terminator2 = (terminator == RTNKEY) ? ALTRTNKEY : (terminator == ALTRTNKEY) ? RTNKEY : -1;
	...
	if(flags & TERM_ONEKEY)
		return terminp1(rp,defvalp,terminator,terminator2,flags);
			...
			// Check completion special keys.
				...
				if(ek == (CTRL | 'I')) {		// Tab ... attempt a completion.
					switch(flags & TERM_C_MASK) {
						...
						case TERM_C_BMODE:
						case TERM_C_GMODE:
							status = comp_mode(inpbuf,&cpos,flags);
							break;
...
clist:
					...
					switch(flags & TERM_C_MASK) {
						...
						case TERM_C_BMODE:
						case TERM_C_GMODE:
							status = clist_mode(inpbuf,cpos,&bufp,flags);
							break;
						default:	// TERM_C_VAR or TERM_C_SVAR
					...
					if(status != SUCCESS || bdelete(bufp,0,NULL) != SUCCESS)
						return rc.status;
					inpbuf[cpos] = '\0';
...
int mlyesno(char *prmtp,bool *resultp) {
	...
	for(;;) {
		if(terminp(vp,prompt.sl_vp->v_strp,"n",RTNKEY,0,TERM_ONEKEY) != SUCCESS)
			return rc.status;
		if(*vp->v_strp ==
#if FRENCH
		 'o') {
...
int getcfam(char *prmtp,uint selector,CFABPtr *cfabp,char *emsg0,char *emsg1) {
	...
	if(vnew(&vp,false) != 0)
		return vrcset();
	if(getarg(vp,prmtp,NULL,RTNKEY,NTERMINP,ARG_FIRST | ARG_STR | TERM_C_CFAM | selector) != SUCCESS)
		return rc.status;
...
int bcomplete(Value *rp,char *prmtp,char *defname,uint op,Buffer **bufpp,bool *createdp) {
	...
	sprintf(pbuf,"%s %s",prmtp,text83);
				// "buffer"
	if(getarg(rp,pbuf,defname,RTNKEY,NBNAME,ARG_FIRST | ARG_STR | TERM_C_BUFFER) != SUCCESS)
		return rc.status;
	if(!vistfn(rp,VNIL)) {
