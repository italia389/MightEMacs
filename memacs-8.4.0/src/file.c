...
int gtfilename(Value *rp,char *prmtp,uint flags) {
	...
	return getarg(rp,prmtp,curbp->b_fname,RTNKEY,MaxPathname,ARG_FIRST | ARG_STR | ARG_NOTNULL | TERM_C_FNAME | flags);
	}
...
int fileout(Value *rp,int n,char *prmtp,int mode) {
	...
	if(mode == 'w') {
		...
		if(vnew(&vsinkp,false) != 0)
			return vrcset();
		if(setfname(curbp,rp->v_strp) != SUCCESS || ((n == INT_MIN || n > 0) && setBufName(vsinkp,1) != SUCCESS))
			return rc.status;
		}
...
int writeout(Buffer *bufp,char *fn,int mode) {
	...
	if(sflag) {
		...
		status = ffwopen(bufp,tnamep->v_strp,mode);
		}
	else
...
int setBufFile(Value *rp,int n) {
	...
	// Get filename.
	if(getarg(rp,text151,NULL,RTNKEY,MaxPathname,ARG_FIRST | ARG_STR | TERM_C_FNAME | TERM_C_NOAUTO) != SUCCESS ||
