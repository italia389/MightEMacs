...
static int buildmsg(Value **vpp,char *prefix) {
	...
	if(vopen(&msg,*vpp,false) != 0 || (prefix != NULL && vputs(prefix,&msg) != 0))
		return vrcset();
	if(!vvoid(vp))
		if((!vempty(&msg) && vputs(": ",&msg) != 0) || vputs(vp->v_strp,&msg) != 0)
			return vrcset();
...
static int startbuf(Buffer *bufp,int stdinflag,bool readFirst) {
	...
	if(stdinflag == 0 && (bufp = bsrch(buffer1,NULL)) != NULL && lforw(bufp->b_hdrlnp) == bufp->b_hdrlnp &&
	 bufp->b_nwind == 0) {
		...
		if(bdelete(bufp,0,NULL) != SUCCESS)
			return rc.status;
		}
	...
	while(--argc > 0) {
		...
		do {
			if(strcmp(*argv,*strpp) == 0)
				return helpExit((enum h_type)(strpp - swlist),binname);
			} while(*++strpp != NULL);
		}
...
decr5:
		...
		if(rc.status == SUCCESS) {
			...
			if(hooktab[HKPREKEY].h_cfab.p_type == PTRNUL || !tobool(&rtn))
