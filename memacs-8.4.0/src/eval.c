...
bool tobool(Value *vp) {
	...
	return !vistfn(vp,VFALSE) && !vistfn(vp,VNIL);
	}
...
static int resub(Value *rp,int n,Value *sp,char *spatp,char *rpatp,ushort flags) {
	...
	for(;;) {
		...
		if(offset >= 0) {
			...
			if(++loopcount > 2 && slp->len == 0 && strlen(sp->v_strp + scanoff) == lastscanlen) {
			...
			if(offset > 0 && vputfs(sp->v_strp,offset,&dest) != 0) {
				(void) vrcset();
				break;
				}
			...
			if(((len = strlen(sp->v_strp + offset + slp->len)) > 0 &&
			 vputfs(sp->v_strp + offset + slp->len,len,&dest) != 0) || vclose(&dest) != 0) {
				(void) vrcset();
				break;
				}
...
static int varfunc(Value *rp,enum cfid fid,char *fname) {
	int status;
	VDesc vd;
	...
	if(opflags & OPEVAL) {
		...
		if(tostr(delimp,TSNOBOOL) != SUCCESS)
			return rc.status;
		if(vistfn(delimp,VNIL))
			vnull(delimp);
		else if(*delimp->v_strp == ' ')
			spacedlm = true;
		else if((fid == cf_shift || fid == cf_pop) && strlen(delimp->v_strp) > 1)
	...
	switch(fid) {
		case cf_pop:					// Get last token from old var value into rp.
			...
			if(*(newvarvalp = oldvarvalp->v_strp) == '\0')	// Null variable value?
				...
				status = rparsetok(rp,&strp1,newvarvalp,spacedlm ? -1 : *delimp->v_strp);
				}
			goto check;
		case cf_shift:
			newvarvalp = oldvarvalp->v_strp;
			status = parsetok(rp,&newvarvalp,spacedlm ? -1 : *delimp->v_strp);
check:
			newvarp = &newvar;
			...
			// instead of nil.
			if(fid == cf_shift) {
				if(!spacedlm && newvarvalp[0] == '\0' && newvarvalp[-1] == *delimp->v_strp)
					nulltok = true;
				}
			else if(strp1 <= newvarvalp) {		// Just popped last token?
				if(strp1 == newvarvalp && !spacedlm)	// Yes, check for leading delimiter.
					nulltok = true;
				*newvarvalp = '\0';
				}
			else
...
// point.  If n < 0, do one repetition and process all newline characters literally (don't create a new line).  Return text in
// rp and return status.
...
bebuf:
		...
		case cf_getKey:
			...
			if(n <= 0)
				vsetint(ektoc(ek),rp);
			else if(vsetstr(ektos(ek,keybuf),rp) != 0)
				(void) vrcset();
			}
			break;
		case cf_gotoFence:
		...
		case cf_includeQ:
			...
			if(strlen(argp[1]->v_strp) > 1)
				return rcset(FAILURE,0,text251,text288,argp[1]->v_strp,1);
					// "%s delimiter '%s' cannot be more than %d character(s)","Function"
			i = false;
			if(!vvoid(argp[0]) && !visnull(argp[2])) {
				char *strp1,*strp2;
				size_t len = strlen(argp[2]->v_strp);
				short dlm = *argp[1]->v_strp == ' ' ? -1 : *argp[1]->v_strp;
				strp = argp[0]->v_strp;
				for(;;) {
					if((strp1 = strstr(strp,argp[2]->v_strp)) == NULL)
						break;

					// Match found.  Check boundaries.
					if((i = (strp1 == argp[0]->v_strp || strp1[-1] == dlm ||
					 (dlm < 0 && (strp1[-1] == ' ' || strp1[-1] == '\t'))) &&
					 (*(strp2 = strp1 + len) == '\0' || *strp2 == dlm ||
					 (dlm < 0 && (*strp2 == ' ' || *strp2 == '\t')))))
						break;

					// Match not at delimiter boundary, keep scanning.
					if(*(strp = strp1 + len) == '\0')
						break;
					}
				}
...
kdctxt:
		...
		case cf_shift:
		case cf_unshift:
			(void) varfunc(rp,fnum,cfp->cf_name);
			break;
		case cf_prevBuf:
...
repl:
		...
		case cf_setWrapCol:
			...
			break;
		case cf_shQuote:
			if(tostr(argp[0],TSNOBOOL) == SUCCESS && vshquote(rp,argp[0]->v_strp) != 0)
				(void) vrcset();
			break;
		case cf_shrinkWind:
