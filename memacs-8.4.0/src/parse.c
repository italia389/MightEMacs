...
// Return true if a value object is a true, false, nil, or Boolean literal (val_xxx); otherwise, false.
bool vistfn(Value *vp,int tfn) {

	return !(vp->v_type & VALSMASK) ? false :
	 (tfn == VANY) ? strcmp(vp->v_strp,val_nil) == 0 ||
	 strcmp(vp->v_strp,val_false) == 0 || strcmp(vp->v_strp,val_true) == 0 :
	 (tfn == VBOOL) ? strcmp(vp->v_strp,val_false) == 0 || strcmp(vp->v_strp,val_true) == 0 :
	...
	if(vp->v_type == VALINT) {
		...
		if(vistfn(vp,VNIL)) {
			if(flags & TSNULL) {
				...
				vnull(vp);
#if MMDEBUG & DEBUG_EXPR
				fputs(" to null string ",logfile);
				dumpval(vp);
				fprintf(logfile,", flags %.8x\n",flags);
#endif
				}
			else if(flags & TSNOBOOLN)
				(void) rcset(FAILURE,0,text358,text359);
					// "Illegal use of %s value","nil"
			}
		else if((flags & (TSNOBOOL | TSNOBOOLN)) && vistfn(vp,VBOOL))
			(void) rcset(FAILURE,0,text358,text360);
