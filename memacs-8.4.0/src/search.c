		...
		if(mcp->mc_type & MCE_CLOSURE) {
			...
			case MCE_EOSALT:
				if(sdot.type == BUFDOT) {
					...
					switch(mcp->mc_type) {
					case MCE_BOL:
						if(sdp->strp != sdp->strp0 && sdp->strp[-1] != '\n')
							return false;
						break;
					case MCE_EOL:
						if(*sdp->strp != '\0' && *sdp->strp != '\n')
							return false;
						break;
					case MCE_BOS:
...
int showRegexp(Value *rp,int n) {
	...
	if(vputf(&rpt,"Match flags: %.4x\n\n",srch.m.flags) != 0)
		return vrcset();
	mkpat(patbuf,&srch.m);
	do {
		// Construct the search header lines.
		if(mcobjp->pat == srch.m.bmcpat && vputs("\n\n",&rpt) != 0)
			return vrcset();
...
grp:
			...
			if(strp != NULL && (vputs(wkbuf,&rpt) != 0 || vputc('\n',&rpt) != 0))
				return vrcset();
	...
	if(vputf(&rpt,"\n\n%s %s /",text310,text311) != 0 || vstrlit(&rpt,srch.m.rpat,0) != 0 ||
	 vputs("/\n",&rpt) != 0)
				// "SEARCH","PATTERN"
		return vrcset();
		...
		switch(rmcp->mc_type) {
			...
			case MCE_LITSTRING:
				if(vputs(wkbuf,&rpt) != 0 || vputf(&rpt,"%-14s'","String") != 0 ||
				 vstrlit(&rpt,rmcp->u.rstr,0) != 0 || vputs("'\n",&rpt) != 0)
					return vrcset();
				strp = NULL;
				break;
			case MCE_GROUP:
