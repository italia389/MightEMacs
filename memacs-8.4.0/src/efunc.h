...
extern int bdelete(Buffer *bufp,uint flags,bool *nukedp);
...
extern int bhook(Value *rp,bool exitHook);
...
extern int fileout(Value *rp,int n,char *prmtp,int mode);
...
extern int getpath(Value *pathp,int n,char *fname);
...
extern int goline(Value *vp,int n,int line);
...
extern void mdelete(Buffer *bufp,uint flags);
...
extern int showModes(Value *rp,int n);
...
extern int stoek(char *keylitp,ushort *resultp);
