...
#define FLIT_getKey		"Get one keystroke (or key sequence if n > 1, ASCII (int) character if n <= 0, default 1)"
...
#define FLIT_insert	"Insert concatenated args at cursor n times (without moving cursor if n == 0, literal newline if n < 0)"
...
#define FLIT_pop		"Remove last token from variable var using delimiter dlm (' ' for white space) and return it"
...
#define FLIT_shift		"Remove next token from variable var using delimiter dlm (' ' for white space) and return it"
