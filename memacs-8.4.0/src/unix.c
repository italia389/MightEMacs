...
int changedir(Value *rp,int n) {
	...
	// Get directory name.
	if(getarg(rp,text277,NULL,RTNKEY,MaxPathname,ARG_FIRST | ARG_STR | ARG_NOTNULL | TERM_C_FNAME) != SUCCESS ||
...
static int getcmd(Value **cfpp,char *prmtp) {
	...
	else if(terminp(*cfpp,prmtp,NULL,RTNKEY,0,ARG_NOTNULL) == SUCCESS && !vistfn(*cfpp,VNIL))
		(void) strval(*cfpp);
