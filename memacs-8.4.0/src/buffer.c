...
int bhook(Value *rp,bool exitHook) {
...
// and remove the name from the CFAM list if it's a macro.  Set *nukedp (if nukedp not NULL) to true if buffer was deleted.
// Return status.
int bdelete(Buffer *bufp,uint flags,bool *nukedp) {
	CFABPtr cfab;
	KeyDesc *kdp;
	bool yep;
...
//	n > 1		Force-delete all other buffers with confirmation.
	...
	if(n >= 0) {
		...
		do {
			...
			if(mlprintf(MLHOME | MLWRAP,text367,bufp->b_bname) != SUCCESS)
				...
				return rc.status;
			cpause(50);
			if(bdelete(bufp,n > 1 ? CLBIGNCHGD : n == 1 ? CLBMULTI : 0,&nuked) != SUCCESS)
				break;
			if(nuked)
				++count;
...
NukeIt:
		// Nuke it.
		if(bdelete(bufp,n < 0 ? CLBIGNCHGD : 0,NULL) != SUCCESS)
			break;
...
ask:
	...
	if(getarg(bnamep,prmtp,NULL,RTNKEY,NBNAME,ARG_FIRST) != SUCCESS || (!(opflags & OPSCRIPT) && vistfn(bnamep,VNIL)) ||
