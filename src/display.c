...
EWindow *windDispBuf(Buffer *pBuf, bool skipCur) {
	...
	do {
		if(pWind->pBuf == pBuf && (pWind != sess.pCurWind || !skipCur))
			return pWind;
...
static int mlbegin(ushort flags) {
	...
	if(((flags & MLHome) || term.msgLineCol < 0 || term.msgLineCol >= term.cols) && mlerase() != Success)
		return sess.rtn.status;
...
static int mlend(ushort flags) {
	...
	if(flags & MLWrap)
		mlputc(flags | MLRaw, ']');
	return (flags & MLFlush) ? mlflush() : sess.rtn.status;
	}
...
// its buffer marks.
void faceInit(WindFace *pWindFace, Line *pLine, Buffer *pBuf) {

	pWindFace->pTopLine = pWindFace->point.pLine = pLine;
	pWindFace->point.offset = pWindFace->firstCol = 0;
...
static int getNum(const char *prompt, int screen, int *pNum) {
	...
	if(sess.opFlags & OpScript) {
		...
		sprintf(workBuf, "%s %s (1-%d)%s", prompt, screen ? text380 : text331,
							// "screen", "window"
		 screen ? scrnCount() : getWindCount(sess.pCurScrn, NULL), screen > 0 ? text445 : "");
									// " or 0 for new"
		if(getNArg(pDatum, workBuf) != Success || pDatum->type == dat_nil)
			return Cancelled;
		}
...
int gotoWind(Datum *pRtnVal, int n, ushort flags) {
	...
	if(n != 0 || !(flags & SWB_Repeat)) {
		EWindow *pWind;
		int windNum, count;
		int windCount = getWindCount(sess.pCurScrn, &windNum);
...
bool pointInWind(void) {
	...
	return lineInWind(sess.pCurWind, sess.pCurWind->face.point.pLine);
	}
...
int moveWindUp(Datum *pRtnVal, int n, Datum **args) {
	...
	Line *pLine = pWindFace->pTopLine;
	int i = sess.pCurWind->rows / 2;
	while(i-- > 0 && pLine->next != NULL)
		pLine = pLine->next;
	pWindFace->point.pLine = pLine;
...
int onlyWind(Datum *pRtnVal, int n, Datum **args) {
	...
	while(sess.windHead != sess.pCurWind) {
		pWind = sess.windHead;
		sess.pCurScrn->windHead = sess.windHead = pWind->next;
	...
	while(sess.pCurWind->next != NULL) {
		pWind = sess.pCurWind->next;
		sess.pCurWind->next = pWind->next;
		--pWind->pBuf->windCount;
		windFaceToBufFace(pWind, pWind->pBuf);
		free((void *) pWind);
		}
	...
	windNewTop(sess.pCurWind, NULL, -sess.pCurWind->topRow);
	sess.pCurWind->topRow = 0;
	sess.pCurWind->rows = term.rows - 2;
...
static int delJoinWind(int n, Datum **args, bool join) {
	...
	if(sess.windHead->next->next != NULL &&
	 ((sess.pCurWind == sess.windHead && !joinDown && force) || (sess.pCurWind->next == NULL && joinDown && force))) {
		int rows;

		// Current window is top or bottom and need to transfer lines to window at opposite end of screen.
		if((sess.pCurWind == sess.windHead) != join) {
			...
			rows = -(sess.pCurWind->rows + 1);			// Top row adjustment for all windows.
			sess.pCurScrn->windHead = sess.windHead = sess.pCurWind->next;	// Remove current window from list.
			}
		else {
			...
			rows = sess.pCurWind->rows + 1;
			windNextIs(sess.pCurWind)->next = NULL;
		...
		pWind = windNextIs(sess.pCurWind);
		if((pWind == NULL || (joinDown && sess.pCurWind->next != NULL)) != join) {	// Next window down.
			...
			targWind = sess.pCurWind->next;
			targWind->topRow = sess.pCurWind->topRow;
			if(pWind == NULL)
				sess.pCurScrn->windHead = sess.windHead = targWind;
			else
				pWind->next = targWind;
			windNewTop(targWind, NULL, -(sess.pCurWind->rows + 1));
			}
		else {									// Next window up.
			...
			targWind = pWind;
			pWind->next = sess.pCurWind->next;
			}
		targWind->rows += sess.pCurWind->rows + 1;
		}
...
// the upper window is reduced by abs(n) lines; if n > 0, the upper window is set to n lines.  *ppWind is set to the new window
...
int wsplit(int n, EWindow **ppWind) {
	...
	bool defIsUpper;			// Default is upper window?
	WindFace *pWindFace = &sess.pCurWind->face;

	// Make sure we have enough space and can obtain a unique id.  If so, create a new window.
	if(sess.pCurWind->rows < 3)
		return rsset(Failure, 0, text293, sess.pCurWind->rows);
	...
	pointRow = 0;
	for(pLine = pWindFace->pTopLine; pLine != pWindFace->point.pLine; pLine = pLine->next)
		++pointRow;

	// Update some settings.
	++sess.pCurBuf->windCount;			// Now displayed twice (or more).
	pWind->pBuf = sess.pCurBuf;
	pWind->face = *pWindFace;			// For now.
	pWind->reframeRow = 0;
	pWind->flags = (WFHard | WFMode);
	pWind->id = id;

	// Calculate new window sizes.
	upperRows = (sess.pCurWind->rows - 1) / 2;	// Upper window (default).
	if(n != INT_MIN) {
		if(n < 0) {
			if((upperRows += n) < 1)
				upperRows = 1;
			...
			upperRows = (n < sess.pCurWind->rows - 1) ? n : sess.pCurWind->rows - 2;
		}
	lowerRows = (sess.pCurWind->rows - 1) - upperRows;	// Lower window.

	// Make new window the bottom one.
	pWind->next = sess.pCurWind->next;
	sess.pCurWind->next = pWind;
	sess.pCurWind->rows = upperRows;
	pWind->rows = lowerRows;
	pWind->topRow = sess.pCurWind->topRow + upperRows + 1;

	// Adjust current window's top line if needed.
	if(pointRow > upperRows)
		pWindFace->pTopLine = pWindFace->pTopLine->next;
	...
	if(pointRow < sess.pCurWind->rows) {
		...
		if(pLine->next == NULL) {
			...
			if(pWindFace->point.pLine->next != NULL) {
				pWind->face.point.pLine = sess.pCurBuf->pFirstLine->prev;
		...
		upperRows = sess.pCurWind->rows / 2;
		pLine = pWindFace->pTopLine;
		while(upperRows-- > 0)
			pLine = pLine->next;
		pWindFace->point.pLine = pLine;
	...
	sess.pCurWind->flags |= (WFHard | WFMode);
	if((n != 0 && !defIsUpper) || (n == 0 && defIsUpper)) {
		sess.pCurScrn->pCurWind = pWind;
		pWind = sess.pCurWind;
		sess.pCurWind = sess.pCurScrn->pCurWind;
		}
	*ppWind = pWind;
...
int chgWindSize(Datum *pRtnVal, int n, int how) {
	...
	adjWind = sess.pCurWind->next;
	if(sess.pCurWind != sess.windHead && (n < 0 || adjWind == NULL))
		adjWind = windNextIs(sess.pCurWind);
	...
	if(how == 0) {
		...
		if(n > sess.pCurWind->rows) {
			n -= sess.pCurWind->rows;
			grow = true;
			}
		else if(n == sess.pCurWind->rows)
			return sess.rtn.status;	// Nothing to do.
		else {
			n = sess.pCurWind->rows - n;
			grow = false;
			}
		}
	...
	if(grow) {
		...
		// Yes, proceed.
		if(sess.pCurWind->next == adjWind) {		// Shrink below.
			windNewTop(adjWind, NULL, n);
			adjWind->topRow += n;
			}
		else {					// Shrink above.
			windNewTop(sess.pCurWind, NULL, -n);
			sess.pCurWind->topRow -= n;
			}
		sess.pCurWind->rows += n;
		adjWind->rows -= n;
		}
	else {
		// Current window big enough?
		if(sess.pCurWind->rows <= n)
			return rsset(Failure, 0, text93, n, n == 1 ? "" : "s");
				// "Current window too small to shrink by %d line%s"

		// Yes, proceed.
		if(sess.pCurWind->next == adjWind) {		// Grow below.
			windNewTop(adjWind, NULL, -n);
			adjWind->topRow -= n;
			}
		else {					// Grow above.
...
int resizeWind(Datum *pRtnVal, int n, Datum **args) {
	...
	if(n == INT_MIN || n == sess.pCurWind->rows)
		return sess.rtn.status;
	...
	if(n == 0) {
		int count = getWindCount(sess.pCurScrn, NULL);
		...
		EWindow *pOldWind = sess.pCurWind;
		EWindow *pWind;
		int leftover;
...
//
// Possible return values are listed below.  The new-pBuf? value is true if RendNewBuf flag set; otherwise, false:
//	n == -1:		pBuf-name (or nil if buffer deleted)
//	n == 0:			[pBuf-name, new-pBuf?]
//	Other n:		[pBuf-name, new-pBuf?, targ-wind-num, new-wind?]
//
// Notes:
...
int render(Datum *pRtnVal, int n, Buffer *pBuf, ushort flags) {
	...
	if(n != 0) {
		...
		// Not popping buffer.  Switch to it?
			...
			if(pBuf != sess.pCurBuf && bswitch(pBuf, 0) != Success)
				return sess.rtn.status;
			pWind = sess.pCurWind;
			}
...
Retn:
	...
	if(n == -1) {
		...
		if((flags & RendNotify) || (n == 0 && (flags & RendNewBuf)))
			(void) rsset(Success, 0, text381, pBuf->bufname);
				// "Buffer '%s' created"
		}
	if(pBuf != NULL && (flags & RendReset))
		pBuf->flags &= ~BFHidden;
	return sess.rtn.status;
LibFail:
	return librsset(Failure);
...
int sfind(ushort scrNum, Buffer *pBuf, EScreen **ppScrn) {
	...
	if(pBuf != NULL) {
		...
		pScrn1->cols = term.cols;
		pScrn1->cursorRow = pScrn1->cursorCol = pScrn1->firstCol = 0;
...
int sswitch(EScreen *pScrn, ushort flags) {
	...
	sess.pCurScrn->pCurWind = sess.pCurWind;
	sess.pCurScrn->rows = term.rows;
	...
	if(pScrn->workDir != sess.pCurScrn->workDir)
		(void) chgdir(pScrn->workDir);

	// Reset the current screen, window and buffer.
	sess.windHead = (sess.pCurScrn = pScrn)->windHead;
	sess.pCurBuf = (sess.pCurWind = pScrn->pCurWind)->pBuf;
	...
	if(n != 0 || !(flags & SWB_Repeat)) {
		...
		if(flags & SWB_Repeat) {
			...
			if(scrTotal == 1 || (n %= scrTotal) == 0)	// If only one screen or repeat count == screen count...
				return sess.rtn.status;			// nothing to do.
			if(flags & SWB_Forw) {
				if((n += sess.pCurScrn->num) > scrTotal)
					n -= sess.pCurScrn->num;
				}
			else if((n = sess.pCurScrn->num - n) < 1)
				n += scrTotal;
			}
		...
		while(pScrn->num != n)
			pScrn = pScrn->next;
		oldDir = sess.pCurScrn->workDir;			// save current directory...
		if(sswitch(pScrn, 0) == Success)			// make new screen current...
			...
			if(!(modeInfo.cache[MdIdxWkDir]->flags & MdEnabled) && sess.pCurScrn->workDir != oldDir)
				rsset(Success, RSHigh | RSNoFormat | RSNoWrap, pScrn->workDir);
		}
	return sess.rtn.status;
	}
...
int selectScreen(Datum *pRtnVal, int n, Datum **args) {
	...
	if(n == 0) {
		...
		if(sfind(0, sess.pCurBuf, &pScrn) != Success || sswitch(pScrn, 0) != Success)
			return sess.rtn.status;
...
Found:
	...
	if(scrNum == 1 || sess.pCurScrn->num != scrNum0) {
...
int delScreen(Datum *pRtnVal, int n, Datum **args) {
	...
	// Make sure it exists.
	if(!sfind((ushort) n, NULL, &pScrn))
		return rsset(Failure, 0, text240, n);
			// "No such screen '%d'"

	// Switch screens if deleting current one.
	if(pScrn == sess.pCurScrn && sswitch(pScrn == sess.scrnHead ? pScrn->next : sess.scrnHead, 0) != Success)
		return sess.rtn.status;
	...
	return rsset(Success, 0, text178, n);
			// "Screen %d deleted"
	}
...
Found:
	...
	if(args[1]->type == dat_nil) {
		...
		Array *pArray = wrapPtr(args[1])->pArray;
