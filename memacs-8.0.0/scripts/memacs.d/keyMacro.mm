...
!macro kmGetKeyName,0
	...
	# Prompt for name.
	name = prompt p
	nil?(name) || null?(name) ? nil : 1 => sub(name,' ','_')
!endmacro
...
!macro kmFindKeyMacro,2
	...
	!else
		globalModes = $globalModes
		$globalModes = $ModeExact | $ModeRegExp
		result = searchForw("^#{selector = $1} ") and result = subLine 0,99999
		$globalModes = globalModes
		notice nil
	!endif
...
!macro kmShowKeyMacros,0
	...
	!until nil?(line = readBuf $KeyMacroBuf)
		j = index(line,' ')
		writeBuf listBuf,"\r",pad(--i,6)
		(isMatch = strip(subString(line,j,999)) == $keyMacro) and found = true
...
bindKey 'C-K',kmSelectKeyMacro
bindKey 'C-^S',kmSaveKeyMacro
bindKey 'C-^D',kmDeleteKeyMacro
bindKey 'H-Y',kmShowKeyMacros
bindKey 'C-E',kmXeqKeyMacro
