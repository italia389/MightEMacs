...
altermode:
		...
		case cf_deleteBackChar:
			...
			(void) ldelete(n == INT_MIN ? -1L : (long) -n,false);
			break;
...
kdctxt:
		...
		case cf_nextArg:
			if(scriptrun == NULL || scriptrun->malp->mal_argp == NULL)
				(void) vnilmm(rp);
			else if(vcpy(rp,scriptrun->malp->mal_argp->ma_vp) != 0)
				(void) vrcset();
			else
...
inlsp:
		...
		case cf_tr:
			...
			break;
		case cf_truncBuf:
			// Truncate buffer.  Delete all text from current buffer position to end (without saving).
