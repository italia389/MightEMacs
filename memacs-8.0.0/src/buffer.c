...
// Inactive all window marks that are outside the current narrowed buffer.
static void mrkoff(EWindow *winp) {
	Mark *mkp,*mkpz;
	Line *lnp;

	// First, inactivate all marks, except any pointing to the buffer header line (EOB), which is always valid.
	mkpz = (mkp = winp->w_face.wf_mark) + NMARKS;
	do {
		if(mkp->mk_dot.off >= 0 && mkp->mk_dot.lnp != curbp->b_hdrlnp)
...
int narrowBuf(Value *rp,int n) {
	...
	if(n == INT_MIN)
		...
		Dot odot = *dotp;
		if(reglines(&n,&wholebuf) != SUCCESS)
			return rc.status;

		// Error if whole buffer was marked.
		if(wholebuf) {
			*dotp = odot;
			return rcset(FAILURE,0,text72);
				// "Cannot narrow whole buffer"
			}
		}
	else if(n < 0) {
		int i = n;
...
static void unnarrow(Buffer *bufp) {
	...
	if(bufp->b_nbotlnp != NULL) {
		if(bufp == curbp) {
			...
			mkpz = (mkp = curwp->w_face.wf_mark) + NMARKS;
			}
		else {
			// If the point is at EOF, move it to the beginning of the bottom fragment.
			...
			mkpz = (mkp = bufp->b_face.wf_mark) + NMARKS;
			}
	...
	do {
		...
		do {
			if(winp->w_bufp == bufp) {
				...
				// Found window attached to widened buffer.  Active all its marks.
				mkpz = (mkp = winp->w_face.wf_mark) + NMARKS;
				do {
					if(mkp->mk_dot.off < 0)
						mkp->mk_dot.off = -mkp->mk_dot.off - 1;
	...
	// and now forget we are narrowed.
	bufp->b_flags &= ~BFNARROW;
	}

// Widen from region.  Restore a narrowed buffer.  Set rp to buffer name and return status.
int widenBuf(Value *rp,int n) {

	// Make sure we are narrow.
	if((curbp->b_flags & BFNARROW) == 0)
...
// Generate a valid buffer name from a pathname.  bnamep is assumed to point to a buffer that is NBUFN + 1 or greater in size.
...
// that bnamep points to is assumed to be of size NBUFN + 1 or greater.
static char *bunique(char *bnamep,char *fnamep) {
	int n,i;
	char *strp0,*strp1,*strpz = bnamep + NBUFN;
	...
	while(bsrch(bnamep,NULL) != NULL) {
		...
		// Add 1 to numeric suffix (or begin at 0), and put it back.  Assume that NBUFN is >= number of bytes needed to
...
int bfind(char *namep,uint cflags,uint bflags,Buffer **bufpp,bool *createdp) {
	...
	if(cflags & CRBCREATE) {
		...
		// Macro name?
		if(*bnamep == SBMACRO && (bflags & BFMACRO) == 0)
		...
		// Add macro name to the CAM list and return results.
		if(bflags & BFMACRO)
			(void) camfind(bufp2->b_bname + 1,OPCREATE,PTRMACRO);
		if(bufpp != NULL)
			*bufpp = bufp2;
		if(createdp != NULL)
			*createdp = true;
...
int bclear(Buffer *bufp,uint flags,bool *clearedp) {
	...
	if(bufp->b_flags & BFNARROW)
		bufp->b_flags |= BFCHGD;			// but mark as changed if narrowed.
	bufinit(bufp,bufp->b_hdrlnp);				// Fix dot and invalidate marks.
	bufErased = true;
...
int bufcount(void) {
	...
	bufp = bheadp;
	do {
		if((bufp->b_flags & BFHIDDEN) == 0)
			++count;
		} while((bufp = bufp->b_nextp) != NULL);
	return count;
	}

// Get a buffer name (if n not default) and perform operation on buffer according to "op" argument.  Set rp to buffer name and
// return status.
int bufop(Value *rp,int n,char *prmpt,int op,uint flag) {
	Buffer *bufp;
	...
	if(n == INT_MIN)
		...
		if(getcbn(rp,prmpt,bufp != NULL ? bufp->b_bname : NULL,OPDELETE,&bufp,NULL) != SUCCESS || bufp == NULL)
			return rc.status;
		}
	...
	switch(op) {
		...
		case BOPCLRFLAG:
			...
			break;
		default:	// BOPMOVEDOT
			// Move to beginning or end of buffer.  (Usually) massive adjustment of dot.  Set "hard motion" flag.
			{EWindow *winp = curwp;
			if(bufp == curbp || bufWind(bufp,NULL,&winp)) {
				winp->w_face.wf_dot.lnp = flag ? winp->w_bufp->b_hdrlnp : lforw(winp->w_bufp->b_hdrlnp);
				winp->w_face.wf_dot.off = 0;
				winp->w_flags |= WFMOVE;
				}
			else {
				bufp->b_face.wf_dot.lnp = flag ? bufp->b_hdrlnp : lforw(bufp->b_hdrlnp);
				bufp->b_face.wf_dot.off = 0;
				}
			}
		}
...
static int bactivate(Buffer *bufp) {

	// Check if buffer is active.
	if((bufp->b_flags & BFACTIVE) == 0) {

		// Not active: read it in and initialize buffer settings.
		if(readin(bufp,NULL,true) > MINEXIT)
...
int insertBuf(Value *rp,int n) {
	...
	bufp = bdefault(false,false);
	if(getcbn(rp,text55,bufp != NULL ? bufp->b_bname : NULL,OPDELETE,&bufp,NULL) != SUCCESS || bufp == NULL)
			// "Insert"
		return rc.status;
	if(bufp == curbp)
	...
	wfp->wf_dot.lnp = lback(wfp->wf_dot.lnp);	// Back up a line and save dot in mark 0.
	wfp->wf_dot.off = 0;
	...
	// Report results.
	return rcset(SUCCESS,RCFORCE,"%s %u %s%s",text154,nline,text205,nline == 1 ? "" : "s");
					// "Inserted","line",
	}
...
int selectBuf(Value *rp,int n) {
	...
	// Get the buffer name.
	bufp = bdefault(false,false);
	if(getcbn(rp,n < 0 && n != INT_MIN ? text27 : text24,bufp != NULL ? bufp->b_bname : (n == INT_MIN || n >= 0) ?
...
// Make given buffer current and return status.  The top line, dot, marks, and column offset values from the current window are
// saved in the old buffer's header and replacement ones are fetched from the new (given) buffer's header.
int bswitch(Buffer *bufp) {

	// Run exit-buffer user hook on current (old) buffer.
	if(!(curbp->b_flags & (BFHIDDEN | BFMACRO)) && exechook(NULL,INT_MIN,hooktab + HKEXITBUF,0) != SUCCESS)
		return rc.status;
	...
	curwp->w_face = curbp->b_face;
	curwp->w_flags |= WFMODE | WFHARD;

	// Run enter-buffer user hook on current (new) buffer.
	if(rc.status == SUCCESS && !(curbp->b_flags & (BFHIDDEN | BFMACRO)))
		(void) exechook(NULL,INT_MIN,hooktab + HKENTRBUF,0);

	return rc.status;
	}

// Clear a buffer by name.  Force it if n > 0.  Set rp to false if buffer is not cleared; otherwise, true.  Return status.
int clearBuf(Value *rp,int n) {
	...
	if(n == INT_MIN)
		...
		if(n <= 0)
			bufp = bdefault(false,false);
		if(getcbn(rp,text169,n > 0 ? curbp->b_bname : bufp != NULL ? bufp->b_bname : NULL,OPDELETE,&bufp,NULL)
		 != SUCCESS || bufp == NULL)
				// "Clear"
			return rc.status;
		}
	...
	if(bclear(bufp,n > 0 ? CLBIGNCHGD | CLBUNNARROW : 0,&cleared) == SUCCESS) {
...
static bool ishook(Buffer *bufp) {
	...
	do {
		fabp = &hrp->h_fab;
		if(fabp->p_type == PTRMACRO && fabp->u.p_bufp == bufp) {
...
// and remove the name from the CAM list if it's a macro.  Return status.
int bdelete(Buffer *bufp,uint flags) {
	...
	// Delete from CAM list.
	if((bufp->b_flags & BFMACRO) && camfind(bufp->b_bname + 1,OPDELETE,0) != SUCCESS)
		return rc.status;

	fab.u.p_bufp = bufp;				// Get key binding, if any.
	kdp = getpentry(&fab);
...
int deleteBuf(Value *rp,int n) {
	...
	if((opflags & OPSCRIPT) == 0) {
		...
		if(getcbn(rp,text26,bufp != NULL ? bufp->b_bname : NULL,OPDELETE,&bufp,NULL) != SUCCESS || bufp == NULL)
				// "Delete"
			return rc.status;
		goto nukeit;
		}
...
ask:
	...
	if(getarg(bnamep,promptp,NULL,CTRL | 'M',ARG_FIRST) != SUCCESS || (!(opflags & OPSCRIPT) && vistfn(bnamep,VNIL)) ||
	 tostr(bnamep) != SUCCESS)
		return rc.status;
	...
	if(!isbname(bnamep->v_strp)) {
		if(opflags & OPSCRIPT)
			return rcset(FAILURE,0,text128,bnamep->v_strp);
				// "Invalid buffer name '%s'"
		sprintf(promptp = askstr,text128,bnamep->v_strp);
				// "Invalid buffer name '%s'"
		strcat(promptp,text324);
				// ".  New name"
		goto ask;	// Try again.
		}
	...
	do {
		if(bufp != curbp) {
			...
			if((*curbp->b_bname == SBMACRO) != (*bnamep->v_strp == SBMACRO)) {
				...
				sprintf(promptp = askstr,"%s'%c'%s",text273,SBMACRO,text324);
...
setnew:
	...
	if((curbp->b_flags & BFMACRO) &&
	 camfind(curbp->b_bname + 1,OPDELETE,0) != SUCCESS)	// Remove old macro name from CAM list ...
		return rc.status;
	strcpy(curbp->b_bname,bnamep->v_strp);			// copy new buffer name to structure ...
	relistbuf();						// reorder buffer in list ...
	if((curbp->b_flags & BFMACRO) &&
	 camfind(curbp->b_bname + 1,OPCREATE,PTRMACRO) != SUCCESS)// add new macro name to CAM list ...
		return rc.status;
...
ulong bufsize(Buffer *bufp) {
	ulong nbytes;
	Line *lnp;

	nbytes = 0;				// Count bytes in buffer.
...
// Read the next line from a buffer.  Return status.
int readBuf(Value *rp,int n) {
	Buffer *bufp;
	EWindow *winp = NULL;

	// Get the buffer name.
	if(macarg(rp,ARG_FIRST) != SUCCESS)
		return rc.status;
	...
	if(bufp->b_nwind > 0) {
		winp = findwind(bufp);
		bufp->b_face.wf_dot = winp->w_face.wf_dot;
		}
	...
	// Grab the current line (from dot to end) as an argument ...
	n = lused(bufp->b_face.wf_dot.lnp) - bufp->b_face.wf_dot.off;

	// return the spoils ...
	if(vsetfstr(ltext(bufp->b_face.wf_dot.lnp) + bufp->b_face.wf_dot.off,n,rp) != 0)
		return vrcset();

	// and step the buffer's line pointer ahead one line.
	bufp->b_face.wf_dot.lnp = lforw(bufp->b_face.wf_dot.lnp);
	bufp->b_face.wf_dot.off = 0;

	// If displayed buffer, update window pointer vars as well.
	if(winp != NULL) {
		winp->w_face.wf_dot = bufp->b_face.wf_dot;
		winp->w_flags |= WFMOVE;
		}
...
// Build and pop up the special buffer containing the list of all buffers (interactive only).  List hidden buffers as well if n
// arg.  Render buffer and return status.
int showBuffers(Value *rp,int n) {
	...
	do {
		...
		*strp++ = ' ';					// Gap.
		...
		sprintf(strp,"%10hu",bufp->b_nwind);
		strp += 10;
#else
		do {
			*strp++ = ' ';
...
#endif
#if 1
		sprintf(strp,"%9lu",bufsize(bufp));		// 9-digit (minimum) buffer size.
#else
		sprintf(strp,"%9lu",bufsize(bufp) + 1023400000ul);		// Test it.
#endif
		strp = strchr(strp,'\0');
		*strp++ = ' ';					// Gap.
#if 0
