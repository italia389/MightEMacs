...
#endif
#if TT_CURSES || (TT_TERMCAP && (REDHAT || OSX))
#include <curses.h>			// Curses screen output.
#endif
#if TT_TERMCAP && (REDHAT || OSX)
#include <term.h>
#endif
...
static struct tkeybind tkeybind[] = {	// Keybinding list.
	...
	{ "kH",FKEY|'>'	},		// Home down key.
	{ "@7",FKEY|'>'	},		// Home down key.
	{ "kh",FKEY|'<'	},		// Home key.
...
// Add character sequence to keymap table.  "fn" is key code.  Return status.
static int addkey(char *seq,int fn) {
	bool first;
...
#elif HPUX8 || HPUX9 || REDHAT || OSX || USG
	int count;
	uchar ch;
...
static int scopen(void) {
	...
	if(!isatty(0)) {
		...
		// done so that FD 0 is always the terminal which should ensure that tgetent() will work properly on brain-dead
		// Unix variants.
		if((stdinfd = dup(0)) == -1)
	...
	// Get keybindings.
#if REDHAT || OSX
	// These keys don't make it into keymap for some reason (in loop below).
...
#if TT_TERMCAP
		if(addkey(tgetstr(kp->name,&strp),kp->value) != SUCCESS)
#else
		if(addkey(tgetstr(kp->name,NULL),kp->value) != SUCCESS)
#endif
			return rc.status;
		++kp;
		}

#if TT_TERMCAP && (HPUX8 || HPUX9)
...
int spal(char *cmd) {
...
#endif
			return rcset(FAILURE,0,text54,cmd);
				// "$palette value '%s' is invalid"
	cmd += 7;
	...
	if(dokeymap)
		...
		if(stoec(cmd,&code) != SUCCESS || addkey(strp,code) != SUCCESS)
			return rc.status;
#if TT_TERMCAP
	else {
		long lval;
...
int changedir(Value *rp,int n) {
	...
	// Get directory name.
	if(complete(rp,text277,NULL,CMPL_FILENAME,NPATHINP,ARG_NOTNULL) != SUCCESS ||
...
static int callout(Value *rp,Value *cfp,bool force) {
	...
	if((opflags & OPSCRIPT) == 0 && (rcode != 0 || force)) {
		mlputs(MLHOME | MLFORCE | MLWRAP,text188,vz_show);
			// "End"
		if(ttflush() == SUCCESS)
			(void) ttgetc(NULL);
		}
...
static int getcmd(Value **cfpp,char *promptp) {
	...
	if(opflags & OPSCRIPT) {
		...
		(void) join(*cfpp,NULL,1,true);
		}
	else if(termarg(*cfpp,promptp,NULL,CTRL | 'M',ARG_NOTNULL) == SUCCESS && !vistfn(*cfpp,VNIL))
		(void) strval(*cfpp);
...
static int prepcmd(Value *rp,StrList *slp,char *promptp,bool *ucancelp,Value **tfilepp1,Value **tfilepp2) {
	Value *cfp;

	// Get shell command.
	if(getcmd(&cfp,promptp) != SUCCESS)
		return rc.status;
...
// Return pointer to directory name, given pathname or filename.  Given name is modified (truncated).
char *fdirname(char *name) {
	char *strp;
...
// Get pathname of given file.  Return status.
int getpath(Value *pathp,char *fname) {
