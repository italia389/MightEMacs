...
// This function deletes "n" bytes, starting at dot.  Positive n deletes forward, negative n deletes backWords.  It understands
// how to deal with end of lines, and with two-byte characters.  It returns current status if all of the characters were
// deleted, NOTFOUND (bypassing rcset()) if they were not (because dot ran into the buffer end), or the appropriate status if an
// error occurred.  The text is put in the kill buffer if kflag is true.
int ldelete(long n,bool kflag) {
	char *strp1,*strp2;
	Line *lnp;
	int offset,chunk;
	EScreen *scrp;
	...
	if(n >= 0) {
		while(n > 0) {
			...
			if(chunk == 0) {
				...
				lchange(curbp,WFHARD);
				if(ldelnewline() != SUCCESS || (kflag && kinsert(FORWARD,'\r') != SUCCESS))
					return rc.status;
				--n;
				continue;
				}
			...
			if(kflag) {
				while(strp1 != strp2) {
					if(kinsert(FORWARD,*strp1) != SUCCESS)
						return rc.status;
					++strp1;
					}
				strp1 = lnp->l_text + offset;
		...
		while(n < 0) {
			...
			if(chunk == 0) {
				...
				lchange(curbp,WFHARD);
				(void) backch(1);
				if(ldelnewline() != SUCCESS || (kflag && kinsert(BACKWARD,'\r') != SUCCESS))
					return rc.status;
				++n;
				continue;
				}
			...
			if(kflag) {
				while(strp1 > strp2) {
					if(kinsert(BACKWARD,*(--strp1)) != SUCCESS)
						return rc.status;
					}
				strp1 = lnp->l_text + offset;
				}
		...
		while(dotp->off <= lused(dotp->lnp)) {
			...
			if(fspace >= 0 && nextab(fspace) <= ccol) {
				...
				if((len = ccol - fspace) >= 2) {
					dotp->off -= len;
					if(ldelete((long) len,false) != SUCCESS || linsert(1,'\t') != SUCCESS)
						return rc.status;
					}
				fspace = -1;
				}
...
int insrfence(int c) {
	...
	*dotp = origdot;
	...
	if(kwlen > 0)
		(void) backch(kwlen);
	if(ldelete((long) -dotp->off,false) != SUCCESS)
		return rc.status;
...
int deleteBlankLines(Value *rp,int n) {
	...
	dotp->off = 0;
	return ldelete((long) count,false);
	}
...
int newlineI(Value *rp,int n) {
	...
	while(n-- > 0)
		if(lnewline() != SUCCESS || (indentp != NULL && n == 0 && linstr(indentp->v_strp) != SUCCESS))
			break;
...
int deleteTab(Value *rp,int n) {
	...
	if(n == INT_MIN)
		n = -1;
	if(n == 0 || (i = lused(dotp->lnp)) == 0 || ((off = dotp->off) == 0 && n < 0) || (off == i && n > 0))
		return rc.status;
	...
	if(stabsize == 0) {
		...
		while((off = dotp->off + i) >= 0 && off < lused(dotp->lnp) && lgetc(dotp->lnp,off) == '\t')
			if(ldelete(direc,false) != SUCCESS || --n == 0)
				break;
		}
	else {
...
nuke:;
						...
						int m = abs(n) - 1;
						(void) ldelete((long) -(chunk1 + (m > maxleft ? maxleft : m) * stabsize),false);
						if(n > 0)
							dotp->off = off;
						}
					}
...
int kdctext(int n,int kdc,Region *regp) {
	...
	if(regp != NULL) {
		...
		if(kdc)
			kprep(false);
		*dotp = regp->r_dot;
		return ldelete(regp->r_size,kdc);
		}
...
int kdcline(int n,int kdc) {
	...
	if(kdc <= 0)
		return (n <= 0 || (n == 1 && used1 == 0)) ? rc.status : ldelete(1L,kdc);
	if((n > 1 || (n == 1 && used1 > 0)) && kinsert(FORWARD,'\r') != SUCCESS)
		return rc.status;
	*dotp = odot;
...
// Join adjacent line(s), replacing all white space in between with a single space character (unless either line is blank or all
// white space) and insert extra space character if first of two adjacent lines ends with any character specified in "delimp".
// Return status.
static int joinln(Value *rp,int n,Value *delimp) {
	...
	do {
		// Move to end or beginning of line, delete newline, delete white space, and if not at beginning or end of new
		// line, insert one or two spaces.
		if(incr == 1) {
		...
		if(ldelete((long) incr,false) != SUCCESS || delwhite() != SUCCESS)
			return rc.status;
		if(dotp->off > 0 && dotp->off < lused(dotp->lnp)) {
			m = 1;
...
int wrapLine(Value *rp,int n) {
	...
	if(dotp->off > 0) {						// Save any indentation (from first line of block) ...
		if(vnew(&indentp,false) != 0 || vsetfstr(ltext(dotp->lnp),dotp->off,indentp) != 0)
			return vrcset();
		if(ldelete(-dotp->off,false) != SUCCESS)		// and delete it.
			return rc.status;
		}
	...
	if(prefixLen > 0) {
		...
		do {
			...
			if(lused(dotp->lnp) - dotp->off >= striplen && memcmp(ltext(dotp->lnp) + dotp->off,prefixp->v_strp,
			 striplen) == 0 && (ldelete(striplen,false) != SUCCESS || (count == n && delwhite() != SUCCESS)))
				return rc.status;
	...
	if(lused(dotp->lnp) > 0) {
		...
		for(;;) {
			...
			if(lused(dotp->lnp) == 0)			// Blank line after wrap?
				return ldelete(1L,false);		// Yes, done.
...
// Kill, delete, or copy forward by "n" words.  Save text to kill ring if kdc is non-zero.  If zero argument, kill or copy just
// one word and no trailing whitespace.  No error if attempt to move past the end of the buffer.
int kdcfword(int n,int kdc) {
	bool oneword;
	Region region;
	Dot *dotp = &curwp->w_face.wf_dot;

	// No op if already at end of buffer.
	if(dotp->lnp == curbp->b_hdrlnp)
		return rc.status;
...
nuke:
	...
	*dotp = region.r_dot;
	if(kdc <= 0) {
		...
		if(kdc)
			kprep(false);
		return ldelete(region.r_size,kdc);
		}
...
// Kill, delete, or copy backward by "n" words.  Save text to kill ring if kdc is non-zero.  No error if attempt to move past
...
int kdcbword(int n,int kdc) {
	...
	// No op if zero argument or already at beginning of buffer.
	if(n == INT_MIN)
		n = 1;
	else if(n == 0 || backch(1) != SUCCESS)
		return rc.status;
