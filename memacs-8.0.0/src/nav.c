...
// Get a non-default n argument (via prompt if interactive) and validate it.  Return status.
int getnum(char *promptp,int *np) {

	// Error if in a script.
	if(opflags & OPSCRIPT)
		return rcset(FAILURE,0,text57);
			// "Argument expected"
	else {
		Value *vp;
		...
		if(vnew(&vp,false) != 0)
			return vrcset();
		if(getargCR(vp,promptp) == SUCCESS && !vistfn(vp,VNIL) && toint(vp) == SUCCESS)
			*np = vp->u.v_int;
		}
	return rc.status;
	}
...
int gotoLine(Value *rp,int n) {
	...
	if(n == INT_MIN && getnum(text7,&n) != SUCCESS)
				// "Go to line"
		return rc.status;
...
int forwPage(Value *rp,int n) {
	...
	if(n > 0) {
		...
		pagesize = curwp->w_nrows - overlap;	// Default scroll.
