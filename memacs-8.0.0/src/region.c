...
// Delete or kill a region.  Ask "getregion" to figure out the bounds of the region.  Move "." to the start, and delete or
...
int dkregion(int n,bool kill) {
	...
	if(kill)
		kprep(false);
	curwp->w_face.wf_dot = region.r_dot;
	return ldelete(region.r_size,kill);
	}
...
int copyreg(Region *regp) {
	...
	while(regp->r_size-- > 0) {
		if(offset == lused(lnp)) {	// End of line.
			if(kinsert(FORWARD,'\r') != SUCCESS)
				return rc.status;
			lnp = lforw(lnp);
			offset = 0;
			}
		else {				// Beginning or middle of line.
			if(kinsert(FORWARD,lgetc(lnp,offset)) != SUCCESS)
				return rc.status;
			++offset;
			}
		}
...
int indentRegion(Value *rp,int n) {
	...
	if(n == INT_MIN)
		...
		return (n < 0) ? rcset(FAILURE,0,text39,text137,n,0) : rc.status;
