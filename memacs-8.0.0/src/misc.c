...
// Display the current position of the cursor in 1-origin X-Y coordinates, the fraction of the text that is before the cursor,
// and the character that is under the cursor (in printable form and hex).  The displayed column is not the current column, but
// the column that would be used on an infinite-width display.  (Interactive only)
int whence(Value *rp,int n) {
	...
	if((modetab[MDR_GLOBAL].flags & MDMSG) == 0)
		return rc.status;
	...
	while(lnp != curbp->b_hdrlnp) {
		...
		if(lnp == dotp->lnp) {
			...
			if((dotp->off) == lused(lnp))
				curchar = '\r';
			else
				curchar = lgetc(lnp,dotp->off);
			}
	...
	if(numchars != 0)
		ratio = (100.0 * predchars) / numchars;

	// Summarize and report the info.
	if(curchar >= ' ' && curchar < 0x7f)
		sprintf(wkbuf,"'%c' 0x%.2x",curchar,curchar);
	else
		sprintf(wkbuf,"0x%.2x",curchar);
	return rcset(SUCCESS,0,text60,predlines + 1,numlines,col,ecol,predchars,numchars,ratio,wkbuf);
			// "Line %lu/%lu, Col %d/%d, Char %lu/%lu (%.1f%%), char = %s"
	}
...
int fmatch(int ch) {
	...
	if(count == 0)
		if(update(false) != SUCCESS || cpause(fencepause) != SUCCESS)
			return rc.status;
...
int countWords(Value *rp,int n) {
	...
	while(size--) {
		...
		// and tabulate it.
		if((wordflag = (ch >= 'a' && ch <= 'z') || (ch >= 'A' && ch <= 'Z') || (ch >= '0' && ch <= '9')))
			++nchars;
		if(wordflag && !inword)
			++nwords;
		inword = wordflag;
...
int seti(Value *rp,int n) {
	...
	if(opflags & OPSCRIPT) {
		...
		if(macarg(vp,ARG_FIRST | ARG_NOTNULL | ARG_INT) != SUCCESS)
			return rc.status;
		if(opflags & OPEVAL)
			i = vp->u.v_int;
		...
		if(havesym(s_comma,false)) {
			...
			if(macarg(vp,ARG_NOTNULL | ARG_INT) != SUCCESS)
				return rc.status;
			if(opflags & OPEVAL)
				inc = vp->u.v_int;
			...
			if(havesym(s_comma,false)) {
				...
				// Yes, get it.
				if(macarg(vp,ARG_NOTNULL | ARG_STR) != SUCCESS)
					return rc.status;
				newfmt = true;
				}
			}
		...
		// Prompt for "i" value.
		if(termarg(vp,text102,"0",CTRL | 'M',0) != SUCCESS || toint(vp) != SUCCESS)
				// "Beginning value"
			return rc.status;
		i = vp->u.v_int;
		...
		sprintf(nbuf,"%d",inc);
		if(termarg(vp,text234,nbuf,CTRL | 'M',0) != SUCCESS || toint(vp) != SUCCESS)
				// "Increment"
			return rc.status;
		inc = vp->u.v_int;

		// Prompt for "format" value.
		if(termarg(vp,text235,ivar.format.v_strp,CTRL | '[',0) != SUCCESS)
				// "Format string"
			return rc.status;
		newfmt = true;
		}
