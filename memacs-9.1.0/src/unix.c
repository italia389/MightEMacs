...
static int tmpfname(Datum *datp) {
	...
	if(dsalloc(datp,24) != 0)
		return -1;
	sprintf(datp->d_str,"/tmp/_mm%u.%u",si.mypid,n++);
	return 0;
	}

// Call out to system to perform given command.  Return status, and false if error so that macros won't abort.
static int callout(Datum *rp,Datum *cfp,ushort flags,bool force) {
	int rcode;

	// Close down.
	if(scclose(flags) != Success)
		return rc.status;
	...
	if(si.opflags & OpScript) {
		...
		if(mlputs(MLHome | MLWrap | MLFlush,text188) == Success)
							// "End"
			(void) ttgetc(NULL);
		}
...
int shellCLI(Datum *rp,int n,Datum **argpp) {
	...
	// Do shell.
	return dnewtrk(&datp) != 0 || dsetstr(shpath,datp) != 0 ? librcset(Failure) :
#if RestoreTerm
	 callout(rp,datp,TermCup,false);
#else
	 callout(rp,datp,0,false);
#endif
	}
...
static int getcmd(Datum **cfpp,char *prmt) {
	...
	if(si.opflags & OpScript) {
		...
		(void) terminp(*cfpp,prmt,ArgNotNull1 | ArgNil1,0,NULL);
	...
	*ucancelp = false;
	if(tfilepp2 != NULL) {
		...
		if(dclose(sfp,sf_string) != 0)
			goto LibFail;
		if(callout(rp,sfp->sf_datp,0,false) == Success && rp->d_type == dat_false)
...
int readPipe(Datum *rp,int n,Datum **argpp) {
	...
	if(prepcmd(rp,&cmd,text170,&ucancel,&tfilep,NULL) == Success && !ucancel && rp->d_type != dat_false) {
		...
		// Return results.
		(void) readFP(rp,n,tfilep->d_str,RWScratch);
		(void) unlink(tfilep->d_str);
		}
...
int filterBuf(Datum *rp,int n,Datum **argpp) {
	...
	fname0 = si.curbp->b_fname;		// Save the original (heap) name.
	si.curbp->b_fname = NULL;

	// Write out the buffer, checking for errors.
	if(writeout(si.curbp,tfilep1->d_str,'w',0) != Success)
		return rc.status;

	// Execute the shell command.  If successful, read in the file that was created.
	if(callout(rp,cmd.sf_datp,0,false) == Success && rp->d_type != dat_false)
		if(readin(si.curbp,tfilep2->d_str,RWExist) == Success) {

			// Mark buffer as changed.
			si.curbp->b_flags |= BFChanged;
			}

	// Restore the original filename and get rid of the temporary files.
	si.curbp->b_fname = fname0;
