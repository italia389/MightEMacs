...
DoFile:
				...
				else if(bfind(str,BS_Create | BS_Hook | BS_Force | BS_File,0,&bufp,NULL) != Success)
					return rc.status;
				if(stdinflag > 0)		// If using standard input...
					stdinflag = -1;		// disable the flag so it can't be used again.
				else if(setfname(bufp,str,NULL) != Success)
					return rc.status;
	...
	for(;;) {
		...
		if(!disnull(&lastrc.msg)) {
			...
			if(mlputs(flags,lastrc.msg.d_str) != Success ||
			 movecursor(si.cursp->s_cursrow,si.cursp->s_curscol) != Success || TTflush() != Success)
				break;
			}
...
JumpStart:
			...
			if(getkseq(&ek,&kbp,true) <= MinExit)
				break;
...
Decr5:
				...
				tungetc(ek);
				if(getkseq(&ek,&kbp,true) != Success)
					goto Retn;
				break;
				}
...
// Convert return message to multiple lines (at ", in") and remove ~ (AttrSeqBegin) sequences if RCTermAttr flag set.  Return
// Success or Failure.
static int cleanmsg(char **strp) {
	...
	if(dopentrk(&msg) == 0) {
		...
		// ~R, and ~0 with nothing (remove) if RCTermAttr flag set.  Also insert newlines in text229 string (from
		// macerror() function), if any, so that error callback is easier to read.
		while((c = *str++) != '\0') {
			if(c == AttrSeqBegin) {
