...
// Create an empty array, stash into rp, and return pointer to it in *arypp.  Return status.
	...
	do {
		if(aip->cflags & type) {
			...
			if(dputs(lead,&prompt) != 0 || (aip->pindex == 1 && dputc(attrname[0],&prompt) != 0) ||
			 dputf(&prompt,"~u~b%c~0%s",(int) attrname[aip->pindex],attrname + aip->pindex + 1) != 0)
				goto LibFail;
			lead = ", ";
			}
		} while((++aip)->name != NULL);
...
fprintf(logfile,"*** Writing %s modes...\n",mtabp->mask == 0 ? "BUFFER" : "GLOBAL");
		...
		while((datp = aeach(&aryp)) != NULL) {
			...
			*str++ = '\n';
			do {
				*str++ = (ftp->c != SMActive || mtabp->mask) ? (msp->ms_flags & ftp->flag ? ftp->c : ' ') :
				(bmsrch1(si.curbp,msp) ? ftp->c : ' ');
				} while((++ftp)->desc != NULL);
			sprintf(str," ~b%-12s~0",msp->ms_name);
			if(dputs(wkbuf,&rpt) != 0 || (msp->ms_desc != NULL && dputs(msp->ms_desc,&rpt) != 0))
				goto LibFail;
			}
...
#endif
	if(dputs("\n\n",&rpt) != 0 || dputs(text401,&rpt) != 0 || dputc('\n',&rpt) != 0)
					// "MODE GROUPS"
		goto LibFail;
	mgp = mi.gheadp;
	do {
		sprintf(wkbuf,"\n %c   ~b%-12s~0",mgp->mg_flags & MdUser ? SMUser : ' ',mgp->mg_name);
		if(dputs(wkbuf,&rpt) != 0 || (mgp->mg_desc != NULL && dputs(mgp->mg_desc,&rpt) != 0) ||
		 dputs("\n                     ",&rpt) != 0)
			goto LibFail;
		c = '[';
		aryp = &mi.modetab;
...
fputs("*** Writing footnote...\n",logfile);
	...
	do {
		sprintf(wkbuf,"\n%c ",(int) ftp->c);
		if(dputs(wkbuf,&rpt) != 0 || dputs(ftp->desc,&rpt) != 0)
			goto LibFail;
		} while((++ftp)->desc != NULL);
