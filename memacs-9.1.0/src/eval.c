...
int groupMode(Datum *rp,int n,Datum **argpp) {
	...
	if(mgp->mg_usect > 0) {
		...
		while((datp = aeach(&aryp)) != NULL) {
			msp = msptr(datp);
			if(msp->ms_group == mgp) {
				if(!(msp->ms_flags & MdGlobal) != (n == INT_MIN))
					return rcset(Failure,0,text404,mgp->mg_name,n == INT_MIN ? text83 : text146);
...
Enabled:
	...
	if(resultp == NULL)
		dsetnil(rp);
	else if(dsetstr(resultp->ms_name,rp) != 0)
		(void) librcset(Failure);
	return rc.status;
	}
	...
	if(cfp->cf_func != NULL)
		...
		switch(fnum) {
			...
			case cf_bprintf:
				...
				// Create formatted string into rp and insert into buffer.
...
BEBuf:
			...
			case cf_getKey:
				...
				if(n == INT_MIN)
					n = 1;
				if((n <= 1 ? getkey(&ek,true) : getkseq(&ek,NULL,true)) != Success)
					break;
...
Repl:
			...
			case cf_restoreBuf:
				...
				if(bswitch(si.savbufp) == Success && dsetstr(si.curbp->b_bname,rp) != 0)
					goto LibFail;
				break;
