...
// newline.  If n == 0, select text from dot to the beginning of the line.  If n > 0, select text from dot forward over that
// number of line breaks to the end of the last line and, if RegInclDelim flag set, include its delimiter.  Lastly, if n < 0,
// select text backward over that number of line breaks to the beginning of the first line.  If RegForceBegin flag set, dot in
// the Region object is forced to beginning of region; otherwise, it is left at original starting point and r_size is negated if
// region extends backward.
