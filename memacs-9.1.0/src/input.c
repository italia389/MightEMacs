...
// an extended key.  The legal prefixes here are the FKey and Ctrl.  If savekey is true, store extended key in kentry.lastkseq.
// Return status.
int getkey(ushort *keyp,bool savekey) {
	ushort ek,c;
...
// Get a key sequence from the keyboard and return it in *keyp.  Process all applicable prefix keys.  If kbpp is not NULL, set
// *kbpp to the getbind() return pointer.  If savekey is true, store extended key in kentry.lastkseq.  Return status.
int getkseq(ushort *keyp,KeyBind **kbpp,bool savekey) {
	ushort ek;		// Fetched keystroke.
	ushort p;		// Prefix mask.
	KeyBind *kbp;		// Pointer to a key entry.
	...
	if(kbp != NULL && kbp->k_targ.p_type == PtrPseudo) {
		switch((enum cfid) (kbp->k_targ.u.p_cfp - cftab)) {
			case cf_metaPrefix:
				p = Meta;
				goto Next;
			case cf_prefix1:
...
Next:
				...
				ek |= p;				// Add prefix.
				if(kbpp != NULL) {
					kbp = getbind(ek);		// Get new binding if requested.
					goto Retn;
					}
				break;
			default:		// Do nothing.
...
fprintf(logfile,"mli_write(...,bcol: %d,boff: %hu,%s,\"%s\",ecol: %d): ibufc: '%s'...\n",
 bcol,boff,extChar ? "true" : "false",str,ecol,isp->ibufc == isp->ibufe ? "EOL" : vizc(isp->ibufc[-boff].c,VBaseDef));
	...
	if((bcol != si.mlcol && mlmove(bcol) != Success) || (extChar && mlputc(MLRaw,LineExt) != Success))
		return rc.status;
	...
	if(si.mlcol < term.t_ncol && TTeeol() != Success)
		return rc.status;
Finish:
...
DelChar:
	...
	if(!(isp->flags & Term_NoKeyEcho)) {
		...
		if(cmd == tc_bs) {
			if(isp->lshift > 0 && si.mlcol - n <= isp->icol0) {
				if(mli_update(isp,false) != Success)
					return rc.status;
				goto Flush;
				}
			if(isp->ibufc == isp->ibufe && (isp->lshift == 0 || isp->clen0 > 0 || si.mlcol - isp->icol0 > 1)) {
#if MMDebug & Debug_MsgLine
fprintf(logfile,"mli_del(): quick delete of %d char(s).\n",n);
#endif
				do {
					if(mlputc(MLRaw,'\b') != Success || mlputc(MLRaw,' ') != Success ||
					 mlputc(MLRaw,'\b') != Success)
						return rc.status;
					} while(--n > 0);
				goto Flush;
				}
			...
			if(isp->ibufc == isp->ibufe && n == 1 &&
			 (isp->lshift == 0 || isp->clen0 > 0 || si.mlcol - isp->icol0 > 1)) {
...
fprintf(logfile,"mli_putc('%s',...)...\n",vizc(c,VBaseDef));
		...
		if((isp->flags & Term_NoKeyEcho))
			...
			isp->ibufc->len = len;
			++isp->ibufc;
			if(((si.mlcol + len < term.t_ncol - 1) ?
			 mli_write(isp,si.mlcol,1,false,NULL,si.mlcol + len) : mli_update(isp,false)) != Success)
				return rc.status;
			goto Flush;
			}
		}
...
static int cMatchCFAB(InpState *isp,uint selector,HashRec **hrpp) {
	short c;
	HashRec **hrpp1;
	UnivPtr *univp;
	UnivPtr *ematchp;	// Last entry that matched string.
	bool comflag;		// Was there a completion at all?
	uint n = isp->ibufe - isp->ibuf;
	HashRec **hrppz = hrpp + exectab->recCount;
	...
	for(;;) {
		...
		do {
			...
			// Is this a match?
			if(!(univp->p_type & selector) || (n > 0 && !ibufcmp(isp,(*hrpp1)->key)))
				goto NextEntry;

			// A match.  If this is the first match, simply record it.
			if(ematchp == NULL) {
				ematchp = univp;
				c = (*hrpp1)->key[n];	// Save next char.
				}
			else if((*hrpp1)->key[n] != c)
	...
	for(;;) {
		...
		while((datp = aeach(&aryp)) != NULL) {
			...
			if((selector & (Term_C_GMode | Term_C_BMode)) != (Term_C_GMode | Term_C_BMode) &&
			 (msp->ms_flags & MdGlobal) != modetype)
				continue;

			// Is this a match?
			if(n > 0 && !ibufcasecmp(isp,msp->ms_name))
				goto NextEntry;

			// A match.  If this is the first match, simply record it.
			if(ematchp == NULL) {
				ematchp = msp;
				c = msp->ms_name[n];	// Save next char.
				}
			else if(msp->ms_name[n] != c)
...
static int cListCFAB(InpState *isp,Buffer **bufpp,uint selector,HashRec **hrpp) {
	UnivPtr *univp;
	HashRec **hrppz = hrpp + exectab->recCount;
	...
	do {
		...
		if((univp->p_type & selector) && ibufcmp(isp,(*hrpp)->key))
			if(bappend(*bufpp,(*hrpp)->key) != Success)
				return rc.status;
...
static int cListMode(InpState *isp,Buffer **bufpp,uint selector) {
	...
	while((datp = aeach(&aryp)) != NULL) {
		...
		// Add name to the buffer if it's a match...
		if(ibufcasecmp(isp,msp->ms_name))
			if(bappend(*bufpp,msp->ms_name) != Success)
				return rc.status;
		}
...
// Get one key from terminal and save in rp as a key literal or character (int).  Return status.
static int terminp1(Datum *rp,char *defval,ushort delim,uint delim2,uint aflags,uint cflags) {
	...
	if(((cflags & Term_KeyMask) == Term_OneKeySeq ?
	 getkseq(&ek,NULL,true) : getkey(&ek,true)) != Success)		// Get a key.
		return rc.status;
...
SetKey:
			dsetint(ektoc(ek,false),rp);			// No, save (raw) key and return.
		}
	else {
		char keybuf[16];
...
// used.  Recognized argument flags are ArgNil1, ArgNIS1, and ArgNotNull1.  Control flags are prompt options + type of
...
int terminp(Datum *rp,char *prmt,uint aflags,uint cflags,TermInp *tip) {
	...
	// Build prompt.
	if(buildprompt(prompt,prmt,(cflags & Term_LongPrmt) ? 90 : si.maxprmt,(cflags & Term_KeyMask) == Term_OneChar &&
	...
	Datum *datp;
	HashRec **hrpp;			// For command completion.
				...
				if(ek == (Ctrl | 'I')) {		// Tab... attempt a completion.
					switch(cflags & Term_C_Mask) {
						case Term_C_Buffer:
							...
							break;
						case Term_C_CFAM:
							n = cMatchCFAB(&istate,cflags,hrpp);
							break;
...
CompList:
					...
					switch(cflags & Term_C_Mask) {
						case Term_C_Buffer:
							...
							break;
						case Term_C_CFAM:
							n = cListCFAB(&istate,&bufp,cflags,hrpp);
							break;
						case Term_C_Filename:
...
int terminpYN(char *prmt,bool *resultp) {
	...
	for(;;) {
		...
		if(TTbeep() != Success || mlerase() != Success)
			break;
		}
	return rc.status;
	}
...
int getnarg(Datum *rp,char *prmt) {
	...
	if(si.opflags & OpScript) {
		if(funcarg(rp,ArgFirst | ArgInt1) != Success)
			return rc.status;
		}
	else if(terminp(rp,prmt,ArgNil1,0,NULL) != Success || rp->d_type == dat_nil)
		return rc.status;
...
int getcfam(char *prmt,uint selector,UnivPtr *univp,char *emsg) {
	...
	else if(terminp(datp,prmt,ArgNotNull1 | ArgNil1,Term_C_CFAM | selector,NULL) != Success)
		return rc.status;
	...
	if(datp->d_type != dat_nil) {
		...
		if(selector & PtrMacro)
			selector |= PtrMacro;
		return execfind(datp->d_str,OpQuery,selector,univp) ? rc.status : rcset(Failure,0,emsg,datp->d_str);
		}
...
int bcomplete(Datum *rp,char *prmt,char *defname,uint flags,Buffer **bufpp,bool *createdp) {
	...
	if(!(si.opflags & OpScript)) {
		...
		sprintf(pbuf,"%s %s",prmt,text83);
				// "buffer"
		if(terminp(rp,pbuf,ArgNotNull1 | ArgNil1,Term_C_Buffer,&ti) != Success)
			return rc.status;
	...
	int status = bfind(rp->d_str,(flags & OpCreate) ? BS_Create | BS_Hook : BS_Query,0,bufpp,createdp);
...
int uprompt(Datum *rp,int n,Datum **argpp) {
	...
	if(dnewtrk(&prmtp) != 0)
		goto LibFail;
	if(funcarg(prmtp,ArgFirst | ArgNil1) != Success)
		return rc.status;
	...
	if(havesym(s_comma,false)) {
		...
		if(dnewtrk(&defargp) != 0)
			goto LibFail;
		if(funcarg(defargp,ArgNIS1) != Success)
			return rc.status;
...
PrmtErr:
		...
		if(defargp->d_type != dat_nil) {
			if(cflags & Term_OneChar) {
				if(!charval(defargp))
					return rc.status;
				dsetchr(defargp->u.d_int,defargp);
				}
			else if(tostr(defargp) != Success)
				return rc.status;
	...
	// Prompt for input.
	return terminp(rp,prmtp->d_type == dat_nil ? "" : prmtp->d_str,ArgNil1,cflags,&ti);
LibFail:
	return librcset(Failure);
	}
