...
extern bool charval(Datum *datp);
extern int chktab(int size,bool hard);
#if MMDebug & (Debug_Datum | Debug_Script | Debug_MacArg | Debug_PPBuf)
extern int debug_execbuf(Datum *rp,int n,struct Buffer *bufp,char *runpath,uint flags,char *caller);
#endif
...
extern int execestmt(Datum *rp,char *cl,short termch,char **clp);
...
extern bool ishook(Buffer *bufp,bool isError);
