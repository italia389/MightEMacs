...
void initInfoColors(void) {
	...
	if(si.opflags & OpHaveColor) {
		...
		if(term.infoColor[0] == -2) {
			fg = term.colorWhite;
			bg = term.colorGreen;
			line = term.colorGreen;
			}
		else {
			fg = term.infoColor[0];
			bg = term.infoColor[1];
...
int rpthdr(DStrFab *rptp,char *title,bool plural,char *colhead,ColHdrWidth *wp,int *widthp) {
	...
	if(wp == NULL) {
		...
		if((si.opflags & OpHaveColor) && dputf(rptp,"%c%hd%c",AttrSpecBegin,term.maxWorkPair - ColorPairIH,
		 AttrColorOn) != 0)
			goto LibFail;
		}
	else {
...
MatchFound:
		...
		if((flags & SHSepLine) || firstItem) {
			if(dputc('\n',&scp->sc_rpt) != 0)
				goto LibFail;
			if((si.opflags & OpHaveColor) && dputf(&scp->sc_rpt,"%c%hd%c",AttrSpecBegin,
			 term.maxWorkPair - ColorPairISL,AttrColorOn) != 0)
				goto LibFail;
			if(dputs(sepline,&scp->sc_rpt) != 0)
				goto LibFail;
			if((si.opflags & OpHaveColor) && dputf(&scp->sc_rpt,"%c%c",AttrSpecBegin,AttrColorOff) != 0)
				goto LibFail;
			}
		firstItem = false;
