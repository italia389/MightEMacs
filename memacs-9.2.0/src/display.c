...
int sepline(int len,DStrFab *sfp) {
	...
	if(dputc('\n',sfp) != 0)
		return -1;
	return (si.opflags & OpHaveColor) ? dputf(sfp,"%c%hd%c%s%c%c",AttrSpecBegin,term.maxWorkPair - ColorPairISL,AttrColorOn,
	 line,AttrSpecBegin,AttrColorOff) : dputs(line,sfp);
	}
...
int showColors(Datum *rp,int n,Datum **argpp) {
	...
	if(n <= 0 && n != INT_MIN) {
		...
		do {
			...
			(void) init_pair(pair - 2,term.colorWhite,c);
