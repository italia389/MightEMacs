...
#define CFLit_getInfo		"Return informational item for given keyword: \"Buffers\" -> names of visible buffers as an\
 array (plus hidden ones if n <= 0, all if n > 0), \"Colors\" -> number of colors and color pairs available as a two-element\
 array (or nil if none), \"Editor\" -> editor name, \"Hooks\" -> hook list in form [[hook-name,macro-name],...] (where\
 macro-name is nil if hook not set), \"Language\" -> language of text messages, \"Modes\" -> mode list in form [[mode-name,\
group-name,user?,global?,hidden?,scope-lock?,active?],...] (where group-name is nil if mode is not in a group, and last five\
 elements are Boolean values), \"OS\" -> operating system name, \"Screens\" -> screen list in form [[screen-num,wind-count,\
work-dir],...], \"Version\" -> editor version, or \"Windows\" -> window list in form [[windNum,bufName],...] (or form\
 [[screenNum,windNum,bufName],...] if n argument)."
...
#define VLit_displayColors	"Three-element array containing color pairs for informational displays, mode line color, and\
 keyboard macro recording indicator.  Each array element is itself a two-element array of the desired foreground and background\
 colors, or nil for the default (white on green for informational displays, reverse video for mode line, and white on red for\
 recording indicator)."
 ...
 *help0,*help1,*help2[],*usage[],
 ...
 text435[],text436[],text437[],text438[],text439[],text440[],text441[],text442[];
#if MMDebug & Debug_ShowRE
extern char
