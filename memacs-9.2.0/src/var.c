			...
			if(mkarray(rp,&aryp0) == Success) {
				...
				do {
					...
					if(cp[0] == -2)
						...
						if(mkarray(&dat0,&aryp1) != Success)
							goto Retn;
						cpz = cp + 2;
						do {
							dsetint((long) *cp++,&dat1);
							if(apush(aryp1,&dat1,true) != 0)
								goto LibFail;
							} while(cp < cpz);
						}
					if(apush(aryp0,&dat0,true) != 0)
						goto LibFail;
					} while(*cpp != NULL);
				}
			}
...
// Return true if given Datum object (array) contains valid values for $displayColors system variable (and store integers
// values in "cp" array); otherwise, set an error and return false.
static bool validPairs(Datum *datp,short *cp) {
	Datum **elpp0,**elpp1,**elppz0,**elppz1,*elp;
	long c;
	Array *aryp = awptr(datp)->aw_aryp;

	if(aryp->a_used != 3)
		goto Err;
	elppz0 = (elpp0 = aryp->a_elpp) + 3;
	do {
		elp = *elpp0++;
		if(elp->d_type == dat_nil) {
			*cp++ = -2;
			*cp++ = -2;
			}
		else if(!aryval(elp) || !haveColor())
			return false;
		else {
			aryp = awptr(elp)->aw_aryp;
			if(aryp->a_used != 2)
				goto Err;
			elppz1 = (elpp1 = aryp->a_elpp) + 2;
			do {
				elp = *elpp1++;
	...
	return true;
Err:
	(void) rcset(Failure,RCNoFormat,text422);
...
BadTyp2:
			...
			switch(svp->sv_id) {
				...
				case sv_displayColors:
					...
					if(validPairs(datp,c)) {
						if(c[0] != term.infoColor[0] || c[1] != term.infoColor[1]) {
							term.infoColor[0] = c[0];
							term.infoColor[1] = c[1];
							supd_wflags(NULL,WFMode);
							}
						if(c[2] != term.mdlColor[0] || c[3] != term.mdlColor[1]) {
							term.mdlColor[0] = c[2];
							term.mdlColor[1] = c[3];

							// Last color pair is reserved for mode line.
							if(c[2] != -2)
								(void) init_pair(term.maxPair - ColorPairML,c[2],c[3]);
							supd_wflags(NULL,WFMode);
							}
						if(c[4] != term.mdlRecInd[0] || c[5] != term.mdlRecInd[1]) {
							term.mdlRecInd[0] = c[4];
							term.mdlRecInd[1] = c[5];

							// Second to last color pair is reserved for keyboard macro recording
							// indicator.
							if(si.opflags & OpHaveColor) {
								...
								(void) init_pair(term.maxPair - ColorPairKMRI,c[4],c[5]);
								}
							}
						}
					}
	...
	if(vd.vd_type == VTyp_SVar) {
		if(vd.p.vd_svp->sv_flags & (V_GetKey | V_GetKeySeq)) {
			if(getsvar(datp,vd.p.vd_svp) != Success)
				goto Retn;
			if(dputf(&sf,"%c%c%s%c%c",AttrSpecBegin,AttrAlt,AttrULOn,datp->d_str,AttrSpecBegin,AttrULOff) != 0)
				goto LibFail;
			cflags |= Term_Attr;
			}
...
LibFail:
	...
	if(terminp(datp,rp->d_str,aflags,cflags,&ti) != Success)
		goto Retn;

	// Evaluate result as an expression if requested.  Type checking is done in putvar() so no need to do it here.
	if(n != INT_MIN) {
		if(execestmt(rp,datp->d_str,TokC_ComLine,NULL) != Success)
			goto Retn;
		}
	else if((datp->d_type & DStrMask) && (vd.vd_type == VTyp_GVar ||
	 (vd.vd_type == VTyp_SVar && (vd.p.vd_svp->sv_flags & V_Int))) && asc_long(datp->d_str,&lval,true))
		dsetint(lval,rp);
	...
	switch(req) {
		...
		default: // SHReqValue
			if(svp->sv_flags & (V_GetKey | V_GetKeySeq)) {
				...
				if(dnewtrk(&datp) != 0)
					goto LibFail;
				if(getsvar(datp,svp) == Success && dputf(&scp->sc_rpt,"%c%c%c%s%c%c",
				 AttrSpecBegin,AttrAlt,AttrULOn,datp->d_str,AttrSpecBegin,AttrULOff) != 0)
LibFail:
					(void) librcset(Failure);
				}
			else
