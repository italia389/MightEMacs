...
bool exactbmode(void) {
	...
	return (srch.m.flags & SOpt_Exact) || ((modetab[MdIdxGlobal].flags & MdExact) && !(srch.m.flags & SOpt_Ignore));
	}

// Return true if buffer Match record specifies Regexp search mode.
bool rebmode(void) {

	return (srch.m.flags & SOpt_Regexp) || ((modetab[MdIdxGlobal].flags & MdRegexp) && !(srch.m.flags & SOpt_Plain));
	}
...
int boundary(Dot *dotp,int dir) {
	...
	return (dir == Forward) ? dotp->lnp == curbp->b_hdrlnp : dotp->off == 0 && dotp->lnp->l_prevp == curbp->b_hdrlnp;
	}
...
static int nextch(ScanDot *sdotp,int direct) {
	...
	if(sdotp->type == ScanDot_Str) {
		...
		if(direct == Forward) {					// Going forward?
			...
			if(dotp->lnp->l_prevp == curbp->b_hdrlnp)		// Yes.  At top of buffer?
				c = -1;					// Yes, return -1.
			else {
...
static bool bjump(int jumpsz,Dot *scandotp,int direct) {
	...
	if(direct == Forward) {						// Going forward.
		...
		scandotp->off += jumpsz;
		spare = scandotp->off - scandotp->lnp->l_used;		// Overflow size.

		while(spare > 0) {					// Skip to next line.
			if((scandotp->lnp = scandotp->lnp->l_nextp) == curbp->b_hdrlnp && spare > 1)
				return false;				// Hit end of buffer.
			scandotp->off = spare - 1;			// Line delimiter.
			spare = scandotp->off - scandotp->lnp->l_used;
			}
		}
	else {								// Going backward.
		scandotp->off -= jumpsz;
		while(scandotp->off < 0) {				// Skip back a line.
			if((scandotp->lnp = scandotp->lnp->l_prevp) == curbp->b_hdrlnp)
				return false;				// Hit beginning of buffer.
			scandotp->off += scandotp->lnp->l_used + 1;
			}
		}
...
Fail:
		...
		if(loopCount <= ProgressLoopCt) {
			if(loopCount++ == ProgressLoopCt)
				if(mlputs(MLHome | MLWrap | MLForce | MLFlush,text203) != Success)
						// "Searching..."
					return rc.status;
			}
		}
...
static bool isWordCh(ScanDot *sdotp,int n) {
	...
	if(sdotp->type == ScanDot_Buf) {
		Dot *dotp = &sdotp->u.bd;
		if(n < 0)
			return dotp->off == 0 ? (dotp->lnp->l_prevp != curbp->b_hdrlnp && wordlist['\n']) :
			 wordlist[(int) dotp->lnp->l_text[dotp->off - 1]];
		else
			return dotp->off == dotp->lnp->l_used ? dotp->lnp != curbp->b_hdrlnp && wordlist['\n'] :
			...
			case MCE_EOSAlt:
				if(sdot.type == ScanDot_Buf) {
					...
					case MCE_EOS:
						...
						else if(dotp->off != 0 || dotp->lnp == curbp->b_hdrlnp->l_nextp)
							return false;
						break;
					default:	// MCE_EOSAlt
						if(flags & SOpt_Multi) {
							if(boundary(dotp,Forward) || dotp->lnp != curbp->b_hdrlnp->l_prevp ||
							 dotp->off != dotp->lnp->l_used)
								return false;
							}
