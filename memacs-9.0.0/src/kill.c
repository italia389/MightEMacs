...
// Attempt to delete n entries from given ring, beginning at top.  It is assumed that ringp->r_entryp != NULL and n > 0.  Return number of
// entries actually deleted.
static ushort rtrim(Ring *ringp,ushort n) {
	RingEntry *rep;
...
static int iorch(short c,ushort t) {
	...
	if(!(t & Txt_Insert) && dotp->off < dotp->lnp->l_used) {
		...
		if((t & Txt_Replace) || dotp->lnp->l_text[dotp->off] != '\t' || getccol(NULL) % htabsize == (htabsize - 1)) {
...
int iorstr(Datum *srcp,int n,ushort style,bool kill) {
	...
	// If n == 0, remember current dot.  However, must save the *previous* line, since the line we are on may disappear due
	// to re-allocation.  This works even if we are on the first line of the file.
	if((pretext = n == 0)) {
		curline = dotp->lnp->l_prevp;
		curoff = dotp->off;
		n = 1;
		}
	...
	// If requested, set point back to the beginning of the new text and move RMark.
	if(pretext) {
		if(kill)
			mset(&curbp->b_mroot,curwp);
		dotp->lnp = curline->l_nextp;
		dotp->off = curoff;
		}
