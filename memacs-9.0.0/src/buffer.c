...
Buffer *bdefault(ushort flags) {
	...
	bufp = (flags & BDefBack) ? curbp->b_prevp : curbp->b_nextp;
	for(;;) {
		if(bufp == NULL)
			bufp = (flags & BDefBack) ? btailp : bheadp;
		else if(bufp == curbp)
			return NULL;
		 ...
		 (!(bufp->b_flags & BFMacro) && (flags & (BDefHidden | BDefTwo)) == BDefHidden))
			...
			bufp = (flags & BDefBack) ? bufp->b_prevp : bufp->b_nextp;
		}
...
static void mrkoff(void) {
	...
	for(lnp = curbp->b_hdrlnp->l_nextp; lnp != curbp->b_hdrlnp; lnp = lnp->l_nextp) {
...
int narrowBuf(Datum *rp,int n,Datum **argpp) {
	...
	do {
		if((dotp->lnp = dotp->lnp->l_nextp) == curbp->b_hdrlnp)
			break;
		} while(--n > 0);
	...
	curbp->b_flags |= BFNarrowed;

	return dsetstr(curbp->b_bname,rp) != 0 ? librcset(Failure) : rcset(Success,0,text73,text58);
								// "%s narrowed","Buffer"
	}
...
//			run "createBuf" hook if CRBHook is set, set *bufpp (if bufpp not NULL) to the buffer pointer, set
...
// In all cases, if a buffer is created and CRBHook is set, the "createBuf" hook is executed before returning.
int bfind(char *name,ushort cflags,ushort bflags,Buffer **bufpp,bool *createdp) {
	...
	if(cflags & CRBCreate) {
		...
		bufp2->b_lastscrp = NULL;
		bufp2->b_fname = NULL;
		strcpy(bufp2->b_bname,bname);

		// Add macro name to the CFAM list.
		if(bflags & BFMacro)
		...
		return (cflags & CRBHook) ? exechook(NULL,INT_MIN,hooktab + HkCreateBuf,1,bname) : rc.status;
		}
...
int bclear(Buffer *bufp,ushort flags) {
	...
	if((bufp->b_flags & BFChanged) && !(flags & CLBIgnChgd) && (!(bufp->b_flags & BFNarrowed) || (flags & CLBUnnarrow))) {
		char *str,prmt[NWork];
		eraseML = true;
	...
	while((lnp = bufp->b_hdrlnp->l_nextp) != bufp->b_hdrlnp)// Free all Line storage.
		lfree(lnp);
	bchange(bufp,WFHard | WFMode);				// Update window flags.
	if(bufp->b_flags & BFNarrowed)
		bufp->b_flags |= BFChanged;			// Mark as changed if narrowed...
	else
...
// Change one or more buffer attributes (flags), given result pointer and action (n < -1: clear all, n == -1: clear, n == 0
// (default): toggle, n == 1: set, or n > 1: clear all and set).  Mutable attributes are: "Changed", "Hidden", and "TermAttr".
// If interactive:
...
//	. Get one or more attribute (keyword) arguments.
// Perform operation, set rp to former state (-1 or 1) of last attribute altered, and return status.
int alterBufAttr(Datum *rp,int n,Datum **argpp) {
	Buffer *bufp;
	long formerState;
	BufFlagSpec *bfsp;
	Datum *datp;
	ushort oldflags,newflags,newflags0,wflags;
	char *keyword;
	...
	// Get attribute if interactive and not "clear all".
	if(!(opflags & OpScript)) {
		if(action >= -1) {
			DStrFab prompt;
			char *attrname;
			...
			if(terminp(datp,prompt.sf_datp->d_str,CFNotNull1 | CFNil1,Term_LongPrmt | Term_Attr | Term_OneChar,
			 NULL) != Success || datp->d_type == dat_nil)
				return rc.status;
		...
		// "Clear all" request or n not the default.  Get buffer name.  If nothing is entered, bag it.
		bufp = bdefault(BDefTwo);
		if(bcomplete(rp,action < -1 ? text377 : text229 + 2,bufp != NULL ? bufp->b_bname : NULL,OpDelete,&bufp,NULL)
		 != Success || rp->d_type == dat_nil)
					// "Clear attributes in",", in"
			return rc.status;
		}
	else {
...
Process:
	...
	if(action >= -1) {
		...
		// If script mode, get attribute arguments.
		if(!(opflags & OpScript))
			goto DoFlag;
		newflags = 0;
		first = true;
		do {
			if(!first && !havesym(s_comma,false))
				break;				// At least one argument retrieved and none left.
			if(funcarg(datp,CFNotNull1) != Success)
				return rc.status;
			first = false;
...
DoFlag:
			...
			return rcset(Failure,0,text374,opflags & OpScript ? keyword : vizc(*keyword,VBaseDef));
...
Done:
	...
	// Wrap it up.
	return (opflags & OpScript) ? rc.status : (newflags == BFChanged) ? mlerase(0) : rcset(Success,RCNoFormat,text375);
												// "Attribute(s) changed"
	}
...
int bufop(Datum *rp,int n,char *prmt,uint op,int flag) {
	...
	switch(op) {
		case BOpBeginEnd:
			...
			dotp->off = 0;
			dotp->lnp = flag ? bufp->b_hdrlnp : bufp->b_hdrlnp->l_nextp;
			break;
		case BOpGotoLn:
			// Go to beginning of buffer and count lines.
			dotp->lnp = bufp->b_hdrlnp->l_nextp;
			dotp->off = 0;
			if(bufp == curbp)
...
int sysbuf(char *root,Buffer **bufpp,ushort flags) {
	...
	bname[0] = BSysLead;
	strcpy(bname + 1,root);
	return bfind(bname,CRBCreate | CRBForce,BFHidden | flags,bufpp,NULL);
	}
...
int bactivate(Buffer *bufp) {
	...
	if(!(bufp->b_flags & BFActive)) {
		...
		(void) readin(bufp,NULL,RWKeep | RWStats);
		}
...
int insertBuf(Datum *rp,int n,Datum **argpp) {
	...
	curbp->b_flags |= BFChanged;			// Mark as changed.
	curbp->b_mroot.mk_rfrow = getwpos(curwp);	// Keep existing framing.
	wfp->wf_dot.lnp = wfp->wf_dot.lnp->l_prevp;	// Back up a line and save dot in mark RMark.
	wfp->wf_dot.off = 0;
	curbp->b_mroot.mk_dot = wfp->wf_dot;

	// Insert each line from the buffer at point.
	nline = 0;
	while((buflnp = buflnp->l_nextp) != bufp->b_hdrlnp) {
		if(lalloc(nbytes = buflnp->l_used,&lnp1) != Success)
			return rc.status;		// Fatal error.
...
int dopop(Datum *rp,int n,bool popbuf) {
	...
	if(popbuf) {
		...
		bool created;
		ushort cflags = n <= 0 && n != INT_MIN ? CRBCreate | CRBFile : CRBCreate | CRBFile | CRBForce;
		...
		if(gtfilename(rp,text299,NULL,0) != Success || rp->d_type == dat_nil ||
		 ...
		 readin(bufp,rp->d_str,RWKeep | RWExist) != Success))
			return rc.status;
	...
	if(n != INT_MIN) {
		...
		else if(terminp(rp,term.t_ncol >= 80 ? text223 : text224,CFNotNull1,Term_LongPrmt | Term_Attr,NULL) != Success)
...
int bscratch(char *dest,Buffer **bufpp) {
	...
	do {
		...
		if(bfind(dest,CRBCreate | CRBHook,0,&bufp,&created) != Success)
			return rc.status;
	...
	(void) bfind(dest,CRBCreate | CRBHook | CRBForce,0,&bufp,NULL);
Retn:
	*bufpp = bufp;
	return rc.status;
	}
...
int bswitch(Buffer *bufp) {
	...
	if(--curbp->b_nwind == 0)
		curbp->b_lastscrp = cursp;
	wftobf(curwp,curbp);
	cursp->s_lastbufp = curbp;
...
int bdelete(Buffer *bufp,ushort flags) {
	...
	if(savbufp == bufp)				// Unsave buffer if saved.
		savbufp = NULL;
	if(cursp->s_lastbufp == bufp)			// Clear "last buffer".
		cursp->s_lastbufp = NULL;
...
// and use it, without prompting; otherwise, get new buffer name, using Arg_First flag in script mode if n is not the default.
// Return status.
int brename(Datum *rp,int n,Buffer *targbufp) {
	Buffer *bufp;
	char *prmt;
	Datum *bnamep;			// New buffer name.
	TermInp ti = {NULL,RtnKey,NBufName,NULL};
	char askstr[NWork];
...
Ask:
	...
	if(opflags & OpScript) {
		if(funcarg(bnamep,n == INT_MIN ? Arg_First | CFNotNull1 : CFNotNull1) != Success)
			return rc.status;
		}
	else if(terminp(bnamep,prmt,CFNotNull1 | CFNil1,0,&ti) != Success || bnamep->d_type == dat_nil)
		return rc.status;
...
SetNew:
	...
	if((targbufp->b_flags & BFMacro) &&
	 amfind(targbufp->b_bname + 1,OpDelete,0) != Success)	// Remove old macro name from CFAM list...
		return rc.status;
	strcpy(targbufp->b_bname,bnamep->d_str);		// copy new buffer name to structure...
	relistbuf();						// reorder buffer in list...
	if((targbufp->b_flags & BFMacro) &&
	 amfind(targbufp->b_bname + 1,OpCreate,PtrMacro) != Success)// add new macro name to CFAM list...
		return rc.status;
...
int renameBuf(Datum *rp,int n,Datum **argpp) {
	...
	if(n == INT_MIN)
		bufp = curbp;
	else if(bcomplete(rp,text29,n > 0 ? curbp->b_bname : (bufp = bdefault(BDefTwo)) != NULL ? bufp->b_bname : NULL,
...
// non-displayed buffers as well and does not set the BFChanged buffer flag.
