...
EWindow *whasbuf(Buffer *bufp,bool skipCur) {
	EWindow *winp = wheadp;
	do {
		if(winp->w_bufp == bufp && (winp != curwp || !skipCur))
			return winp;
		} while((winp = winp->w_nextp) != NULL);
	return NULL;
	}
...
int mlerase(ushort flags) {
	...
	if(!(modetab[MdIdxGlobal].flags & MdMsg) && !(flags & MLForce))
		return rc.status;
	...
	if((flags & MLRaw) || (c >= ' ' && c <= '~')) {
		...
		if(c == '\b')
			...
			if(skipSpace && TTul(false) != Success)
				return rc.status;
			if(mlcol == term.t_ncol - 1 && !(flags & MLNoEOL))
				c = LineExt;
			}
		if(TTputc(c) != Success)
			return rc.status;
		...
		do {
			if(TTputc(mlcol == term.t_ncol - 1 && !(flags & MLNoEOL) ? LineExt : c) != Success)
				break;
			} while(++mlcol < term.t_ncol && (c = *++str) != '\0');
		}

	return rc.status;
	}

// Prepare for new message line message.  If MLHome flag is set, clear message line.  If MLForce flag is set, write string
// regardless of the 'msg' global flag setting.  If MLWrap flag is set, write leading '[' of message-wrap characters "[]".
// Return rc.status (Success) if successful; otherwise, Cancelled (bypassing rcset()).
static int mlbegin(ushort flags) {

	// If we are not currently echoing on the command line and it's not a force, skip this.
	if(!(modetab[MdIdxGlobal].flags & MdMsg) && !(flags & MLForce))
		return Cancelled;

	// Position cursor and/or begin wrap, if applicable.
	if(((flags & MLHome) || mlcol < 0 || mlcol >= term.t_ncol) && mlerase(flags) != Success)
		return rc.status;
...
static int mlputstr(ushort flags,char *str) {
	...
	while((c = *str++) != '\0')
		if(mlputc(flags,c) != Success || mlcol >= term.t_ncol)
			break;
...
int mlputs(ushort flags,char *str) {
	...
	// If we are not currently echoing on the command line and it's not a force, skip this.
	if(mlbegin(flags) == Cancelled || rc.status != Success)
		return rc.status;
...
// Initialize point position, marks, and first column position of a face record, given line pointer.  If bufp is not NULL, reset
// its buffer marks.
...
bool wnewtop(EWindow *winp,Line *lnp,int n) {
	...
	if(n < 0) {
		while(lnp->l_prevp != winp->w_bufp->b_hdrlnp) {
			lnp = lnp->l_prevp;
			if(++n == 0)
				break;
			}
		...
		while(lnp->l_nextp != winp->w_bufp->b_hdrlnp) {
			lnp = lnp->l_nextp;
			if(--n == 0)
				break;
			}
...
static int getnum(char *prmt,bool screen,int *np) {
	...
	if(opflags & OpScript) {
		...
		sprintf(wkbuf,"%s %s (1-%d)",prmt,screen ? text380 : text331,screen ? scrcount() : wincount(cursp,NULL));
						// "screen","window"
...
int selectWind(Datum *rp,int n,Datum **argpp) {
	...
	// Get window number.
	if(wheadp->w_nextp == NULL && !(opflags & OpScript))
...
// Return true if dot in in current window; otherwise, false.
bool dotinwind(void) {
...
int onlyWind(Datum *rp,int n,Datum **argpp) {
	...
	wnewtop(curwp,NULL,-curwp->w_toprow);
	curwp->w_toprow = 0;
	...
	if(wheadp->w_nextp->w_nextp != NULL && ((curwp == wheadp && n != INT_MIN && n < -1) ||
	 (curwp->w_nextp == NULL && n > 1))) {
		...
		if(curwp == wheadp) {
			...
			rows = -(curwp->w_nrows + 1);			// Top row adjustment for all windows.
			cursp->s_wheadp = wheadp = curwp->w_nextp;	// Remove current window from list.
			}
		else {
			targwinp = wheadp;
			rows = curwp->w_nrows + 1;
		...
		if(winp == NULL || (n > 0 && curwp->w_nextp != NULL)) {		// Next window down.
			...
			wnewtop(targwinp,NULL,-(curwp->w_nrows + 1));
			}
		else {								// Next window up.
			targwinp = winp;
...
int joinWind(Datum *rp,int n,Datum **argpp) {
	...
	else if(curwp == wheadp || (n > 0 && curwp->w_nextp != NULL)) {
...
int getwid(ushort *widp) {
	...
	if((scrp = sheadp) == NULL)
		...
		// Get count of all windows in all screens and add it to the last user mark value.
		do {
			// In all windows...
			winp = scrp->s_wheadp;
			do {
...
// abs(n) lines; if n > 0, the upper window is set to n lines.  Set *winpp to the new window not containing dot.  Return status.
int wsplit(int n,EWindow **winpp) {
	...
	if(nrowdot < curwp->w_nrows) {
		...
		if(lnp == curbp->b_hdrlnp) {
			...
			if((lnp = wfp->wf_dot.lnp) != curbp->b_hdrlnp && lnp != curbp->b_hdrlnp->l_prevp) {
				winp->w_face.wf_dot.lnp = curbp->b_hdrlnp->l_prevp;
			...
			nrowu = nrowl / 2;
			while(nrowu-- > 0) {
				if((lnp = lnp->l_nextp) == curbp->b_hdrlnp)
					break;
				}
	
			// Set dot line to mid-point of lower window or last line of buffer.
			winp->w_face.wf_dot.lnp = (lnp == curbp->b_hdrlnp) ? lnp->l_prevp : lnp;
			winp->w_face.wf_dot.off = 0;
			}
		}
	else {
		...
		winp->w_face.wf_toplnp = (lnp == curbp->b_hdrlnp) ? lnp->l_prevp : lnp;
...
// the hard work.  (Can't just set "force reframe" because dot would move.)  Return status.
...
int render(Datum *rp,int n,Buffer *bufp,ushort flags) {
	...
	if(n != 0) {
		...
		// Not popping buffer.  Switch to it?
			...
			if(bufp != curbp && bswitch(bufp) != Success)
				return rc.status;
			winp = curwp;
			}
		...
		// No, displaying buffer in another window.
			...
			if(n > 1 || wheadp->w_nextp == NULL) {		// If force-creating or only one window...
			...
			else if((winp = wnextis(curwp)) == NULL)	// or the window above, if none found.
				winp = curwp->w_nextp;
			...
			if(bswitch(bufp) != Success)			// and switch to new buffer.
				return rc.status;
			if(flags & RendReset)
				faceinit(&winp->w_face,
				 bufp->b_hdrlnp->l_nextp,NULL);
...
static void dumpbuffer(Buffer *bufp) {
	...
	mkp = &bufp->b_mroot;
	do {
		fprintf(logfile,"\t\t'%s': [%s] %d %hd\n",mkp->mk_id,lninfo(mkp->mk_dot.lnp),mkp->mk_dot.off,mkp->mk_rfrow);
		} while(++i < NMARKS);

fprintf(logfile,"\tb_face.wf_firstcol: %d\n\tb_nwind: %hu\n\tb_nexec: %hu\n\tb_nalias: %hu\n\tb_flags: %.04x\n\tb_modes: %.04x\n",
	 bufp->b_face.wf_firstcol,bufp->b_nwind,bufp->b_mip == NULL ? -1H : bufp->b_mip->mi_nexec,bufp->b_nalias,
...
fprintf(logfile,"\t\tw_face.wf_dot.lnp: %s\n\t\tw_face.wf_dot.off: %d\n\t\tw_toprow: %hu\n\t\tw_nrows: %hu\n\t\tw_rfrow: %hu\n",
	 lninfo(winp->w_face.wf_dot.lnp),winp->w_face.wf_dot.off,winp->w_toprow,winp->w_nrows,winp->w_rfrow);
...
int sswitch(EScreen *scrp) {
	...
	if(strcmp(scrp->s_wkdir,cursp->s_wkdir) != 0)
		(void) changedir(scrp->s_wkdir);
...
int gotoScreen(int n,ushort flags) {
	...
	if(n != 0 || !(flags & EScrWinRepeat)) {
		...
		char olddir[strlen(cursp->s_wkdir) + 1];
		...
		if(flags & EScrWinRepeat) {
			...
			if(scrct == 1 || (n %= scrct) == 0)	// If only one screen or repeat count equals screen count...
				return rc.status;		// nothing to do.
			if(flags & EScrWinForw) {
				if((n += cursp->s_num) > scrct)
					n -= cursp->s_num;
				}
			else if((n = cursp->s_num - n) < 1)
				n += scrct;
			}
		...
		scrp = sheadp;					// Find the screen...
		while(scrp->s_num != n)
			scrp = scrp->s_nextp;
		strcpy(olddir,cursp->s_wkdir);			// save current directory...
		if(sswitch(scrp) == Success)			// make new screen current...
			...
			if(!(opflags & OpScript) && !(modetab[MdIdxGlobal].flags & MdWkDir) &&
			 strcmp(cursp->s_wkdir,olddir) != 0)
...
int selectScreen(Datum *rp,int n,Datum **argpp) {
	...
	if(n == INT_MIN) {
		...
		if(sheadp->s_nextp == NULL && !(opflags & OpScript))
...
int deleteScreen(Datum *rp,int n,Datum **argpp) {
	...
	if(scrp == cursp && sswitch(scrp == sheadp ? scrp->s_nextp : sheadp) != Success)
		return rc.status;
	...
	do {
		...
		do {
			...
			if(bufp->b_fname != NULL) {
				if(dputf(&rpt,"%-*s%s",(NBufName - 2),bufp->b_bname,bufp->b_fname) != 0)
					goto Fail;
				}
