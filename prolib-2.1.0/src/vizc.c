...
int fvizc(short c,ushort flags,FILE *file) {
	char *str;

	return (str = vizc(c,flags)) == NULL ? -1 : fputs(str,file) == EOF ? emsge(-1) : 0;
	}
...
int fvizs(const void *str,size_t len,ushort flags,FILE *file) {
	char *str1 = (char *) str;
	size_t n = (len == 0) ? strlen(str1) : len;

	for(; n > 0; --n)
		if(fvizc(*str1++,flags,file) != 0)
			return -1;
	return 0;
	}
