...
char
 ...
 text29[] = "Rename",
 text30[] = "~bACHMCPNT    Size       Buffer               File~0",
 text31[] = "Active",
 ...
 text58[] = "Buffer",
 text59[] = "Wrap column",
 text60[] = "Line %U/%U, Col %d/%d, Char %U/%U (%s%%), char = %s",
...
#define CFLit_alias		"Create alias name of command, function, or macro cfm.  Replace any existing alias if n > 0."
...
#define CFLit_definedQ		"Check if name (default), mark (n < 0), active mark (n == 0), mode (n == 1), or mode group\
 (n > 1) is defined and return keyword result or nil if not found, or Boolean result if n argument.  If n <= 0, name is\
 interpreted as an ASCII (int) character; otherwise, a string.  Returned keywords are: \"alias\", \"buffer\", \"command\",\
 \"function\", \"macro\", \"pseudo-command\" (prefix key), or \"variable\"."
...
macro-name],...] (where macro-name is nil if hook not set), \"Language\" -> language of text messages, \"Modes\" -> mode list\
 in form [[mode-name,group-name,user?,global?,hidden?,scope-lock?,active?],...] (where group-name is nil if mode is not in a\
 group, and last five elements are Boolean values), \"OS\" -> operating system name, \"Screens\" -> screen list in form\
 [[screen-num,wind-count,work-dir],...], \"Version\" -> editor version, or \"Windows\" -> window list in form\
 [[windNum,bufName],...] (or form [[screenNum,windNum,bufName],...] if n argument)."
...
#define CFLit_getWord		"Get previous (n < 0), current (n == 0, default), or next (n > 0) word from current line and\
 ...
 beginning of the word if n < 0 and at the end if n >= 0.  When going backward, point movement mimics the ~bbackWord~0 command\
...
#define CFLit_groupModeQ	"Check if a buffer mode in given group is set in named buffer (or a global mode if n argument)\
 and return its name if so; otherwise, nil."
...
#define CFLit_prompt		"Get terminal input and return string, ASCII (int) character, or nil result (with no echo if\
 ...
 (nil, ASCII character, int, or string), type (string: ~u~bb~0uffer completion, single ~u~bc~0haracter, ~u~bf~0ile completion,\
 ~u~bK~0ill ring access, ~u~bo~0ne key, ~u~bO~0ne key sequence, ~u~bR~0eplace ring access, ~u~bS~0earch ring access,\
 ~u~bs~0tring (default), mutable ~u~bv~0ariable completion, all ~u~bV~0ariables completion), and delimiter key literal.  All\
 arguments after the first are optional.  Any completion type letter may be preceded by ~u~b^~0 to suppress auto-completion. \
 ...
 ~#u[ ]~U if the prompt type is ~u~bc~0, and must be an ASCII character in that case; otherwise, it must be string or int and\
 is stored in the input buffer in string form as the initial value.  A key literal is returned for types ~u~bo~0 and ~u~bO~0;\
 ...
 prompt type is ~u~bc~0; otherwise, nil is returned.  If the user presses ~uC-SPC~U at any time, a null string is returned\
 instead (or a null character if the prompt type is ~u~bc~0); otherwise, the string in the input buffer is returned when the\
...
#define CFLit_showBuffers	"Generate list of visible buffers (or all buffers if n argument, excluding macros if n == -1)\
 ...
 buffer attributes: ~u~bA~0ctive, ~u~bC~0hanged, ~u~bH~0idden, ~u~bM~0acro, ~u~bC~0onstrained, ~u~bP~0recompiled,\
 ~u~bN~0arrowed, and ~u~bT~0ermAttr.  Returns: ~bselectBuf~0 values."
...
#define CFLit_showMacros	"Generate list of non-constrained macros (or all macros if n argument) and their bindings in a\
 new buffer and render it per ~bselectBuf~0 options (in a pop-up window if default n, matching pat if n argument).  If pattern\
 is plain text, match is successful if name contains pat.  Returns: ~bselectBuf~0 values."
...
#define CFLit_wrapLine		"Rewrap [-]n lines (default 1, region lines if n == 0) at $wrapCol, given a \"line prefix\"\
 string and \"end of sentence\" characters to pass to the ~bjoinLines~0 command.  All lines in the block are filled and/or\
 ...
 also recognized and removed from each line of the block before ~bjoinLines~0 is called.  When command is invoked\
...
#define HLitArg_none		"~b0:~0 (none)"
#define HLitArg_createBuf	"~b1:~0 Buffer name."
#define HLitArg_enterBuf	"~b1:~0 \"exitBuf\" return^  value."
#define HLitArg_mode		"~b2:~0 nil,prior value of^  $GlobalModes ~uOR~U^  buffer name,prior^  value of $BufModes."
#define HLitArg_postKey		"~b1:~0 \"preKey\" return^  value."
#define HLitArg_read		"~b2:~0 Buffer name,filename^  (or nil if standard^  input)."
#define HLitArg_write		"~b2:~0 Buffer name,filename."
#define HLitArg_filename	"~b2:~0 Buffer name,filename^  (or nil if none)."
