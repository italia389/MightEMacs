...
#define Debug_Temp	0x00040000	// For ad-hoc use.
...
#define OpStartup		0x0020		// In pre-edit-loop state or ignoring return messages?
...
#define MLNoSpace	0x0004		// Skip spaces when underlining in mlputc().
#define MLWrap		0x0008		// Wrap message [like this].
#define MLRaw		0x0010		// Output raw character; otherwise, convert to visible form if needed.
#define MLNoEOL		0x0020		// Don't overwrite char at right edge of screen with LineExt ($) char.
#define MLFlush		0x0040		// Flush output in mlputs() and mlprintf().
...
#define BS_File		0x0008		// Derive buffer name from filename.
...
#define RWFNHook	0x0004		// Run filename hook (buffer activation).
#define RWNoHooks	0x0008		// Do not run any hooks (filename or read).
#define RWScratch	0x0010		// Create scratch buffer.
#define RWStats		0x0020		// Return message containing I/O statistics.
...
//  4. If no argument type flag (ArgNilN, ArgBoolN, ArgIntN, or ArgArrayN) is set, the corresponding Nth argument must be string;
//     otherwise, if ArgIntN or ArgArrayN is set, the Nth argument must be (or may be if ArgMay set) that type; otherwise, the Nth
//     argument must be string, nil (if ArgNilN set), or Boolean (if ArgBoolN set).
...
enum cfid {
	...
	cf_endBuf,cf_endKeyMacro,cf_endLine,cf_endWhite,cf_endWord,cf_entabLine,cf_env,cf_eval,cf_exit,cf_filterBuf,cf_findFile,
	cf_forwChar,cf_forwLine,cf_forwPage,cf_forwPageNext,cf_forwPagePrev,cf_forwTab,cf_forwWord,cf_getInfo,cf_getKey,
	cf_getWord,cf_glob,cf_globalModeQ,cf_gotoFence,cf_gotoLine,cf_gotoMark,cf_groupModeQ,cf_growWind,cf_help,cf_huntBack,
	cf_huntForw,cf_includeQ,cf_indentRegion,cf_index,cf_insert,cf_insertBuf,cf_insertFile,cf_insertLineI,cf_insertPipe,
	cf_insertSpace,cf_inserti,cf_interactiveQ,cf_join,cf_joinLines,cf_joinWind,cf_keyPendingQ,cf_kill,cf_killFencedRegion,
	cf_killLine,cf_killRegion,cf_killToBreak,cf_killWord,cf_lastBuf,cf_length,cf_let,cf_lowerCaseLine,cf_lowerCaseRegion,
	...
	cf_onlyWind,cf_openLine,cf_ord,cf_outdentRegion,cf_overwrite,cf_pathname,cf_pause,cf_pop,cf_popBuf,cf_popFile,
	...
	cf_selectScreen,cf_selectWind,cf_setBufFile,cf_setHook,cf_setMark,cf_setWrapCol,cf_seti,cf_shQuote,cf_shell,cf_shellCmd,
	cf_shift,cf_showAliases,cf_showBuffers,cf_showCommands,cf_showDir,cf_showFunctions,cf_showHooks,cf_showKey,
	cf_showKillRing,cf_showMacros,cf_showMarks,cf_showModes,cf_showPoint,
...
// Command argument control flag(s).  May be combined with ArgXXX validation flags or MdXXX mode flags so higher bits are used.
...
typedef struct {
	Datum **curblp;			// Current slot in buffer list (buftab array).  Updated on-demand.
	...
	int mlcol;			// Last cursor column on message line (which may be equal to terminal width).
...
extern int inslines(uint *np,Buffer *srcbufp,Buffer *targbufp,Dot *targdotp,int *statusp,bool *finalDelimp);
...
extern int setfname(Buffer *bufp,char *fname,bool *changedp);
...
SessionInfo si = {			// Session parameters.
	NULL,NULL,NULL,NULL,FPause,NASave,NASave,8,MaxPromptPct,INT_MAX,0U,OpEval | OpStartup | OpScrRedraw,2,1,
