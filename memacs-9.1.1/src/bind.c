...
static int getkb(char *prmt,int n,Datum **argpp,ushort *resultp) {
	...
	if(si.opflags & OpScript) {
		...
		if(mlputs(MLHome | MLFlush,prmt) == Success)
			(void) ((n != INT_MIN && n <= 0) ? getkey(resultp,false) : getkseq(resultp,NULL,false));
		}
...
int showKey(Datum *rp,int n,Datum **argpp) {
	...
	if(mlprintf(MLHome | MLTermAttr,"~#u%s~U -> ~b%s~0",keybuf,name) == Success) {
		if(usage != NULL && mlprintf(MLTermAttr," ~b%s~0",usage) != Success)
			return rc.status;
...
int bindKeyCM(Datum *rp,int n,Datum **argpp) {
	...
	if(univ.p_type == PtrMacro_C)
		return rcset(Failure,RCTermAttr,"%s%s%s%s '~b%s~0'",text418,text416,text417,text414,univ.u.p_bufp->b_bname + 1);
...
int unbindKey(Datum *rp,int n,Datum **argpp) {
	...
	if(!(si.opflags & OpScript))
		(void) mlprintf(MLTermAttr,"~#u%s~U",keybuf);
	else if(n > 0)
