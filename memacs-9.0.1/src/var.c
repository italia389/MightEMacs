	...
	if(svp->u.sv_str != NULL) {
		...
		switch(vid = svp->sv_id) {
			...
			case sv_BufInpDelim:
				...
				goto Kopy;
			case sv_BufModes:
				(void) getmodes(rp,curbp->b_modes);
				break;
			case sv_Date:
				str = timeset();
				goto Kopy;
			case sv_GlobalModes:
				(void) getmodes(rp,modetab[MdIdxGlobal].flags);
				break;
			case sv_HorzScrollCol:
				dsetint((modetab[MdIdxGlobal].flags & MdHScrl) ? curwp->w_face.wf_firstcol :
				 cursp->s_firstcol,rp);
				break;
			case sv_LineLen:
				dsetint((long) curwp->w_face.wf_dot.lnp->l_used,rp);
				break;
			case sv_Match:
			...
			case sv_RegionText:
				...
				if(getregion(&region,true,NULL) != Success)
					return rc.status;
			...
			case sv_ScreenCount:
				...
				break;
			case sv_ShowModes:
				(void) getmodes(rp,modetab[MdIdxShow].flags);
				break;
			case sv_TermCols:
			...
			case sv_TermRows:
				...
				break;
			case sv_WindCount:
				dsetint((long) wincount(cursp,NULL),rp);
				break;
			case sv_autoSave:
			...
			case sv_lineChar:
				...
				dsetchr(dotp->lnp->l_used == dotp->off ? '\n' : dotp->lnp->l_text[dotp->off],rp);
				}
				break;
			case sv_lineCol:
			...
			switch(svp->sv_id) {
				case sv_autoSave:
					...
					if((n = (datp->u.d_int > INT_MAX ? INT_MAX : datp->u.d_int)) == 0) {
						...
						if(modetab[MdIdxGlobal].flags & MdASave) {
							modetab[MdIdxGlobal].flags &= ~MdASave;
				...
				case sv_lineCol:
					...
					break;
				case sv_lineOffset:
					{int llen = curwp->w_face.wf_dot.lnp->l_used;
...
static int uvarnew(char *var,VDesc *vdp) {
	...
	if(cfabsearch(var,NULL,PtrCFAM) == 0)
		return rcset(Failure,RCTermAttr,text165,var);
...
static int svtosf(DStrFab *destp,SVar *svp,ushort flags) {
	...
	if(svp->sv_id == sv_RegionText || (svp->sv_id == sv_lineText && dotp->lnp->l_used > term.t_ncol * 2)) {
		...
		else if(curbp->b_mroot.mk_dot.lnp != dotp->lnp || curbp->b_mroot.mk_dot.off != dotp->off) {
			Region region;
	...
	return dtosfchk(destp,datp,NULL,flags | (svp->sv_id == sv_replacePat || svp->sv_id == sv_searchPat ?
	 CvtVizStrQ : CvtExpr));
	}
...
int setvar(Datum *rp,int n,Datum **argpp) {
	...
	if(dnewtrk(&datp) != 0)
		return librcset(Failure);
	if(terminp(datp,text51,CFNil1,Term_C_SVar,NULL) != Success || datp->d_type == dat_nil)
			// "Assign variable"
		return rc.status;
	...
	if(n == INT_MIN) {
		...
		aflags = CFNil1;
		cflags = (vd.vd_type != VTyp_SVar || !(vd.p.vd_svp->sv_flags & (V_GetKey | V_GetKeySeq))) ? 0 :
		 (vd.p.vd_svp->sv_flags & V_GetKeySeq) ? Term_OneKeySeq : Term_OneKey;
		}
	else {
		delim = RtnKey;
		aflags = CFNotNull1;
...
LibFail:
	...
	// Got new value.  Convert it to an integer if needed.
	...
	switch(req) {
		...
		case SHReqUsage:
			...
			if(dsetstr(svp->sv_name,&scp->sc_name) != 0)
				return librcset(Failure);
			scp->sc_desc = (svp->sv_flags & V_Mode) ? ((ModeSpec *) svp->sv_desc)->desc : svp->sv_desc;
			*namep = svp->sv_name;
			break;
		default: // SHReqValue
