...
static char *showflags(Parse *newp) {
	static char *str,wkbuf[100];

	sprintf(wkbuf,"OpScript %s, last",opflags & OpScript ? "SET" : "CLEAR");
	str = strchr(wkbuf,'\0');
	if(last == NULL)
		str = stpcpy(str," NULL");
	else {
...
int setHook(Datum *rp,int n,Datum **argpp) {
	...
	else if(terminp(datp,text318,CFNotNull1 | CFNil1,0,NULL) != Success || datp->d_type == dat_nil)
			// "Set hook "
		return rc.status;
...
int showHooks(Datum *rp,int n,Datum **argpp) {
	...
	// For all hooks...
	do {
		strn = carat(hrp->h_narg);
		if(dputf(&rpt,"\n%-11s%-22s%-24.*s",hrp->h_name,hrp->h_bufp == NULL ? "" : hrp->h_bufp->b_bname + 1,
		 (int) (strn - hrp->h_narg),hrp->h_narg) != 0)
			goto Fail;
		strm = carat(hrp->h_margs);
		if(dputf(&rpt,"%.*s",(int) (strm - hrp->h_margs),hrp->h_margs) != 0)
			goto Fail;
		if(*strn == '^' || *strm == '^') {
			if(dputf(&rpt,*strn == '^' ? "\n%34s" : "\n%58s","") != 0)
				goto Fail;
			if(*strn == '^' && dputf(&rpt,*strm == '^' ? "%-24s" : "%s",strn + 1) != 0)
				goto Fail;
			if(*strm == '^' && dputs(strm + 1,&rpt) != 0)
Fail:
				return librcset(Failure);
			}
...
int exechook(Datum *rp,int n,HookRec *hrp,uint arginfo,...) {
	...
	if(arginfo != 0) {
		...
		(void) execestmt(rp,cmd.sf_datp->d_str,TokC_Comment,NULL,NULL);
		fprintf(logfile,"exechook(): execestmt() returned rc.msg '%s', status %d.\n",
		 rc.msg.d_str,rc.status); fflush(logfile);
		if(rc.status != Success)
#else
		if(execestmt(rp,cmd.sf_datp->d_str,TokC_Comment,NULL,NULL) != Success)
#endif
			goto Fail;
		fscall = true;
		}
		...
		opflags |= OpNoLoad;
		(void) execbuf(rp,n,hrp->h_bufp,NULL,Arg_First);
...
int run(Datum *rp,int n,Datum **argpp) {
	...
	if((oldscript = opflags & OpScript)) {
		...
		if(opflags & OpEval) {
			...
			if(cfab.p_type == PtrAlias_F)
				return rcset(Failure,RCTermAttr,text314,last->p_tok.d_str);
					// "Cannot execute alias '~b%s~0' interactively"
			}
	...
	// Prompt the user to type a named command, alias, or macro.
			...
			// "No such command, alias, or macro '%s'","Cannot execute alias '~b%s~0' interactively"
	 cfab.p_type == PtrNul)
		return rc.status;
	...
	if(cfab.p_type == PtrMacro)
		(void) execbuf(rp,n,cfab.u.p_bufp,NULL,
		 opflags & OpParens ? Arg_First | SRun_Parens : Arg_First);	// Call the macro.
...
int eval(Datum *rp,int n,Datum **argpp) {
	...
	else if(terminp(datp,": ",CFNotNull1 | CFNil1,n == INT_MIN ? 0 : Term_Eval,NULL) != Success ||
	 datp->d_type == dat_nil)
		return rc.status;
...
#endif
	return execestmt(rp,datp->d_str,TokC_Comment,NULL,NULL);
	}
...
static int margload(Buffer *bufp,Datum *margs,uint flags) {
	...
	if((opflags & (OpScript | OpNoLoad)) == OpScript) {
		...
		uint aflags = (flags & Arg_First) | CFBool1 | CFArray1 | CFNIS1;
...
// Parse and execute a string as an expression statement, given result pointer, string pointer, terminator character, optional
// pointer to pass-through Parse object, and optional pointer in which to return updated string pointer.  This function is
// called anytime a string needs to be executed as a "statement" (without a leading statement keyword); for example, by
// execbuf(), xeqCmdLine(), "#{...}", or the -e switch at startup.  lastflag and thisflag are also updated.
int execestmt(Datum *rp,char *cl,short termch,Parse *newp,char **clp) {
	Parse *oldlast,newlast;
	ENode node;

#if MMDebug & (Debug_Datum | Debug_Script)
	fprintf(logfile,"execestmt(%.8X,\"%s\",%.4hX,%.8X,%.8X): %s...\n",
	 (uint) rp,cl,termch,(uint) newp,(uint) clp,showflags(newp));
	fflush(logfile);
#endif
	...
	if(newp == NULL) {
		newlast.p_flags = opflags & OpScript;
		opflags = (opflags & ~OpParens) | OpScript;
		}
	else {
		newlast.p_flags = newp->p_flags;
		opflags &= ~OpParens;
		}

#if MMDebug & Debug_Script
	fprintf(logfile,"execestmt(): newlast.p_flags set to %.4hX\n",newlast.p_flags);
#endif
	...
	fprintf(logfile,"execestmt(): calling ge_andor() on token '%s', line '%s'...\n",last->p_tok.d_str,last->p_cl);
#endif
...
#if MMDebug & Debug_Expr
	fprintf(logfile,"execestmt(): Returning status %hd\n",rc.status);
	fflush(logfile);
#endif
	...
	parseend(oldlast);
	if(newp == NULL)
		opflags = (opflags & ~OpScript) | newlast.p_flags;
	return rc.status;
	}

// Find an alias by name and return status or boolean result.  If the alias is found: if op is OpQuery or OpCreate, set *app
// (if app not NULL) to the alias structure associated with it; otherwise (op is OpDelete), delete the alias and the associated
// CFAM record.  If op is OpQuery return true; othewise, return status.  If the alias is not found: if op is OpCreate, create a
// new entry, set its pointer record to *cfabp, and set *app (if app not NULL) to it; if op is OpQuery, return false, ignoring
// app; otherwise, return an error.
int afind(char *aname,ushort op,CFABPtr *cfabp,Alias **app) {
	Alias *ap1,*ap2;
	int result;
...
int aliasCFM(Datum *rp,int n,Datum **argpp) {
	...
	else if(terminp(dnamep,text215,CFNotNull1 | CFNil1,0,NULL) != Success || dnamep->d_type == dat_nil)
			// "Create alias "
		return rc.status;
...
static int deleteAM(Datum *rp,char *prmt,uint selector,char *emsg) {
	CFABPtr cfab;
	uint count = 0;
...
Fail:
	...
	if((opflags & OpEval) && disnull(&rc.msg))
...
int deleteAlias(Datum *rp,int n,Datum **argpp) {
	...
	return deleteAM(rp,text269,PtrAlias,text271);
...
int deleteMacro(Datum *rp,int n,Datum **argpp) {
	...
	return deleteAM(rp,text216,PtrMacro,text116);
...
#if MMDebug & Debug_Script
	(void) execbuf(rp,n,bufp,bufp->b_fname,opflags & OpParens ? SRun_Parens : 0);
	fprintf(logfile,"*** xeqBuf(): execbuf() returned '%s' result [%d]",dtype(rp,false),rc.status);
	if(rc.status != Success)
		fprintf(logfile," \"%s\"",rc.msg.d_str);
	fputc('\n',logfile);
	return rc.status;
#else
	return execbuf(rp,n,bufp,bufp->b_fname,opflags & OpParens ? SRun_Parens : 0);
#endif
	}
...
static int ppbuf(Buffer *bufp,uint flags) {
	...
	saltlevel = 0;
	hlp = bufp->b_hdrlnp;
	for(lnp = hlp->l_nextp; lnp != hlp; lnp = lnp->l_nextp) {
...
static int getmacro(Datum *rp,Buffer **mbufpp) {
	...
	if(strlen(last->p_tok.d_str) > NBufName - 1)
		return rcset(Failure,RCTermAttr,text232,last->p_tok.d_str,NBufName - 1);
	...
	if(getsym() != NotFound) {
		...
		if(havesym(s_lparen,false)) {
			...
			if(!needsym(s_rparen,false)) {
				if(funcarg(datp1,Arg_First | CFInt1) != Success)
					return rc.status;
				...
				if(!havesym(s_comma,false))
					...
					if(getsym() < NotFound || (!havesym(s_rparen,false) &&
					 funcarg(datp2,Arg_First | CFInt1) != Success))
						return rc.status;
...
Err:
	...
	if((cfabsearch(rp->d_str + 2,&cfab,PtrCFAM) == 0 && (cfab.p_type != PtrMacro ||
	 !(modetab[MdIdxGlobal].flags & MdClob))) || uvarfind(rp->d_str + 2) != NULL)
...
static int xbuf(Datum *rp,Buffer *bufp,Parse *newp,uint flags) {
	...
	fprintf(logfile,"*** BEGIN xbuf(%u): %s...\n",myinstance,showflags(newp));
#endif
	blnp = elnp = (hlp = bufp->b_hdrlnp)->l_nextp;
	while(elnp != hlp) {
...
DErr:
		...
		if((kwid = kwfind(&eline1,strlen(eline1))) != s_nil) {
			if(thisIsCL) {
				...
				if(parsebegin(newp,eline1,TokC_Comment) != Success)
					goto RCExit;
				switch(kwid) {
					case kw_macro:
...
JumpDown:
					...
					for(lbp = bufp->b_mip->mi_execp; lbp != NULL; lbp = lbp->lb_next)
						if(lbp->lb_mark == blnp) {
							...
							// "endloop" is bypassed.
							elnp = (kwid & SBreakType) ? lbp->lb_jump->l_prevp : lbp->lb_jump;
...
Misplaced:
		...
		if(elevp->live) {
			(void) execestmt(rp,eline1,TokC_Comment,newp,NULL);
...
Onward:
		...
		if(!(lastWasCL = thisIsCL) && newp->p_cl != NULL) {
			parseend(oldlast);
			newp->p_cl = NULL;
			}
		elnp = elnp->l_nextp;
...
#if MMDebug & Debug_MacArg
int _execbuf(Datum *rp,int n,Buffer *bufp,char *runpath,uint flags) {
#else
int execbuf(Datum *rp,int n,Buffer *bufp,char *runpath,uint flags) {
#endif
#if MMDebug & (Debug_Datum | Debug_Script)
	fprintf(logfile,"execbuf(...,%s,...,%.8X): %s...\n",bufp->b_bname,flags,showflags(NULL));
#endif
...
#if MMDebug & Debug_Script
	fprintf(logfile,"execbuf(): newlast.p_flags set to %.4hX\n",newlast.p_flags);
#endif
				// Flag that we are executing the buffer and execute it.
				++bufp->b_mip->mi_nexec;
				(void) xbuf(rp,bufp,&newlast,flags);		// Execute the buffer.
...
#if MMDebug & Debug_MacArg
// Call _execbuf() with debugging.
int execbufDebug(Datum *rp,int n,Buffer *bufp,char *runpath,uint flags) {

	fprintf(logfile,"*execbuf(): n: %d, b_bname: %s, runpath: %s, flags: %.8X...\n",n,bufp->b_bname,runpath,flags);
	fflush(logfile);
	(void) _execbuf(rp,n,bufp,runpath,flags);
	fprintf(logfile,"execbuf(%s) returned status %d [%s]\n",bufp->b_bname,rc.status,rc.msg.d_str);
	fflush(logfile);
	return rc.status;
	}
#endif
...
int execfile(Datum *rp,char *fname,int n,uint flags) {
	...
	if(bfind(fname,CRBCreate | CRBForce | CRBFile,BFHidden,		// Create a hidden buffer...
	 &bufp,NULL) != Success)
		return rc.status;
	bufp->b_modes = MdRdOnly;					// mark the buffer as read-only...
	uint mflag = modetab[MdIdxGlobal].flags & MdMsg;		// save current message flag...
	modetab[MdIdxGlobal].flags &= ~MdMsg;				// clear it...
	(void) readin(bufp,fname,RWKeep | RWExist);			// read the file...
	modetab[MdIdxGlobal].flags |= mflag;				// restore message flag...
	if(rc.status == Success)
...
int xeqFile(Datum *rp,int n,Datum **argpp) {
	...
	if(!(opflags & OpScript)) {
		...
		if(terminp(rp,text129,CFNotNull1 | CFNil1,Term_C_Filename,&ti) != Success || rp->d_type == dat_nil)
				// "Execute script file"
			return rc.status;
		}
