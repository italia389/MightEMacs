...
// Send a command to the terminal to move the hardware cursor to row "row" and column "col" (origin 0).  Update "mlcol" if
...
static int wupd_reframe(EWindow *winp) {
	...
	if(!(winp->w_flags & WFReframe)) {
		...
		if((lnp = winp->w_face.wf_toplnp) == winp->w_bufp->b_hdrlnp) {
			if(winp->w_bufp->b_hdrlnp->l_nextp == winp->w_bufp->b_hdrlnp)
				return rc.status;	// Buffer is empty, no reframe needed.
			winp->w_face.wf_toplnp = winp->w_bufp->b_hdrlnp->l_nextp;
...
Reframe:
	...
	if(!(winp->w_flags & WFReframe)) {
		...
		// Search through the buffer looking for point.  If found, set i to top or bottom line if smooth scrolling,
	...
	wnewtop(winp,winp->w_face.wf_dot.lnp,-i);
...
fprintf(logfile,"pupd_line(): virFlags: %.8X, virLeft: %.8X, virLen: %d '%.*s', phyFlags: %.8X, phyLeft: %.8X, phyLen: %d\n",
 vvlp->v_flags,(uint) virLeft,vvlp->v_len,vvlp->v_len,virLeft,pvlp->v_flags,(uint) phyLeft,pvlp->v_len); fflush(logfile);
#endif
...
static void vupd_wind(EWindow *winp) {
	...
	bool attrOn = (winp->w_bufp->b_flags & BFTermAttr) && (!(modetab[MdIdxGlobal].flags & MdHScrl) ||
	 winp->w_face.wf_firstcol == 0);
	Line *curlnp = (winp == curwp) ? winp->w_face.wf_dot.lnp : NULL;
	...
	do {
		...
		vscreen[row]->v_attrlen = 0;
		taboff = (modetab[MdIdxGlobal].flags & MdHScrl) ? winp->w_face.wf_firstcol :
		 lnp == curlnp ? cursp->s_firstcol : 0;
		vtmove(row,-taboff);
		if(taboff == 0)
...
wkbuf,row,taboff,(int) vscreen[row]->v_flags); getkey(&u);}
#endif
			vtputln(lnp);
			lnp = lnp->l_nextp;
			}
...
static void supd_dex(void) {
	...
	winp = wheadp;
	do {
		attrOn = (winp->w_bufp->b_flags & BFTermAttr) && (!(modetab[MdIdxGlobal].flags & MdHScrl) ||
		 winp->w_face.wf_firstcol == 0);
		lnp = winp->w_face.wf_toplnp;
		endrow = (row = winp->w_toprow) + winp->w_nrows;
		do {
			// Update virtual line if it's currently extended (shifted) or is a dot line that may need rendering.
			if((winp != curwp || lnp != winp->w_face.wf_dot.lnp) && ((vscreen[row]->v_flags & VFExt) ||
...
mlprintf(MLHome | MLFlush,"supd_dex(): De-extending line '%s'...",wkbuf); getkey(&u);
#endif
					vtmove(row,taboff = 0);
					if(attrOn)
						checkAttr(lnp,row);
...
static int pupd_redraw(void) {
	...
	if(movecursor(0,0) != Success || TTeeop() != Success)	// Erase the terminal window.
		return rc.status;
	opflags &= ~OpScrRedraw;				// Clear redraw flag.
...
static void vupd_dotline(EWindow *winp) {
	...
	vscreen[row]->v_flags = vscreen[row]->v_flags & ~(VFRev | VFTermAttr) | VFChgd;
	vscreen[row]->v_attrlen = 0;
	taboff = (modetab[MdIdxGlobal].flags & MdHScrl) ? winp->w_face.wf_firstcol : (winp == curwp) ? cursp->s_firstcol : 0;
	vtmove(row,-taboff);
	if(winp != curwp && (winp->w_bufp->b_flags & BFTermAttr) && (!(modetab[MdIdxGlobal].flags & MdHScrl) ||
	 winp->w_face.wf_firstcol == 0))
		checkAttr(lnp,row);
	vtputln(lnp);
	vteeol();
...
void vupd_modeline(EWindow *winp,Buffer *popbuf) {
	...
	if(winp == curwp)				// Make the current window's mode line stand out.
		lchar = '=';
	...
	if(popbuf == NULL) {
		bufp = winp->w_bufp;
		vtputc((bufp->b_flags & BFNarrowed) ? '<' : lchar);	// "<" if narrowed.
		vtputc((bufp->b_flags & BFChanged) ? '*' : lchar);	// "*" if changed.
		vtputc(' ');
		n = 3;
		...
		md[0].flags = modetab[MdIdxGlobal].flags & modetab[MdIdxShow].flags;
		md[1].flags = winp->w_bufp->b_modes & modetab[MdIdxShow].flags;
		mdp = md;
		do {
			// If bottom screen or not global modes...
			if(winp->w_nextp == NULL || mdp->mask != MdGlobal) {
				msp = modeinfo;
				c = mdp->leadch;
				do {
					// Skip if wrong type of mode.
					if((msp->mask & MdGlobal) != mdp->mask)
						continue;
	...
	// well.
	if(winp == curwp && popbuf == NULL && (modetab[MdIdxGlobal].flags & MdWkDir) && (term.t_ncol - n) > 12) {
		char wkbuf[TT_MaxCols];

		n += vupd_tab(lchar);
		n += vtputs(text274);
			// "WD: "
		n += vtputs(strfit(wkbuf,term.t_ncol - n - 1,cursp->s_wkdir,0)) + 1;
		vtputc(' ');
		}
...
static void vupd_cursor(void) {
	...
	ushort modeflag = ((modetab[MdIdxGlobal].flags & MdHScrl) || (curbp->b_modes & (MdLine | MdCol)) ? WFMode : 0);

	// Find the current row.
	lnp = wfp->wf_toplnp;
	lastrow = cursp->s_cursrow;
	cursp->s_cursrow = curwp->w_toprow;
	...
	while(i < wfp->wf_dot.off)
		cursp->s_curscol = newcol(lnp->l_text[i++],cursp->s_curscol);
	...
	if(modetab[MdIdxGlobal].flags & MdHScrl)
		cursp->s_curscol -= *(firstcolp = &wfp->wf_firstcol);
	else {
		if(cursp->s_cursrow == lastrow)
			cursp->s_curscol -= cursp->s_firstcol;
		else
			cursp->s_firstcol = 0;
		firstcolp = &cursp->s_firstcol;
		}

	// Make sure it is not off the left edge of the screen or on a LineExt ($) character at the left edge.
	while(cursp->s_curscol < 0 || (cursp->s_curscol == 0 && cursp->s_firstcol > 0)) {
		if(*firstcolp >= vtc.hjumpcols) {
			cursp->s_curscol += vtc.hjumpcols;
			*firstcolp -= vtc.hjumpcols;
			}
		else {
			cursp->s_curscol += *firstcolp;
			*firstcolp = 0;
			}
		curwp->w_flags |= (WFHard | modeflag);
		}

	// Calculate window or line shift if needed.
	while(cursp->s_curscol >= term.t_ncol - 1) {
		cursp->s_curscol -= vtc.hjumpcols;
		*firstcolp += vtc.hjumpcols;
		curwp->w_flags |= (WFHard | modeflag);
		}
	...
	if(*firstcolp > 0 && !(modetab[MdIdxGlobal].flags & MdHScrl))
		vscreen[cursp->s_cursrow]->v_flags |= (VFDot | VFExt);
	else
		vscreen[cursp->s_cursrow]->v_flags = vscreen[cursp->s_cursrow]->v_flags & ~VFExt | VFDot;
...
wfp->wf_firstcol,cursp->s_firstcol,cursp->s_curscol); getkey(&u);
#endif
		vupd_wind(curwp);
		}
	else if((curwp->w_flags & WFEdit) || vscreen[cursp->s_cursrow]->v_attrlen > 0)
		vupd_dotline(curwp);
	...
	if((curwp->w_flags & WFMode) || (curbp->b_modes & MdCol) || (curwp->w_flags && (curbp->b_modes & MdLine)))
		vupd_modeline(curwp,NULL);
	curwp->w_flags = 0;

	// If horizontal scrolling mode not enabled and line is shifted, put a '$' in column 0.
	if(!(modetab[MdIdxGlobal].flags & MdHScrl) && cursp->s_firstcol > 0)
		vscreen[cursp->s_cursrow]->v_text[0] = LineExt;
	}
...
int update(int n) {
	...
	if(cursp->s_flags) {		// EScrResize set?
	...
	if(n == INT_MIN || n > 0) {
		...
		if(!(opflags & OpScrRedraw) || pupd_redraw() == Success) {
			...
			if(pupd_all(force) == Success) {
				...
				if(movecursor(cursp->s_cursrow,cursp->s_curscol) == Success)
					(void) TTflush();
				}
			}
		}
...
static int hkey(int (*cfunc)(Datum *rp,int n,Datum **argpp)) {
	...
	do {
		if(kdp->k_cfab.p_type == PtrCmd && kdp->k_cfab.u.p_cfp->cf_func == cfunc) {
			...
			if(!(kdp->k_code & (Prefix | FKey))) {
				if(mlprintf(MLForce | MLTermAttr,"%c~u%s~U",c,ektos(kdp->k_code,wkbuf,false)) != Success)
					return rc.status;
				c = '|';
				found = true;
				}
	...
	return mlprintf(MLForce | MLTermAttr," ~b%c%s~0,",cmd0,cmd);
	}
...
int bpop(Buffer *bufp,ushort flags) {
	...
	lpmax = NULL;
	n = 0;
	for(lnp = bufp->b_hdrlnp->l_nextp; lnp != bufp->b_hdrlnp; lnp = lnp->l_nextp) {
		if(++n > disprows) {
	...
	if(flags & RendShift) {
		...
		for(dot.lnp = bufp->b_hdrlnp->l_nextp; dot.lnp != bufp->b_hdrlnp; dot.lnp = dot.lnp->l_nextp) {
	...
	for(;;) {
		...
		if(n < 0) {
			do {
				...
				if(lpmax == NULL || lnp1 == bufp->b_hdrlnp->l_nextp)
					break;
		...
		// Display prompt.
		if(mlputs(MLHome | MLForce | MLFlush,hprompt != NULL ? hprompt : lpmax == NULL || lnp1 == lpmax ? text201 :
		...
		for(;;) {
			...
			// SPC f C-v +pg, b C-z -pg, d +half, u -half, C-n +ln, C-p -ln, g top, G bot, ESC q quit, ? help:
				if(mlprintf(MLHome | MLForce | MLTermAttr,"~uSPC~U|~uf~U") != Success ||
				 hkey(forwPage) != Success || mlprintf(MLForce | MLTermAttr," ~ub~U") != Success ||
				 hkey(backPage) != Success || mlprintf(MLForce | MLTermAttr,
				 " ~ud~U ~b+half~0, ~uu~U ~b-half~0,") != Success || hkey(forwLine) != Success ||
				 hkey(backLine) != Success || mlprintf(MLForce | MLFlush | MLTermAttr,
