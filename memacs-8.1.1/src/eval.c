...
// Find window on current screen displaying given buffer (name) and return its ordinal number, or zero if not found.
static int bufWind(char *bnamep) {
	EWindow *winp = wheadp;
	int wnum = 0;
	...
	do {
		++wnum;
		if(strcmp(winp->w_bufp->b_bname,bnamep) == 0)
			return wnum;
...
pnbuf:
		...
		case cf_print:
			...
			(void) mlputv(n <= 0 ? MLHOME : MLHOME | MLFORCE,rp);
			break;
		case cf_prompt:
			(void) uprompt(rp,rp1);
			break;
...
// List the names of all the functions.  If default n, make full list; otherwise, get a match string and make partial list of
// function names that contain it, ignoring case.  Render buffer and return status.
