...
static int edinit1(void) {
	...
	*sampbuf.buf = '\0';
	...
	// Initialize the the core-key record and the CAM list with all command and function names.
	usp = (ushort *) &ckeys;
	fab.p_type = PTRCMD;
	cfp = cftab + NFUNCS;
	crp2 = NULL;
...
int help(Value *rp,int n) {
	...
	return (hrp->h_fab.p_type == PTRNUL) ? rcset(FAILURE,0,text246) : exechook(rp,n,hrp,0);
							// "Help hook not set"
	}
