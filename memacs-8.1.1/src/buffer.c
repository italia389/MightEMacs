...
int bufop(Value *rp,int n,char *prmtp,int op,uint flag) {
	Buffer *bufp = NULL;
	...
	switch(op) {
		...
		case BOPCLRFLAG:
			bufp->b_flags &= ~flag;
			break;
		default:	// BOPBEGEND or BOPREADBUF
...
int clearBuf(Value *rp,int n) {
	...
	if(n == INT_MIN)
		...
		if(n <= 0)
			bufp = bdefault(false,false);
		if(bcomplete(rp,text169,n > 0 ? curbp->b_bname : bufp != NULL ? bufp->b_bname : NULL,OPDELETE,&bufp,NULL)
		 != SUCCESS || bufp == NULL)
				// "Clear"
			return rc.status;
		}
...
ask:
	...
	if(getarg(bnamep,prmtp,NULL,CTRL | 'M',ARG_FIRST) != SUCCESS || (!(opflags & OPSCRIPT) && vistfn(bnamep,VNIL)) ||
