...
int gtfilename(Value *rp,char *prmtp,uint flags) {
	...
	return complete(rp,prmtp,curbp->b_fname,CMPL_FILENAME | flags,NPATHINP,ARG_NOTNULL);
	}
...
int fileout(Value *rp,char *prmtp,int mode) {
	...
	// Get the filename.
	if(gtfilename(rp,prmtp,CMPL_NOAUTO) != SUCCESS || ((opflags & OPSCRIPT) == 0 && vistfn(rp,VNIL)))
		return rc.status;
