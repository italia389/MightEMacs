...
// forcefit() table for tfn (true/false/nil), int, and string coersion combinations.
static struct ffinfo {
	...
	ushort str_op;			// Operations which cause call to tostr().  High bits determine left or right side.
		...
		{ /* tfn */
/* tfn */	{FF_ASSIGN | FF_CONCAT | FF_EQNE | FF_LANDOR | FF_COND,0},
/* int */	{FF_ASSIGN | FF_CONCAT | FF_EQNE | FF_LANDOR | FF_COND,FF_CONCAT | FF_EQNE | STR_RIGHT},
/* string */	{FF_ASSIGN | FF_CONCAT | FF_REQNE | FF_EQNE | FF_LANDOR | FF_COND,0}},

		{ /* int */
/* tfn */	{FF_ASSIGN | FF_EQNE | FF_LANDOR | FF_COND,FF_EQNE | STR_LEFT},
/* int */	{~(FF_CONCAT | FF_FORMAT | FF_REQNE),0},
/* string */	{FF_ASSIGN | FF_LANDOR | FF_COND,0}},

		{ /* string */
/* tfn*/	{FF_ASSIGN | FF_CONCAT | FF_FORMAT | FF_REQNE | FF_EQNE | FF_LANDOR | FF_COND,0},
...
static bool lvalue(ENode *np) {
	...
	if((np->en_flags & EN_GNVAR) || ((np->en_flags & EN_IDENT) && uvarfind(np->en_rp->v_strp) != NULL))
		return true;
	if(np->en_flags & EN_IDENT)
...
static void ge_dump(char *fname,ENode *np1,ENode *np2,bool begin) {
	...
	ge_dumpval(np1);
	fprintf(logfile,", flags %0.8x",np1->en_flags);
	if(np2 != NULL) {
		fputs(", val2 ",logfile);
		ge_dumpval(np2);
		fprintf(logfile,", flags %0.8x",np2->en_flags);
		}
...
// Update EN_TFN flag in given node.
static void updtfn(ENode *np) {

	if(vistfn(np->en_rp,VNIL) || vistfn(np->en_rp,VTRUE) || vistfn(np->en_rp,VFALSE))
		np->en_flags |= EN_TFN;
	}
...
static int ge_deref(ENode *np) {
	...
	if(!(opflags & OPEVAL))
		...
		np->en_flags &= ~(EN_IDENT | EN_GNVAR | EN_WHITE);

	// Evaluating.  Dereference and clear flags.
	else if((np->en_flags & EN_GNVAR) || ((np->en_flags & EN_IDENT) && !fabsearch(np->en_rp->v_strp,NULL,PTRFAM))) {
...
static int forcefit(ENode *np1,ENode *np2,ushort kind,char *op) {
	...
	// Get info from table.
	info = fftb[(np1->en_flags & EN_TFN) ? 0 : np1->en_rp->v_type == VALINT ? 1 : 2] +
	 ((np2->en_flags & EN_TFN) ? 0 : np2->en_rp->v_type == VALINT ? 1 : 2);

	// Valid operand types?
	if((info->legal & kind) == 0)
#if MMDEBUG & DEBUG_EXPR
	...
	if(info->str_op & kind) {
		...
		fprintf(logfile,"### forcefit(): coerced %s operand from ",strp);
		ge_dumpval(np);
		tostr(np->en_rp);
		fputs(" to ",logfile);
		ge_dumpval(np);
		fputc('\n',logfile);
#else
		tostr((info->str_op & STR_LEFT) ? np1->en_rp : np2->en_rp);
#endif
		}
	return rc.status;
	}
...
echeck:
		...
		case s_gvar:
		case s_nvar:
			np->en_flags |= EN_GNVAR;
			// Fall through.
		case s_ident:
		case s_identq:
			np->en_flags |= EN_IDENT;
...
#endif
	if(fabsearch(np->en_rp->v_strp,&fab,PTRFAM)) {
#if MMDEBUG & (DEBUG_VALUE | DEBUG_EXPR)
		fputs("found '",logfile);
		switch(fab.p_type) {
			case PTRALIAS:
				fprintf(logfile,"%s' ALIAS",fab.u.p_aliasp->a_name);
				break;
			case PTRMACRO:
				fprintf(logfile,"%s' MACRO",fab.u.p_bufp->b_bname);
				break;
			case PTRCMD:
			case PTRFUNC:
				fprintf(logfile,"%s' %s",fab.p_type == PTRCMD ? "COMMAND" : "FUNCTION",fab.u.p_cfp->cf_name);
			}
		fprintf(logfile," (type %hu) for token '%s'\n",fab.p_type,np->en_rp->v_strp);
#endif
		// Yes.  Resolve any alias.
		if(fab.p_type == PTRALIAS)
			fab = fab.u.p_aliasp->a_fab;

		// Check if interactive only.
		if(fab.p_type == PTRCMD) {
			if(fab.u.p_cfp->cf_flags & CFTERM)
				return rcset(FAILURE,0,text282,fab.u.p_cfp->cf_name);
					// "'%s' command not allowed in a macro (use \"run\")"

			// If "alias" command (which uses "alias xxx = yyy" syntax), parentheses not allowed.
			if(needrparen && fab.u.p_cfp->cf_func == aliasCM)
		...
		// If not evaluating and (1), a command or function, n arg is not the default, and CFNOARGS is set;
		// or (2), xxx() call form (no arguments); skip execution.
		if((opflags & OPEVAL) || ((!(fab.p_type & (PTRCMD | PTRFUNC)) || !(np->en_flags & EN_NEEDFUNC) ||
		 !(fab.u.p_cfp->cf_flags & CFNOARGS)) && (!needrparen || !havesym(s_rparen,false)))) {
			opflags = (opflags & ~OPPARENS) | (needrparen ? OPPARENS : 0);
			int argct = (fab.p_type == PTRMACRO) ? fab.u.p_bufp->b_nargs : ((np->en_flags & EN_NEEDFUNC) &&
			 (fab.u.p_cfp->cf_flags & CFNOARGS)) ? 0 : fab.u.p_cfp->cf_nargs +
			 ((fab.u.p_cfp->cf_flags & CFADDLARG) && np->en_narg != INT_MIN ? 1 : 0);

			// Call the command, function, or macro (as a function) if it's a command or function and CFSPECARGS is
			// set, or evaluating and (1), it's a macro; or (2), it's a command or function and the n argument is
			// not zero or not just a repeat count.
			if(((fab.p_type & (PTRCMD | PTRFUNC)) && (fab.u.p_cfp->cf_flags & CFSPECARGS)) ||
			 ((opflags & OPEVAL) && (fab.p_type == PTRMACRO || np->en_narg != 0 ||
			 !(fab.u.p_cfp->cf_flags & CFNCOUNT)))) {
				...
				if(vnilmm(np->en_rp) == SUCCESS) {	// Set default return value.
					...
					if(fab.p_type == PTRMACRO)
						(void) dobuf(np->en_rp,(int) np->en_narg,fab.u.p_bufp,NULL,
				...
				bool first = true;
				int reqct = (argct >= 0) ? argct : -argct - 1;
				np->en_flags &= EN_CONCAT;
				if(argct != 0 && ((!havesym(s_rparen,false) && havesym(s_any,false)) ||
				 ((opflags & OPEVAL) && argct > 0))) {
					for(;;) {
#if MMDEBUG & DEBUG_EXPR
fprintf(logfile,"### fcall(): consuming (argct %d, reqct %d): examing token '%s' ...\n",argct,reqct,last->p_tok.v_strp);
#endif
						if(first)
							first = false;
			...
			if(argct != 0 && havesym(s_comma,false))
wrong:
	...
	for(;;) {
		...
		switch(last->p_sym) {
			...
			default:
				...
				if((np->en_flags & (EN_IDENT | EN_GNVAR)) == EN_IDENT) {
					bool found;
...
clear:
						...
						np->en_flags &= (EN_TFN | EN_CONCAT);
						break;
						}
					}

				// Not a function call.  Was last operator an infusion (n arg)?
				if(np->en_flags & EN_NEEDFUNC)
nofunc:
			...
			if(sym == s_incr || sym == s_decr) {
				...
				if(opflags & OPEVAL) {
					if(sym != s_not && !intval(np->en_rp))
						return rc.status;
					if(sym == s_not) {
						(void) ltos(np->en_rp,vistrue(np->en_rp) == false);
						np->en_flags |= EN_TFN;
						}
					else if(sym != s_plus)
...
static int ge_binop(ENode *np,OpInfo *oip) {
	...
	for(;;) {
		...
		if(opflags & OPEVAL) {
			if(forcefit(np,&node2,oip->flags,op->v_strp) != SUCCESS)
				goto retn;
			np->en_flags &= ~EN_TFN;
			switch(*symp) {
				...
				case s_band:
					...
					if(oip->flags & FF_CONCAT) {
						...
						if(vopen(&sl,np->en_rp,true) != 0 || vputv(node2.en_rp,&sl) != 0 ||
						 vclose(&sl) != 0)
							(void) vrcset();
						}
					else
...
divzero:
				...
				// Equality and relational: s_eq, s_ne, s_ge, s_gt, s_le, s_lt
				default:
					{long lval1,lval2;
...
static int ge_concat(ENode *np) {
	static enum e_sym sy_concat[] = {s_band,s_any};
	static OpInfo oi_concat = {ge_bitor,sy_concat,FF_CONCAT};

#if MMDEBUG & DEBUG_EXPR
	return ge_binop(np,"ge_bitor",&oi_concat);
#else
	return ge_binop(np,&oi_concat);
#endif
	}
...
#if 0
// Process regular expression equality and inequality operators ~~ and !~.
static int ge_reqne(ENode *np) {
	static enum e_sym sy_reqne[] = {s_req,s_rne,s_any};
	static OpInfo oi_reqne = {ge_rel,sy_reqne,FF_REQNE};
	ENODE *ep1;
	SymType *tp1;

#if MMDEBUG & DEBUG_EXPR
	if((tp1 = ge_binop(&e,"ge_rel",&oi_reqne)) != NULL &&
#else
	if((tp1 = ge_binop(&ep1,&oi_reqne)) != NULL &&
#endif
	 (ep1->kind == en_req || ep1->kind == en_rne))
		renode(ep1->v.e.p1);
	*node = ep1;
...
static int ge_eqne(ENode *np) {
	static enum e_sym sy_eqne[] = {s_eq,s_ne,s_any};
...
andor:
				...
				else if(vistrue(np->en_rp) == b) {		// No, does first argument determine outcome?
...
#endif
					 ltos(np->en_rp,vistrue(node2.en_rp)) != SUCCESS) // and convert to logical.
						goto retn;
					np->en_flags |= EN_TFN;
					}
				break;
...
static int ge_assign(ENode *np) {
	...
	if(last->p_sym < s_assign || last->p_sym > s_asbor) {
		...
		if((opflags & OPEVAL) && (np->en_flags & EN_IDENT)) {
#if MMDEBUG & DEBUG_EXPR
	...
	if(opflags & OPEVAL) {
		if(fabsearch(np->en_rp->v_strp,NULL,PTRFAM))
notvar:
	...
	switch(sym = last->p_sym) {
		...
		case s_asband:
			if((opflags & OPEVAL) && !intvar(&vd)) {
				f = FF_ASSIGN | FF_CONCAT;
				break;
				}
			// else fall through.
		default:
			f = FF_ASSIGN | FF_BITOP;
		}
	...
	vdump(np->en_rp,"ge_assign(): to dest object ...");
				...
				np->en_flags |= node2.en_flags & EN_TFN;
#if VDebug
...
divzero:
			...
			case s_asband:
				if(f & FF_BITOP)
					...
					if(vopen(&sl,np->en_rp,true) != 0 || vputv(node2.en_rp,&sl) != 0 || vclose(&sl) != 0)
						(void) vrcset();
					}
				break;
			case s_asbxor:
...
static int ge_not(ENode *np) {
	...
	if(last->p_sym == kw_not) {
		...
		// Perform operation.
		if(opflags & OPEVAL) {
			(void) ltos(np->en_rp,!vistrue(np->en_rp));
			np->en_flags |= EN_TFN;
			}
		}
	else
...
#endif
					 ltos(np->en_rp,vistrue(node2.en_rp)) != SUCCESS)
						goto retn;
					np->en_flags |= EN_TFN;
					}
				break;
...
int macarg(Value *rp,uint aflags) {
	ENode node;

#if MMDEBUG & DEBUG_EXPR
	fprintf(logfile,"macarg(...,%.8x): BEGIN at token '%s', remainder '%s' ...\n",aflags,last->p_tok.v_strp,last->p_clp);
	fflush(logfile);
#endif
	...
	// If evaluating, a null string, and flag set, return an error.
	if((opflags & OPEVAL) && (aflags & ARG_NOTNULL) && visnull(rp))
