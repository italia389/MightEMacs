	...
	if(kmacro.km_state == KMPLAY) {
		...
		if(kmacro.km_slotp == kmacro.km_endp) {
			...
			if(--kmacro.km_n <= 0) {
				...
				if(kmacro.km_n < 0 && abs(kmacro.km_n) > maxloop)
...
nilrtn:
		...
		if(!quotef) {
			...
			if(ec == (CTRL | '?')) {
				if(cpos != 0) {
					if(mlputc(MLRAW | MLTRACK,'\b') != SUCCESS || TTflush() != SUCCESS)
						return rc.status;
					--cpos;
					}
				continue;
		...
		if(ec & (FKEY | SHFT)) {
			...
			while(*kp) {
				if(cpos < NTERMINP) {
					if((modetab[MDR_GLOBAL].flags & MDKECHO) && mlputc(MLRAW | MLTRACK,*kp) != SUCCESS)
						return rc.status;
					inpbuf[cpos++] = *kp++;
					}
				}
		...
		if(cpos < NTERMINP) {
			...
			if((modetab[MDR_GLOBAL].flags & MDKECHO) && (mlputc(MLTRACK,c) != SUCCESS || TTflush() != SUCCESS))
				return rc.status;
			}
		}
...
static int buildprompt(char *destp,char *prmtp,char *defvalp,int terminator) {
	bool addSpace;
	bool literal = false;
	bool addColon = false;
	char *srcp,*prmtpz;
	...
	if(terminator != (CTRL | 'M')) {
		if(addSpace && vputc(' ',&prompt) != 0)
			return vrcset();
		if(vputc(ectoc(terminator),&prompt) != 0)
			return vrcset();
		addColon = true;
		}
...
// Get a terminal (interactive) argument, given result pointer, prompt, default value, input terminator, and argument flags.
// Home the cursor and write the prompt, default value in brackets (if not NULL), terminator (if not CR) and a colon to the
// message line (with exceptions).  Read back a response and, if ARG_ONEKEY set in aflags, get just one key sequence.  If
// nothing is entered and a default value is given, copy it to the destination buffer and return rc.status (SUCCESS); otherwise,
// return status dependent on aflags, as described in getarg() comments.
int termarg(Value *rp,char *prmtp,char *defvalp,int terminator,uint aflags) {
	char wkbuf[term.t_ncol];
	...
	if(buildprompt(wkbuf,prmtp,defvalp,terminator) != SUCCESS)
		return rc.status;
...
int getarg(Value *rp,char *prmtp,char *defvalp,int terminator,uint aflags) {

	return (opflags & OPSCRIPT) == 0 ? termarg(rp,prmtp,defvalp,terminator,aflags) : macarg(rp,aflags);
	}
...
int complete(Value *rp,char *prmtp,char *defvalp,uint cflags,uint maxlen,uint aflags) {
	...
	if(opflags & OPSCRIPT)
		...
		Buffer *bufp;
		char prbuf[term.t_ncol + NTERMINP + 1],*inpbufp = prbuf + term.t_ncol;	// Prompt and input buffers.
		int status;
		...
		// Display prompt.
		if(buildprompt(prbuf,prmtp,defvalp,CTRL | 'M') != SUCCESS)
			return rc.status;
			...
			if(ec == (CTRL | '?')) {			// Rubout/erase.
			 ...
			 (hdirp = getenv("HOME")) != NULL) {
				...
				if(cpos > 0 && inpbufp[cpos - 1] != '/') {
					if(mlputc(MLRAW | MLTRACK,'/') != SUCCESS)
						return rc.status;
					inpbufp[cpos++] = '/';
					}
				if(TTflush() != SUCCESS)
...
clist:
				...
				inpbufp[cpos++] = ec;
				if(mlputc(MLTRACK,ec) != SUCCESS || TTflush() != SUCCESS)
					return rc.status;
				}
			else {
...
// found" error message.
int getcam(char *prmtp,uint selector,FABPtr *fabp,char *emsg) {
	Value *vp;

	if(vnew(&vp,false) != 0)
		return vrcset();
	if(complete(vp,prmtp,NULL,CMPL_CAM,NTERMINP,selector) != SUCCESS)
		return rc.status;
