...
!macro renderBuf,3
	...
	!else
		...
		!if $1 == $bufName
			setMark
			$3 => gotoLine
			$windSize / 4 => redrawScreen
			notice 'Mark 0 set to previous position'
		!else
			# Buffer being displayed in another window?
			$0 == defn and $0 = 1
			...
			$0 => selectBuf $1
			$0 == 1 && $windSize / 4 => redrawScreen
			$2 || notice 'Old buffer'
		!endif
	!endif
!endmacro

# Determine boundaries of a line block using n arg in same manner as the xxxLine commands.  Set mark 0 to last line of block
...
!macro langFindRoutine,4
	...
	dir = prompt 'Directory to search',haveDefault ? (eval $2) : nil,nil,'f'
	!if nil?(dir) && !haveDefault || null?(dir)
		clearMsg
		!return false
	!endif
	nil?(dir) or eval "#{$2} = subString(dir,-1,1) == '/' && length(dir) > 1 ? subString(dir,0,-1) : dir"

	# Get routine name.
	routineName = prompt "#{$4} name"
	!if nil?(routineName) || null?(routineName)
		clearMsg
		!return false
	!endif

	# Call file scanner.
	!if (isNewBuf = eval "1 => locateFile #{$2},#{quote $3},#{quote sub eval($1),'%NAME%',routineName}") == false
		-1 => notice 'Not found'
	!else
		# Render buffer.
...
# Activate or deactivate a tool package, given (1), tool name; (2), binding list; and (3), Boolean action.  Return Boolean
# result.
!macro toolInit,3
	result = true
	...
	notice "#{$1} tools #{$3 ? 'activated' : 'deactivated'}"
	!return result
!endmacro
