...
static bool iscmd(int key,int (*cfunc)(Value *rp,int n)) {
	KeyDesc *kdp;

	return (kdp = getbind(key)) != NULL && kdp->k_fab.p_type == PTRCMD && kdp->k_fab.u.p_cfp->cf_func == cfunc;
	}
...
static int hkey(StrList *destp,int (*cfunc)(Value *rp,int n),int c) {
	...
	kdp = nextbind(true);
	do {
		if(kdp->k_fab.p_type == PTRCMD && kdp->k_fab.u.p_cfp->cf_func == cfunc) {

			// Found one.  Prefixed?
			if((kdp->k_code & (META | CPREF | HPREF | XPREF | FKEY)) == 0) {

				// No.  Save it.
				if(vputs(strp,destp) != 0)
					return vrcset();
				strp = ",";
				if(vputs(ectos(kdp->k_code,wkbuf,false),destp) != 0)
					return vrcset();
				found = true;
				}
			}
...
int bpop(Buffer *bufp,bool altmodeline,bool endprompt) {
	...
	for(;;) {
		...
		if(n != 0 && lnp1 == lnp)
			...
			// Bail out if called from complete() and one-page buffer.
		...
		// Display prompt.
		mlputs(MLHOME | MLFORCE,hprompt != NULL ? hprompt : lpmax == NULL || lnp1 == lpmax ? text201 : ": ",vz_show);
													// "End: "
		if(TTflush() != SUCCESS)
			return rc.status;
		...
		for(;;) {
			...
			if(c == '?') {
				...
				if(vclose(&msg) != 0)
					return vrcset();
				mlputs(MLHOME | MLFORCE,msg.sl_vp->v_strp,vz_show);
				}
			else {
