...
// before the new text; otherwise, after.  If n < 0, srcp == NULL, and kill is true, get the -nth kill instead of the most
// recent one without cycling the kill ring.  Update kill state if srcp is NULL.  Return status.
int iortext(Value *srcp,int n,enum e_textedit t,bool kill) {
	long size;
