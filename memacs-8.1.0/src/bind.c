...
static int getkb(char *promptp,int n,int *result) {
	...
	if(opflags & OPSCRIPT) {
		...
		if(mlputs(MLHOME | MLFORCE,promptp,vz_show) == SUCCESS)
...
// Get command or macro name.  Store command or buffer pointer in *fabp.  If interactive mode, pass promptp to getcam().
// Return status.
static int getcm(char *promptp,FABPtr *fabp) {
	...
	if(opflags & OPSCRIPT) {
		...
		(void) getcam(promptp,PTRCMD | PTRMACRO,fabp,text130);
					// "No such command or macro '%s'"
	return rc.status;
	}
...
int bindKeyCM(Value *rp,int n) {
	...
	// If interactive mode, change key to something we can print and dump it out.
	if(opflags & OPSCRIPT)
		promptp = NULL;
	else {
		if(mlputs(MLFORCE,ectos(c,keybuf,true),vz_show) != SUCCESS)
			return rc.status;
		sprintf(promptp = wkbuf,"%s%s %s %s",text15,keybuf,text339,text267);
			// "Bind key ","to","command or macro"
		}

	// Get the command or macro name.
	if(((opflags & OPSCRIPT) && !getcomma(true)) || getcm(wkbuf,&fab) != SUCCESS)
		return rc.status;
...
int unbindKey(Value *rp,int n) {
	...
	if(!(opflags & OPSCRIPT))
		(void) mlputs(MLFORCE,wkbuf,vz_show);
...
int aliasCM(Value *rp,int n) {
	...
	else if(mlputs(MLHOME | MLFORCE,text215,vz_show) != SUCCESS || termarg(vnamep,NULL,NULL,CTRL | 'M',0) != SUCCESS ||
	 vistfn(vnamep,VNIL))
				// "Create alias "
		return rc.status;
...
int deleteAM(char *promptp,int type,char *emsg) {
	FABPtr fab;

	// If interactive, get alias or macro name from user.
	if((opflags & OPSCRIPT) == 0) {
		if(getcam(promptp,type,&fab,emsg) != SUCCESS || fab.p_type == PTRNUL)
			return rc.status;
		goto nukeit;
		}
	...
	do {
		if(!havesym(s_ident,false) && !havesym(s_identq,true))
			return rc.status;
		if(opflags & OPEVAL) {
			if(!fabsearch(last->p_tok.v_strp,&fab,type))
...
int apropos(Value *mstrp,char *promptp) {
	char wkbuf[NWORK + 1];

	sprintf(wkbuf,"%s %s",text20,promptp);
			// "Apropos"
...
// List all commands and their bindings, if any (interactive only).  If default n, make full list; otherwise, get a match string
// and make partial list of command names that contain it, ignoring case.  Render buffer and return status.
